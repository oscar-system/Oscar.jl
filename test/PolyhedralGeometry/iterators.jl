@testset "iterators" begin
    ...
    @testset "SubObjectIterator" begin
        ...
        c = cube(2)
        function _testSOI(::Type{Pair{PointVector{fmpq}, AffineHalfspace{fmpq}}}, obj::Polymake.BigObject, i::Base.Integer)
            x = PointVector{fmpq}(obj.VERTICES[i, 2:end])
            a, b = Oscar.decompose_hdata(obj.FACETS)
            y = AffineHalfspace{fmpq}(a[i, :], b[i])
            return Pair{PointVector{fmpq}, AffineHalfspace{fmpq}}(x, y)
        end
        soi = SubObjectIterator{Pair{PointVector{fmpq}, AffineHalfspace{fmpq}}}(Oscar.pm_object(c), _testSOI, 4)
        @test soi isa AbstractVector{Pair{PointVector{fmpq}, AffineHalfspace{fmpq}}}
        @test length(soi) == 4
        @test firstindex(soi) == 1
        @test lastindex(soi) == 4
        for i in 1:4
            p = soi[i]
            @test p[1] == PointVector{fmpq}(Oscar.pm_object(c).VERTICES[i, 2:end])
            a, b = Oscar.decompose_hdata(Oscar.pm_object(c).FACETS)
            @test p[2] == AffineHalfspace{fmpq}(a[i, :], b[i])
        end
