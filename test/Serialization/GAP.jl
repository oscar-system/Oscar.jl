  ...
  mktempdir() do path
    ...
    @testset "IsFreeGroup" begin
      ...
      for wfilt in [getproperty(GAP.Globals, x) for x in wfilts]
        ...
        Fgens = GAPWrap.GeneratorsOfGroup(F)
        U = GAPWrap.Subgroup(F, GapObj([Fgens[1]]))
        ...
        @test GAPWrap.GeneratorsOfGroup(loadedU)[1] in loadedF
        ...
        V = GAPWrap.Subgroup(F, GapObj([Fgens[2]]))
        v = (F, U, V)
        ...
        @test GAPWrap.GeneratorsOfGroup(loaded[2])[1] in loaded[1]
        @test GAPWrap.GeneratorsOfGroup(loaded[3])[1] in loaded[1]
      end
    end
    ...
    @testset "IsSubgroupFpGroup" begin
      ...
      U = GAPWrap.Subgroup(G, GapObj([Ggens[1]]))
      test_save_load_roundtrip(path, U) do loaded
        @test U == loaded
      end
      ...
      loadedU = Oscar.load(filenameU)
      @test GAPWrap.GeneratorsOfGroup(loadedU)[1] in loadedG
      ...
      # full group and subgroup together in the same Julia session
      V = GAPWrap.Subgroup(G, GapObj([Ggens[2]]))
      v = (G, U, V)
      test_save_load_roundtrip(path, v) do loaded
        @test v == loaded
      end
    ...
    @testset "IsPcGroup" begin
      ...
      for (n, i) in paras
        ...
        Oscar.reset_global_serializer_state()
        loaded = Oscar.load(filenamev)
        @test GAPWrap.One(loaded[2]) in loaded[1]
        @test GAPWrap.One(loaded[3]) in loaded[1]
      end
    end
  end
end
