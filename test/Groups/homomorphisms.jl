...
@testset "quo for FPGroup" begin
   ...
   # - Currently GAP's `\/` cannot handle this;
   #   up to GAP 4.12.2, GAP first constructs a quotient group
   #   but later runs into an error when one asks for its order.
   # - Trying to compute the quotient of `h` by the normal closure
   #   in `h` of the given generators runs into a GAP error.
   ...
   # This is handled by creating the normal subgroup generated by the
   # given relators; either this works, or GAP signals an error.
   h = sub(q2, [q2[2]^5])[1]
   q4, epi4 = quo(h, [h[1]^2])
   @test order(q4) == 2
end
