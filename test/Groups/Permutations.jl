...
# Setup permutation groups
g = symmetric_group(4)             # S₄
s = sylow_subgroup(g, 3)[1]        # a Sylow 3-subgroup
e = one(g)                         # identity element of the group
τ = g[1]                           # an example element of g

# Tests for fixed_points on groups 
@test fixed_points(s) == Int[]                    # Sylow 3-subgroup has no fixed points
@test fixed_points(e) == collect(1:degree(g))     # Identity fixes all points
@test fixed_points(g) == Int[]                    # Full symmetric group fixes nothing

# Tests for fixed_points on individual elements 
@test sort(fixed_points(τ)) == sort([i for i in 1:degree(g) if τ[i] == i])

#  Additional example with a specific permutation 
σ = PermGroupElem([1, 2, 3, 4] => [1, 2, 4, 3], g)  # σ = (3 4)
expected_fixed = [1, 2]
@test sort(fixed_points(σ)) == expected_fixed
