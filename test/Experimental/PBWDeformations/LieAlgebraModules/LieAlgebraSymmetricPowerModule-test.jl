@testset "PBWDeformations.LieAlgebraSymmetricPowerModule" begin
  @testset "constructors for R=$R, n=$n" for R in [QQ, cyclotomic_field(4)[1]],
    n in 1:5,
    L in [
      general_linear_liealgebra(R, n),
      special_linear_liealgebra(R, n),
      special_orthogonal_liealgebra(R, n),
    ]
  ...
  @testset "conformance tests for $desc" for (desc, L, V, parentT, elemT) in [
    (
      ...
      LieAlgebraSymmetricPowerModule{QQFieldElem},
      LieAlgebraSymmetricPowerModuleElem{QQFieldElem},
    ),
    (
      "S^2 ⋀^2 V of so_4(QQ)",
    ...
    (
      ...
      symmetric_power(
        standard_module(special_orthogonal_liealgebra(cyclotomic_field(4)[1], 4)), 3
      ),
