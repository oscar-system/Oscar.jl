...
@testset "Linear representations" begin
  ...
  reps = @inferred all_irreducible_representations(E)
  ...
  for i in 1:5
    co = rand(0:2, 5)
    chi = sum([co[j]*chis[j] for j in 1:5])
    ...
    @test character_representation(RR, representation_mapping(rep2)) == exterior_power(representation_mapping(rep), 2)
...
@testset "Projective representations" begin
  fpr = @inferred faithful_projective_representations(32, 49, 4)
  ...
  for prep in fpr
    ...
    @test is_irreducible(prep) == is_irredcible(chi)
    ...
    @test character_linear_lift(prep2) == symmetric_power(conj(chi), 2)
  end
end
...
@testset "Symmetric Grassmannians" begin
  ...
  for prep in fpr
    ...
    for nu in constituents(character_linear_lift(prep), 2)
      cg = @inferred irreducible_compenent(ig, nu)
      @test projective_dimension(cg) <= projective_dimension(ig)
    end

    ac = all_characters(RR, 1)
    prep2 = prep+prep
    dgs = @inferred DeterminantGrassmannian[determinant_grassmannian(prep2, 2, l) for l in ac]
    for j in 1:length(dgs)
      ...
      for cg in irreducible_components(dg)
        b = reduce(vcat, [reduca(vcat,b) for b in standard_element(cg)])
        nu = character_representation(action_on_submodule(linear_lift(prep2, h)))
        @test determinant(nu) == ac[j]
      end
    end
end

@begin "Symmetric intersections" begin
  si = @inferred symmetric_intersections((21, 1), 4, 4, 1)
  for (prep, symci) in si
    ...
    fs, n = @inferred parametrization_data(S)
    @test n == 1
  ...
  si = @inferred symmetric_intersections((96, 204), 4, 3, 4)
  for (prep, symci) in si
    Is = @inferred standard_element.(symci)
