@testset "OscarPolytope" begin
    ...
    @testset "conformance tests" begin
        ...
        @test collect(vertices(Q0)) == collect(vertices(convex_hull(point_matrix(vertices(Q0)))))
    end

    @testset "convex_hull" begin
        @test size(point_matrix(vertices(Q0))) == (3, 2)
        @test size(point_matrix(vertices(Q1))) == (3, 2)
        @test size(point_matrix(rays(Q1))) == (1, 2)
        @test size(point_matrix(lineality_space(Q1))) == (0, 2)
        @test size(point_matrix(vertices(Q2))) == (2, 2)
        @test size(point_matrix(rays(Q2))) == (0, 2)
        @test size(point_matrix(lineality_space(Q2))) == (1, 2)
        @test dim(Q0) == 2
        @test dim(Q1) == 2
        @test dim(Q2) == 2
    ...
    @testset "standard constructions" begin
        ...
        @test C0 == convex_hull(point_matrix(vertices(C0)))
        @test isbounded(C0)
        @test issmooth(C0)
        @test isnormal(C0)
        @test isfeasible(C0)
    ...
    @testset "newton_polytope" begin
        ...
        @test dim(newt) == 2
        @test point_matrix(vertices(newt)) == [4 1; 2 0; 0 2; 0 0]
    end
    ...
    @testset "Construct from fmpq" begin
        ...
        A[1, 1] = 1
        A[3, 2] = 4
        @test point_matrix(vertices(convex_hull(A))) == [1 0; 0 0; 0 4]
