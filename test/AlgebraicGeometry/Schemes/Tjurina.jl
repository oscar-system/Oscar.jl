...
@testset "lokal Tjurina number" begin
  ...
  @test -1 == tjurina_number(L(x^2))
end

@testset "is_finitelydetermined" begin
  R, (x,y) = QQ["x", "y"]
  L,_  = localization(R, complement_of_point_ideal(R, [0, 0]))
  L1,_  = localization(R, complement_of_point_ideal(R, [1, 1]))
  @test_throws ErrorException("Equivalence typ must be ':right' or ':contact'.") is_finitelydetermined(L(0), :leftright)
  @test !is_finitelydetermined(L(0))
  @test !is_finitelydetermined(L(0), :right)  
  @test is_finitelydetermined(L(x^2+y))
  @test is_finitelydetermined(L(x^2+y), :right)
  @test !is_finitelydetermined(L(x^2))
  @test !is_finitelydetermined(L(x^2), :right)  
  @test is_finitelydetermined(L(x^2+y^2))
  @test is_finitelydetermined(L(x^2+y^2), :right)   
  @test is_finitelydetermined(L(1))
  @test !is_finitelydetermined(L(1), :right)    
  @test is_finitelydetermined(L(x^2+1)) 
  @test !is_finitelydetermined(L(x^2+1), :right) 
  @test is_finitelydetermined(L(x^2+y+1))
  @test is_finitelydetermined(L(x^2+y+1), :right)
  @test is_finitelydetermined(L(x^2+y^2+1))
  @test is_finitelydetermined(L(x^2+y^2+1), :right)
  @test is_finitelydetermined(HypersurfaceGerm(AffineScheme(quo(R, ideal(R, x^3+y^2))[1]), [0, 0]))  
  @test is_finitelydetermined(HypersurfaceGerm(AffineScheme(quo(R, ideal(R, x^3+y^2))[1]), [0, 0]), :right)  
  @test is_finitelydetermined(HypersurfaceGerm(AffineScheme(quo(R, ideal(R, x^3+x*y^3))[1]), [0, 1]))  
  @test is_finitelydetermined(HypersurfaceGerm(AffineScheme(quo(R, ideal(R, x^3+x*y^3))[1]), [0, 1]), :right)  
  @test is_finitelydetermined(L1((x-1)^2+(y-1)^2))
  @test is_finitelydetermined(L1((x-1)^2+(y-1)^2), :right)  
  @test is_finitelydetermined(L1((x-1)^2+(y-1)^2+1))
  @test is_finitelydetermined(L1((x-1)^2+(y-1)^2+1), :right) 
end

@testset "is_finitelydetermined positive characteristic" begin
  R, (x,y) = GF(2)["x", "y"]
  L,_  = localization(R, complement_of_point_ideal(R, [0, 0]))
  @test !is_finitelydetermined(L(0))
  @test !is_finitelydetermined(L(0), :right)
  @test is_finitelydetermined(L(1))
  @test !is_finitelydetermined(L(1), :right)
  @test is_finitelydetermined(L(x))
  @test is_finitelydetermined(L(x), :right) 
  @test is_finitelydetermined(L(x^3+y^2))
  @test !is_finitelydetermined(L(x^3+y^2), :right)  
  @test !is_finitelydetermined(L(x^2+y^2))
  @test !is_finitelydetermined(L(x^2+y^2), :right)
end
...
@testset "determinacy_bound positive characteristic" begin
  ...
  @test -1 == determinacy_bound(L(x^3+y^2), :right)  
  @test -1 == determinacy_bound(L(x^2+y^2))
  @test -1 == determinacy_bound(L(x^2+y^2), :right)  
...
@testset "sharper_determinacy_bound" begin
  ...
  L,_  = localization(R, complement_of_point_ideal(R, [0, 0]))
  @test_throws ErrorException("Equivalence typ must be ':right' or ':contact'.") determinacy_bound(L(0), :leftright)
  @test -1 == sharper_determinacy_bound(L(0)) 
  @test -1 == sharper_determinacy_bound(L(0), :right)  
  @test 1 == sharper_determinacy_bound(L(x^2+y))
  @test 1 == sharper_determinacy_bound(L(x^2+y), :right)
  @test -1 == sharper_determinacy_bound(L(x^2))
  @test -1 == sharper_determinacy_bound(L(x^2), :right)  
  ...
  @test -1 == sharper_determinacy_bound(L(1), :right)   
...
@testset "sharper_determinacy_bound positive characteristic" begin
  ...
  @test -1 == sharper_determinacy_bound(L(x^3+y^2), :right)  
  @test -1 == sharper_determinacy_bound(L(x^2+y^2))
  @test -1 == sharper_determinacy_bound(L(x^2+y^2), :right)  
...
@testset "contact equivalence" begin
  ...
  ## not isolated
  @test_throws ErrorException("Unable to determine if is contact equivalent. (Singularity is not isolated)") is_contact_equivalent(L(x^5+x^3), L(-y^3))
end
