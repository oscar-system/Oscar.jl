...
@testset "singular schemes" begin
  ...
  C = proj(B)
  @test defining_ideal(singular_locus(C; algorithm=:projective_jacobian, is_saturate=true)) == ideal(A, [x, y])
  @test defining_ideal(singular_locus(C; algorithm=:projective_jacobian, is_saturate=false)) == ideal(A, [2*x, 2*y, x^2 + y^2])
  @test defining_ideal(singular_locus(C; algorithm=:affine_cone, is_saturate=true)) == ideal(A, [x, y])
  @test defining_ideal(singular_locus(C; algorithm=:affine_cone, is_saturate=false)) == ideal(A, [2*x, 2*y, x^2 + y^2])
...
@testset "properties of projective schemes" begin
  ...
  @test dim(C) == 1
  @test degree(C) == 2
  @test defining_ideal(singular_locus(C; algorithm=:projective_jacobian, is_saturate=true)) == ideal(ambient_coordinate_ring(C), [1])
  @test defining_ideal(singular_locus(C; algorithm=:projective_jacobian, is_saturate=false)) == ideal(ambient_coordinate_ring(C), [2*x, -z, -y, x^2 - y*z])
  @test defining_ideal(singular_locus(C; algorithm=:affine_cone, is_saturate=true)) == ideal(ambient_coordinate_ring(C), [1])
  @test defining_ideal(singular_locus(C; algorithm=:affine_cone, is_saturate=false)) == ideal(ambient_coordinate_ring(C), [2*x, -z, -y, x^2 - y*z])
  @test is_smooth(C; algorithm=:projective_jacobian)
  C = proj(Q)
  @test is_smooth(C; algorithm=:covered_jacobian)
  C = proj(Q)
