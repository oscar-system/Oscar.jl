@testset "trivializations of coherent sheaves" begin
  ...
  U = affine_charts(C)
  @test WC(U[1], V[1]) isa OFPModuleHom
  ...
  UU = simplify(A)
  @test TX(CC[1], UU) isa OFPModuleHom
  UUU = PrincipalOpenSubset(UU, one(OO(UU)))
  @test TX(CC[1], UUU) isa OFPModuleHom
  B = PrincipalOpenSubset(CC[2], one(OO(CC[2])))
  @test Oscar.is_open_func(TX)(A, B)
  @test WX(B, A) isa OFPModuleHom
  @test TX(B, A) isa OFPModuleHom
  ...
  @test WX(BU, A) isa OFPModuleHom
  @test TX(BU, A) isa OFPModuleHom
  @test TX(BU, UU) == compose(TX(BU, A), TX(A, UU))
  @test TX(BU, UUU) == compose(TX(BU, A), TX(A, UUU))
  @test TX(BU, UUU) == compose(TX(BU, UU), TX(UU, UUU))
  @test WX(BUU, A) isa OFPModuleHom
  @test TX(BUU, A) isa OFPModuleHom
