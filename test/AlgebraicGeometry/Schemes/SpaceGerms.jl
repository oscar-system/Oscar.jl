...
@testset "Space Germ constructors AffineScheme-Ideal" begin
  ...
  Xg = AffineScheme(R)
  Xgq = AffineScheme(R,J)
  Xl1 = AffineScheme(R, U0)
  Xl2 = AffineScheme(R, U1)
  Xlq = AffineScheme(R, J ,U0)
...
@testset "germ_at_point constructors 1" begin
  ...
  J = ideal(R, [x-1, y-2])
  X = AffineScheme(R, I*J, units_of(R))
  @test_throws ArgumentError("Base rings must be the same.") AffineScheme(R,ideal(S,[u,v,w]))
...
@testset "SpaceGerm from AffineScheme" begin
  ...
  X = AffineScheme(R, I*J, units_of(R))
  Y = AffineScheme(R, I, units_of(R))
  Z = AffineScheme(R, J ,units_of(R))
  W = AffineScheme(R, K ,units_of(R))
  V = AffineScheme(R, L ,units_of(R))
  SY = AffineScheme(R, ideal(R,[x,y,z]))
  X0 = SpaceGerm(X,[0,0,0])
  Y0 = SpaceGerm(Y,[0,0,0])
