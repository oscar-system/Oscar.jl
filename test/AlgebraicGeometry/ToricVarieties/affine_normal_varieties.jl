@testset "Affine normal toric varieties" begin
  ...
  antv2 = normal_toric_variety(Oscar.positive_hull([1 1; -1 1]))
  ...
  @testset "Basic attributes" begin
    ...
    @test torsion_free_rank(
      domain(map_from_character_lattice_to_torusinvariant_weil_divisor_group(antv))
    ) == 2
    @test torsion_free_rank(
      codomain(map_from_character_lattice_to_torusinvariant_weil_divisor_group(antv))
    ) == 2
    @test elementary_divisors(
      codomain(map_from_torusinvariant_weil_divisor_group_to_class_group(antv))
    ) == [2]
  ...
  @testset "Should fail" begin
    ...
    @test_throws ArgumentError map_from_torusinvariant_cartier_divisor_group_to_picard_group(
      antv4
    )
