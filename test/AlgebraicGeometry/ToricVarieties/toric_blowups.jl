@testset "Toric blowups" begin
  ...
  @testset "Basic tests for simple toric blowup" begin
    ...
    @test matrix(morphism_on_torusinvariant_cartier_divisor_group(bl)) ==
      matrix(morphism_on_torusinvariant_weil_divisor_group(bl))
  end
  ...
  @testset "Arithmetics, comparison and composition" begin
    ...
    @test lattice_homomorphism(bl * toric_identity_morphism(codomain(bl))) ==
      lattice_homomorphism(bl)
    @test bl !== bl2
  end
