...
# 1) Create a PR that contains only the formatting changes automatically
#    generated by JuliaFormatter - no other modifications.
# 2) Create a second PR with the following updates:
#    a) Add the commit hash from the merge commit of step 1 to `.git-blame-ignore-revs`, so that
