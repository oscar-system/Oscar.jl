...
function _compute_glueing_base_change(gd::BaseChangeGlueingData)
  ...
  UU, map_U = change_base(phi, U, ambient_map=pc1)
  VV, map_V = change_base(phi, V, ambient_map=pc2)
  ...
  _, ff, _ = change_base(phi, f, domain_map=map_U, codomain_map=map_V)
  _, gg, _ = change_base(phi, g, domain_map=map_V, codomain_map=map_U)

  return Glueing(XX, YY, ff, gg)
end

function change_base(phi::Any, G::AbsGlueing;
    patch_change1::AbsSpecMor=change_base(phi, glueing_domains(G)[1])[2], # The base change morphism for 
    patch_change2::AbsSpecMor=change_base(phi, glueing_domains(G)[2])[2]  # the two glueing patches
  )
