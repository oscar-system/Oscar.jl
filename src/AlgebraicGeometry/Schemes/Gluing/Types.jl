...
@attributes mutable struct Gluing{
  ...
  function Gluing(
    ...
    @check compose(f, g) == id_hom(domain(f)) "gluing maps are not inverse of each other"
    @check compose(g, f) == id_hom(domain(g)) "gluing maps are not inverse of each other"
