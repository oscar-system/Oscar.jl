...
function base_map(phi::ProjectiveSchemeMor)
  if !isdefined(phi, :map_on_base_schemes)
    phi.map_on_base_schemes = affine_morphism(base_scheme(domain(phi)), base_scheme(codomain(phi)), coefficient_map(pullback(phi)))
  end
...
function map_on_affine_cones(
  ...
  if !isdefined(phi, :map_on_affine_cones)
    ...
    phi.map_on_affine_cones = affine_morphism(C_dom, C_cod, pb_res)
  end
...
function map_on_affine_cones(
  ...
  if !isdefined(phi, :map_on_affine_cones)
    ...
    phi.map_on_affine_cones = affine_morphism(CP, CQ, vcat(imgs_fiber, imgs_base), check=check)
  end
...
function map_on_affine_cones(phi::ProjectiveSchemeMor{<:AbsProjectiveScheme{<:SpecOpenRing}, <:AbsProjectiveScheme{<:SpecOpenRing}})
  if !isdefined(phi, :map_on_affine_cones)
    ...
    list = [affine_morphism(CX[i], Q, restriction_map(CX, CX[i]).(coord_imgs)) for i in 1:ngens(CX)]
