...
function pushforward(inc::ProjectiveClosedEmbedding, M::SubquoModule)
  ...
  gT = [sum(lift(x[i])*FT[i] for i in 1:length(g); init=zero(FT)) for x in coordinates.(g)]
  rel = relations(M)
  relT = [sum(lift(x[i])*FT[i] for i in 1:length(g); init=zero(FT)) for x in coordinates.(rel)]
  G, inc_G = sub(FT, vcat(gT, relT))
  Q, inc_Q = sub(G, gens(G)[length(gT)+1:end])
  MT = cokernel(inc_Q)
  id = hom(MT, M, vcat(gens(M), [zero(M) for i in 1:length(relT)]))
  return MT, id
end
