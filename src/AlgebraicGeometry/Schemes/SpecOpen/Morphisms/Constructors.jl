...
function SpecOpenMor(U::SpecOpen, V::SpecOpen, f::Vector{<:RingElem}; check::Bool=true)
  Y = ambient_scheme(V)
  maps = [affine_morphism(W, Y, f, check=check) for W in affine_patches(U)]
  return SpecOpenMor(U, V, [affine_morphism(W, Y, f, check=check) for W in affine_patches(U)], check=check) 
end
...
function SpecOpenMor(X::SpecType, d::RET, Y::SpecType, e::RET, f::Vector{RET}; check::Bool=true) where {SpecType<:Spec, RET<:RingElem}
  ...
  return SpecOpenMor(U, V, [affine_morphism(U[1], Y, OO(U[1]).(f), check=check)], check=check)
end
...
function identity_map(U::SpecOpen) 
  phi = SpecOpenMor(U, U, 
                    [affine_morphism(V, ambient_scheme(U), gens(OO(V)), check=false) for V in affine_patches(U)],
                    check=false
                   )
  return phi
end
...
function restrict(f::SpecOpenMor, W::AbsSpec, Y::AbsSpec; check::Bool=true)
  ...
  return affine_morphism(W, Y, fy, check=false)
end
...
function maximal_extension(
  ...
  for i in 1:n
    push!(maps, affine_morphism(affine_patches(U)[i], Y, [restrictions(a)[i] for a in g]))
  end
