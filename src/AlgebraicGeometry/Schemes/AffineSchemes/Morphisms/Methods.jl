...
function fiber_product(f::PrincipalOpenEmbedding, g::AbsSpecMor)
  ...
  ff = PrincipalOpenEmbedding(affine_morphism(result, B, gens(OO(result)), check=false), pbh, check=false)
...
function _induced_map_to_fiber_product(
  ...
  return affine_morphism(W, XxY, img_gens, check=check)
end
...
function _restrict_domain(f::AbsSpecMor, D::PrincipalOpenSubset; check::Bool=true)
  D === domain(f) && return f
  ambient_scheme(D) === domain(f) && return affine_morphism(D, codomain(f), OO(D).(pullback(f).(gens(OO(codomain(f))))), check=false)
  @check is_subset(D, domain(f)) "domain incompatible"
  return affine_morphism(D, codomain(f), OO(D).(pullback(f).(gens(OO(codomain(f))))), check=check)
end
...
function _restrict_codomain(f::AbsSpecMor, D::PrincipalOpenSubset; check::Bool=true)
  ...
  if ambient_scheme(D) === codomain(f) 
    ...
    return affine_morphism(domain(f), D, OO(domain(f)).(pullback(f).(gens(OO(codomain(f))))), check=false)
  end
  @check is_subset(D, codomain(f)) "codomain incompatible"
  @check is_subset(domain(f), preimage(f, D))
  return affine_morphism(domain(f), D, OO(domain(f)).(pullback(f).(gens(OO(codomain(f))))), check=check)
end
...
function product(X::AbsSpec{BRT, RT}, Y::AbsSpec{BRT, RT};
  ...
  XxY = Spec(KL)
  pr1 = affine_morphism(XxY, X, gens(KL)[1:m], check=false)
  pr2 = affine_morphism(XxY, Y, gens(KL)[m+1:m+n], check=false)
  return XxY, pr1, pr2
end
...
function product(X::StdSpec, Y::StdSpec;
  ...
  pr1 = affine_morphism(XxY, X, gens(RS)[1:m], check=false)
  pr2 = affine_morphism(XxY, Y, gens(RS)[m+1:m+n], check=false)
  return XxY, pr1, pr2
end
=#
...
function base_change(phi::Any, f::AbsSpecMor; 
  ...
  return domain_map, affine_morphism(XX, YY, pbF, check=false), codomain_map # TODO: Set to false after testing
end
