...
julia> affine_morphism(X, Y, gens(OO(X)))
...
```
"""
function affine_morphism(
      X::AbsSpec,
      Y::AbsSpec,
      f::Vector{<:RingElem};
      check::Bool=true
  )
  return affine_morphism(X, Y, hom(OO(Y), OO(X), OO(X).(f), check=check), check=check)
end
...
function affine_morphism(
  ...
  return affine_morphism(X, Y, OO(X).(f), check=check)
end
...
```
"""
identity_map(X::AbsSpec{<:Any, <:MPolyRing}) = affine_morphism(X, X, hom(OO(X), OO(X), gens(OO(X)), check=false), check=false)
identity_map(X::AbsSpec{<:Any, <:MPolyQuoLocRing}) = affine_morphism(X, X, hom(OO(X), OO(X), gens(ambient_coordinate_ring(X)), check=false), check=false)
identity_map(X::AbsSpec{<:Any, <:MPolyLocRing}) = affine_morphism(X, X, hom(OO(X), OO(X), gens(ambient_coordinate_ring(X)), check=false), check=false)
identity_map(X::AbsSpec{<:Any, <:MPolyQuoRing}) = affine_morphism(X, X, hom(OO(X), OO(X), gens(ambient_coordinate_ring(X)), check=false), check=false)
...
```
"""
inclusion_morphism(X::AbsSpec, Y::AbsSpec; check::Bool=true) = affine_morphism(X, Y, gens(ambient_coordinate_ring(Y)), check=check)
...
function compose(f::AbsSpecMor, g::AbsSpecMor)
  ...
  return affine_morphism(domain(f), codomain(g), compose(pullback(g), pullback(f)), check=false)
end
...
function restrict(f::SpecMor, U::AbsSpec, V::AbsSpec; check::Bool=true)
  ...
  return affine_morphism(U, V, (x->OO(U)(x, check=check)).(pullback(f).(gens(domain(pullback(f))))), check=check)
end
