@attributes mutable struct ProjectiveVariety{BaseRing<:Field, RingType<:MPolyAnyRing} <: AbsProjectiveVariety{BaseRing, RingType}
  X::Spec

  function ProjectiveVariety(X::Spec{S, T}; check::Bool) where {S, T}
    if check
