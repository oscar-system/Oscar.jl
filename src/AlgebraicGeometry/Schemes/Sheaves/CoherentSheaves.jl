...
@attributes mutable struct SheafOfModules{SpaceType, OpenType,
  ...
  ID::IdDict{AbsAffineScheme, OFPModule} # the modules on the basic patches of the default covering
  ...
  function SheafOfModules(X::AbsCoveredScheme,
      MD::IdDict{AbsAffineScheme, OFPModule}, # A dictionary of modules on the `affine_charts` of `X`
      MG::IdDict{Tuple{AbsAffineScheme, AbsAffineScheme}, MatrixElem}; # A dictionary for pairs `(U, V)` of
                                                       # `affine_charts` of `X` such that
                                                       # A = MG[(U, V)] has entries aᵢⱼ with
                                                       # gᵢ = ∑ⱼ aᵢⱼ ⋅ fⱼ on U ∩ V with gᵢ the
    ...
    Mpre = PreSheafOnScheme(X,
                      OpenType=AbsAffineScheme, OutputType=OFPModule,
                      RestrictionType=Map,
                      is_open_func=_is_open_func_for_schemes_without_affine_scheme_open_subscheme(X)
                      #is_open_func=_is_open_for_modules(X)
                     )
    M = new{typeof(X), AbsAffineScheme, OFPModule, Map}(MD, MG, OOX, Mpre, default_cov)
    @check begin
...
function twisting_sheaf(IP::AbsProjectiveScheme{<:Field}, d::Int)
  ...
  X = covered_scheme(IP)
  MD = IdDict{AbsAffineScheme, OFPModule}()
...
@attr OFPModule function cotangent_module(X::AbsAffineScheme{<:Field, <:MPolyRing})
  R = OO(X)
  F = FreeMod(R, ["d$(x)" for x in symbols(R)])
  return F
end

@attr OFPModule function cotangent_module(X::AbsAffineScheme{<:Field, <:MPolyLocRing})
  R = OO(X)
  P = base_ring(R)
...
@attr OFPModule function cotangent_module(X::AbsAffineScheme{<:Field, <:MPolyQuoRing})
  R = OO(X)
  P = base_ring(R)
  F = FreeMod(R, ["d$(x)" for x in symbols(P)])
...
@attr OFPModule function cotangent_module(X::AbsAffineScheme{<:Field, <:MPolyQuoLocRing})
  R = OO(X)
  P = base_ring(R)
  F = FreeMod(R, ["d$(x)" for x in symbols(P)])
  rels, _ = sub(F, transpose(change_base_ring(R, jacobian_matrix(base_ring(modulus(R)), gens(modulus(R))))))
...
julia> typeof(T)
Oscar.HomSheaf{CoveredScheme{FqField}, AbsAffineScheme, OFPModule, Map}
...
@attributes mutable struct HomSheaf{SpaceType, OpenType, OutputType,
  ...
  function HomSheaf(F::AbsCoherentSheaf, G::AbsCoherentSheaf)
    ...
    M = new{typeof(X), AbsAffineScheme, OFPModule, Map}(F, G, OOX, Mpre)
...
julia> typeof(W)
DirectSumSheaf{CoveredScheme{FqField}, AbsAffineScheme, OFPModule, Map}
...
@attributes mutable struct DirectSumSheaf{SpaceType, OpenType, OutputType,
  ...
  function DirectSumSheaf(X::AbsCoveredScheme, summands::Vector{<:AbsCoherentSheaf})
    ...
    M = new{typeof(X), AbsAffineScheme, OFPModule, Map}(summands, OOX, Mpre)
...
julia> typeof(inc_F)
PushforwardSheaf{NormalToricVariety, AbsAffineScheme, OFPModule, Map}
...
@attributes mutable struct PushforwardSheaf{SpaceType, OpenType, OutputType,
  ...
  function PushforwardSheaf(inc::CoveredClosedEmbedding, M::AbsCoherentSheaf)
    ...
    MY = new{typeof(Y), AbsAffineScheme, OFPModule, Map}(inc, OOX, OOY, M, ident, Blubber)
    return MY
  end
end
...
julia> typeof(inc_F)
PullbackSheaf{CoveredScheme{QQField}, AbsAffineScheme, OFPModule, Map}
...
@attributes mutable struct PullbackSheaf{SpaceType, OpenType, OutputType,
  ...
  function PullbackSheaf(f::AbsCoveredSchemeMorphism, M::AbsCoherentSheaf)
    ...
    MY = new{typeof(X), AbsAffineScheme, OFPModule, Map}(f, OOX, OOY, M, pullbacks, Blubber)
    return MY
  end
end
...
@attr MatrixElem function _presentation_matrix(M::OFPModule)
