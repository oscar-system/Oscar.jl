...
function toric_ideal(R::MPolyRing, pts::ZZMatrix)
    laurent_parent, x = polynomial_ring(base_ring(R), 2*ncols(pts); cached=false)
    laurent_eqs = ideal(laurent_parent, [x[i]*x[ncols(pts)+i]-1 for i in 1:ncols(pts)])
    images = []
    for i in 1:nrows(pts)
      exppos = [h >= 0 ? h : 0 for h in pts[i,:]]
      expneg = [h <= 0 ? -h : 0 for h in pts[i,:]]
      push!(images, prod(x.^vcat(exppos, expneg)))
    end
    f = hom(R, laurent_parent, images)
    return preimage(f, laurent_eqs)
end
