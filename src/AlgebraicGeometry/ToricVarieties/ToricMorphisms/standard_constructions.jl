...
@attr ToricMorphism function morphism_from_cox_variety(variety::NormalToricVarietyType)
  ...
  rays_for_cox_variety = matrix(
    ZZ, [[
      if i == j
        1
      else
        0
      end for j in 1:n_rays(variety)
    ] for i in 1:n_rays(variety)]
  )
  cox_variety = normal_toric_variety(
    polyhedral_fan(max_cones_for_cox_variety, rays_for_cox_variety)
  )
