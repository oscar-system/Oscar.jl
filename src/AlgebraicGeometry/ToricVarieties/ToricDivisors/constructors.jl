...
@attributes mutable struct ToricDivisor <: AbsWeilDivisor{NormalToricVariety,ZZRing}
  ...
  ToricDivisor(polymake_divisor::Polymake.BigObject,
    toric_variety::NormalToricVarietyType,
    coeffs::Vector{T}) where {T<:IntegerUnion} = new(
    polymake_divisor, toric_variety, [ZZRingElem(c) for c in coeffs]
  )
end
...
```
"""
function divisor_of_character(
  v::NormalToricVarietyType, character::Vector{T}
...
Base.:*(c::T, td::ToricDivisor) where {T<:IntegerUnion} = toric_divisor(
  toric_variety(td), [ZZRingElem(c) * x for x in coefficients(td)]
)
# method ambiguity requires the implementation of the following method
Base.:*(c::ZZRingElem, td::ToricDivisor) = toric_divisor(
  toric_variety(td), [c * x for x in coefficients(td)]
)
...
function Base.show(io::IO, td::ToricDivisor)
  ...
  push_attribute_if_exists!(
    properties_string, td, :is_cartier, "cartier"; callback=(q_car_cb!)
  )
