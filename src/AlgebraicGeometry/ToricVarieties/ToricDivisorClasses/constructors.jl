...
@attributes mutable struct ToricDivisorClass
    ...
    function ToricDivisorClass(toric_variety::NormalToricVarietyType, class::FinGenAbGroupElem)
        @req parent(class) === class_group(toric_variety)[1] "The class must belong to the class group of the toric variety"
...
```
"""
toric_divisor_class(v::NormalToricVarietyType, coeffs::Vector{T}) where {T <: IntegerUnion} = ToricDivisorClass(v, class_group(v)[1]([ZZRingElem(c) for c in coeffs]))
