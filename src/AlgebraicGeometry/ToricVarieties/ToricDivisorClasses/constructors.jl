...
@attributes mutable struct ToricDivisorClass
  ...
  function ToricDivisorClass(
    toric_variety::NormalToricVarietyType, class::FinGenAbGroupElem
  )
...
toric_divisor_class(v::NormalToricVarietyType, coeffs::Vector{T}) where {T<:IntegerUnion} =
  ToricDivisorClass(
    v, class_group_with_map(v)[1]([ZZRingElem(c) for c in coeffs])
  )
...
Base.:*(c::T, tdc::ToricDivisorClass) where {T<:IntegerUnion} = toric_divisor_class(
  toric_variety(tdc), ZZRingElem(c) * divisor_class(tdc)
)
...
function Base.:(==)(tdc1::ToricDivisorClass, tdc2::ToricDivisorClass)
  return toric_variety(tdc1) === toric_variety(tdc2) &&
         divisor_class(tdc1) == divisor_class(tdc2)
end
