...
function forget_toric_structure(X::AffineNormalToricVariety)
  ...
  iso = affine_morphism(Y, X, identity_map(OO(X)), check=true)
  iso_inv = affine_morphism(X, Y, identity_map(OO(X)), check=true)
...
# V = affine_charts(X)[j]
  ...
  xx = gens(OO(UV))
  yy = gens(OO(VU))
  f = affine_morphism(UV, VU, [prod((e[i] >= 0 ? u^e[i] : inv(u)^-e[i]) for (i, u) in enumerate(xx); init=one(OO(UV))) for e in y_to_x], check=true)
  g = affine_morphism(VU, UV, [prod((e[i] >= 0 ? v^e[i] : inv(v)^-e[i]) for (i, v) in enumerate(yy); init=one(OO(VU))) for e in x_to_y], check=true)
...
function _compute_gluings(X::AffineNormalToricVariety, Y::AffineNormalToricVariety, vmat::ZZMatrix, U::PrincipalOpenSubset, V::PrincipalOpenSubset)
  ...
  f = affine_morphism(U, V, fres)
  g = affine_morphism(V, U, gres)
  set_attribute!(g, :inverse, f)
  set_attribute!(f, :inverse, g)
