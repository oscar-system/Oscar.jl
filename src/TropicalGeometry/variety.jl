...
function tropical_variety(I::Union{MPolyIdeal,MPolyRingElem}, nu::Union{TropicalSemiringMap,Nothing}=nothing; weighted_polyhedral_complex_only::Bool=false, skip_saturation::Bool=false, skip_primary_decomposition::Bool=false)
    ...
    for P in toTropicalize
        ...
        elseif max(total_degree.(GB)...)==1
            ...
            # P general
            TropV = tropical_variety_singular(P,nu,weighted_polyhedral_complex_only=weighted_polyhedral_complex_only)
        end
...
# this function takes the string of a gfan fan and converts it to an OSCAR polyhedral complex
...
function singular_fan_string_to_oscar_complex(input_string::String, negateFan::Bool=false, dehomogenizeFan::Bool=false)
    ...
    # Convert MAXIMAL_CONES to a Matrix (for later manipulation)
    coneIncidences = Base.stack([parse.(Int, split(replace(line, r"[{}]" => ""), r"\s+")) .+ 1 for line in stringsParsed[3]],dims=1)
    ...
    if dehomogenizeFan
        ...
        coneIncidences = hcat(coneIncidences, ones(Int,nrows(coneIncidences),1))
        return polyhedral_complex(IncidenceMatrix(coneIncidences), rayGenerators, rayIndices, linealityGenerators)
    end
...
function compute_weights_and_construct_tropical_variety(Sigma::PolyhedralComplex,I::MPolyIdeal,nu::TropicalSemiringMap)
...
function tropical_variety_singular(I::MPolyIdeal, nu::TropicalSemiringMap{QQField,Nothing,<:Union{typeof(min),typeof(max)}}; weighted_polyhedral_complex_only::Bool=false)
    R = base_ring(I)
    ...
    Sigma = singular_fan_string_to_oscar_complex(TropIString,convention(nu)==max,false)
    TropI = compute_weights_and_construct_tropical_variety(Sigma,I,nu)
    return TropI
end

function tropical_variety_singular(I::MPolyIdeal, nu::TropicalSemiringMap{QQField,ZZRingElem,<:Union{typeof(min),typeof(max)}}; weighted_polyhedral_complex_only::Bool=false)
    R = base_ring(I)
    singularCommand = join(["ring r=0,("*join(string.(symbols(R)),",")*"),dp;",
                            "ideal I = "*join(string.(gens(I)), ",")*";",
                            "if (!defined(tropicalVariety)) { LIB \"tropical.lib\"; };",
    ...
    Singular.call_interpreter(singularCommand)
    TropIString = Singular.lookup_library_symbol("Top", "TropIString")
    Sigma = singular_fan_string_to_oscar_complex(TropIString,convention(nu)==max,true)
    TropI = compute_weights_and_construct_tropical_variety(Sigma,I,nu)
    return TropI
end
