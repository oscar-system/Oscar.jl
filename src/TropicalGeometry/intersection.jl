...
function intersect_after_lex_perturbation_with_intersection(sigma1::Polyhedron{QQFieldElem},sigma2::Polyhedron{QQFieldElem})
    ...
    # Quick test 2: return false if the affine hulls of sigma1 and sigma2 span the entire space
    ...
    # C_0(sigma1-sigma2) = C_p(sigma1)-C_p(sigma2) for p some relative interior
    # point of sigma12.
    ###

    p = relative_interior_point(sigma12)

    # construct C_p(sigma1) from equations and active facets at p
    E1 = affine_equation_matrix(affine_hull(sigma1))
    F1 = affine_inequality_matrix(facets(sigma1))
    activeFacets = findall(iszero,F1*vcat([1],p))
    Cp1 = cone_from_inequalities(F1[activeFacets,2:end],E1[:,2:end])

    # construct -C_p(sigma1) from equations and active facets at p negated
    E2 = affine_equation_matrix(affine_hull(sigma2))
    F2 = affine_inequality_matrix(facets(sigma2))
    activeFacets = findall(iszero,F2*vcat([1],p))
    Cp2Neg = cone_from_inequalities(-F2[activeFacets,2:end],E2[:,2:end])

    # construct C_0(sigma1-sigma2) = C_p(sigma1)-C_p(sigma2).  there is no
    # minkowski sum for cones in Oscar or Polymake, so we use rays and lineality
    R1, L1 = rays_modulo_lineality(Cp1)
    R2, L2 = rays_modulo_lineality(Cp2Neg)
    R12 = (isempty(R1) && isempty(R2)) ? zero_matrix(QQ,0,n) : vcat(R1,R2)
    Cp12 = positive_hull(R12, vcat(L1,L2))
