...
function tropical_intersection_multiplicity(sigma1::Polyhedron,sigma2::Polyhedron)
    K1 = kernel(affine_equation_matrix(affine_hull(sigma1))[:,2:end], side = :right)
    B1 = saturate(matrix(ZZ,[ numerator.(K1[:, i] .* lcm(denominator.(K1[:, i]))) for i in 1:ncols(K1) ]))

    K2 = kernel(affine_equation_matrix(affine_hull(sigma2))[:,2:end], side = :right)
    B2 = saturate(matrix(ZZ,[ numerator.(K2[:, i] .* lcm(denominator.(K2[:, i]))) for i in 1:ncols(K2) ]))
