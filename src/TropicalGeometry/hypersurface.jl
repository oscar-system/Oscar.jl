...
Return the tropical prevariety generated by intersecting tropical hypersurfaces obtained from elements of $A$.
"""
function to_tropical(A,minOrMax)    
    nu = tropical_semiring_map(QQ,minOrMax)
    return [tropical_polynomial(g,nu) for g in A]
end

function p_tropical_hypersurface(tf)
...
function tropical_prevariety(A,minOrMax)
    TA = to_tropical(A,minOrMax)
    HTA = [p_tropical_hypersurface(tf) for tf in TA]
    RTA = [add_lineality(h) for h in HTA]
    return reduce(Polymake.fan.common_refinement,RTA)
end
