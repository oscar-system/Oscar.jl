...
```
"""
function simulate_valuation(
    I::MPolyIdeal,
    nu::TropicalSemiringMap{fieldType,uniformizerType,conventionType},
    w::AbstractVector{QQFieldElem}
    ) where {
        fieldType <: Field,
        uniformizerType <: RingElem,
        conventionType <: Union{typeof(min),typeof(max)}
    }
...
# special case: trivial valuation, no change in ideal required
function simulate_valuation(
    I::MPolyIdeal,
    ::TropicalSemiringMap{fieldType,Nothing,conventionType},
    w::AbstractVector{QQFieldElem}
    ) where {
        fieldType <: Field,
        conventionType<:Union{typeof(min),typeof(max)}
    ...
    # negate w if min-convention, and scale until integral
