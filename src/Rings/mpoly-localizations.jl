...
Given a multivariate polynomial ring ``R`` over a field ``K``, say ``R = K[x_1,\dots, x_n]``, and
given a vector ``a = (a_1, \dots, a_n)`` of ``n`` elements of ``K``, return the multiplicatively
...
Given an element `f` of a multivariate polynomial ring `R`, say, return the multiplicatively
closed subset of `R` which is formed by the powers of `f`.
...
Given an ideal `I` of a localization, say, `RL` of a multivariate polynomial ring, say, `R`,
return the saturation of `I` over `R`. That is, return the largest ideal of `R` whose extension to 
`RL` is `I`. This is the preimage of `I` under the localization map.
...
@doc raw"""    
    ideal(RL::MPolyLocRing, V::Vector)

Given a localization `RL` of a multivariate polynomial ring, and given a vector `V` of
elements of `RL`, create the ideal of `RL` which is generated by the entries of `V`.
"""
function ideal(
    W::MPolyLocRing,
    gens::Vector
...
Given a localized ring `RL`of type `MPolyLocRing`, say `RL` is the localization
...
the homomorphism from `RL` to `S` whose composition with the localization map is `phi`.

    hom(RL::MPolyLocRing, S::Ring, V::Vector)

Given a localized ring `RL` as above, and given a vector `V` of `nvars` elements of `S`, let `phi`
be the homomorphism from `R` to `S` which is determined by the entries of `V` as the images of
the generators of `R`. If `phi` satisfies the condition above, proceed as above.
...
of `RQ` to zero, return the homomorphism from `RQL` to `S` whose composition with the localization map `RQ -> RQL`
...
function is_local(R::MPolyLocRing{<:Field})
  return inverted_set(R) isa MPolyComplementOfPrimeIdeal || inverted_set(R) isa MPolyComplementOfKPointIdeal
end

is_known(::typeof(is_local), ::MPolyLocRing{<:Field}) = true
