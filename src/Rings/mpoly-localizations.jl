...
### type getters 
domain_type(::Type{MPolyLocalizedRingHom{D, C, M}}) where {D, C, M} = D
domain_type(f::MPolyLocalizedRingHom) = domain_type(typeof(f))
codomain_type(::Type{MPolyLocalizedRingHom{D, C, M}}) where {D, C, M} = C
codomain_type(f::MPolyLocalizedRingHom) = domain_type(typeof(f))
restricted_map_type(::Type{MPolyLocalizedRingHom{D, C, M}}) where {D, C, M} = M
restricted_map_type(f::MPolyLocalizedRingHom) = domain_type(typeof(f))

morphism_type(::Type{R}, ::Type{S}) where {R<:MPolyLocalizedRing, S<:Ring} = MPolyLocalizedRingHom{R, S, <:Map}
morphism_type(L::MPolyLocalizedRing, S::Ring) = morphism_type(typeof(L), typeof(S))

### additional getter functions
@attr images(PHI::MPolyLocalizedRingHom) = [PHI(x) for x in gens(base_ring(domain(PHI)))]
...
function is_isomorphism(
    phi::MPolyLocalizedRingHom{<:MPolyLocalizedRing{<:Any, <:Any, <:Any, <:Any, 
                                                    <:MPolyPowersOfElement},
  ...
  for i in 1:n
    ...
    #push!(pre_imagesA, evaluate(p, vcat([zero(A) for i in 0:n], gens(A))))
    push!(pre_images, evaluate(p, vcat([zero(K) for i in 0:n], [K(one(R), d1)], K.(gens(R)))))
  end
  ...
  set_attribute!(phi, :inverse, MPolyLocalizedRingHom(L, K, pre_images))
