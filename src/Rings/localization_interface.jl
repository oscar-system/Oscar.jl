...
"""
abstract type AbsLocalizedRingHom{RingType, RingElemType, DomainMultSetType, CodomainMultSetType} end
...
### required functionality
@Markdown.doc """
(f::AbsLocalizedRingHom{RT, RET, DMST, CMST})(a::AbsLocalizedRingElem{RT, RET, DMST}) where {RT, RET, DMST, CMST}

Applies the map `f` to the element `a`.
"""
...
### generic functions
@Markdown.doc """
(f::AbsLocalizedRingHom{RT, RET, DMST, CMST})(a::RET) where {RT, RET, DMST, CMST}

Applies the map `f` to `a//1`.
"""
