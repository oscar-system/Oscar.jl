...
Given an ideal `I` and optional parameters monomial ordering `ord` and `complete_reduction`,
compute a Groebner basis (if `complete_reduction = true` the reduced Groebner basis) of `I`
    w.r.t. the given monomial ordering `ord` (as default `:degrevlex`).
...
```
"""
function groebner_basis(I::MPolyIdeal; ord::Symbol = :degrevlex, complete_reduction::Bool = false)
...
function leading_ideal(I::MPolyIdeal, ord::Symbol)
  return leading_ideal(groebner_basis(I, ord=ord), ord)
end
