...
@doc Markdown.doc"""
    leading_ideal(g::Vector{Any}; ordering::MonomialOrdering)
...
w.r.t. the given monomial ordering.
"""
function leading_ideal(g::Vector{Any}; ordering::MonomialOrdering)
  return leading_ideal(typeof(g[1])[ f for f in g ], ordering=ordering)
end
...
#= @doc Markdown.doc"""
 ...
 =
 = """
 = function leading_ideal(Rx::MPolyRing, g::Vector{Any}; ordering::MonomialOrdering)
 =   h = elem_type(Rx)[ Rx(f) for f in g ]
 =   return leading_ideal(h, ordering=ordering)
 = end =#
