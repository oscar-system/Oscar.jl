...
w.r.t. the given monomial ordering `ord`.
...
julia> I = ideal(R,[-1+c+b+a^3,-1+b+c*a+2*a^3,5+c*b+c^2*a])
ideal generated by: a^3 + b + c - 1, 2*a^3 + a*c + b - 1, a*c^2 + b*c + 5

julia> J = ideal(R,[-1+c+b,-1+b+c*a+2*a*b])
ideal generated by: b + c - 1, 2*a*b + a*c + b - 1
...
julia> normal_form(I,J)
ideal generated by: a^3, 2*a^3 + 2*a - 2*c, 4*a - 2*c^2 - c + 5
```
"""
...
julia> Oscar.normal_form_internal(-1+c+b+a^3,J)
ideal generated by: a^3
```
"""
