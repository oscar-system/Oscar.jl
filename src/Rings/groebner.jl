export  groebner_basis, groebner_basis_with_transform, leading_ideal, syzygy_generators
...
Given an ideal `I` and optional parameters `ord` for a monomial ordering and `complete_reduction`
this function computes a Groebner basis (if `complete_reduction = true` the reduced Groebner basis) of the
ideal spanned by the elements in `B` w.r.t. the given monomial ordering `ord` and the
transformation matrix from the ideal to the Groebner basis.
...
julia> G,m = groebner_basis_with_transform(I)
(fmpq_mpoly[x*y^2 - 1, x^3 + x*y + y^2, x^2 + y^4 + y], [1 0; 0 1; -x^2 - y y^2])
```
"""
function groebner_basis_with_transform(I::MPolyIdeal; ord::Symbol = :degrevlex, complete_reduction::Bool=false)
  R = singular_ring(base_ring(I), ord)
  !Oscar.Singular.has_global_ordering(R) && error("The ordering has to be a global ordering.")
