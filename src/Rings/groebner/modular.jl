...
function groebner_basis_modular_broken(I::MPolyIdeal{QQMPolyRingElem}; ordering::MonomialOrdering = default_ordering(base_ring(I)),
                                certify::Bool = false)
    ...
    while n_stable_primes < 2
      ...
      for (i, f) in enumerate(std_basis_mod_p_lifted)
        if !iszero(f - std_basis_crt_previous[i])
          # induce_crt is problematic, leads to false assertion and/or too restricted prime selection
