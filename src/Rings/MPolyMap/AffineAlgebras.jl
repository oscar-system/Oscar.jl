...
# Mixed case: ungraded -> graded (HomUngradedToGraded).
function kernel(f::MPolyAnyMap{DT, CT, Nothing, V, HomUngradedToGraded}) where {
    DT <: Union{MPolyRing, MPolyQuoRing},
    CT <: Union{MPolyRing, MPolyQuoRing},
    V
  }
  return invoke(kernel, Tuple{AffAlgHom}, f)
end

# Mixed case: graded -> ungraded (HomGradedToUngraded).
# disallow kernel
function kernel(f::MPolyAnyMap{DT, CT, Nothing, V, HomGradedToUngraded}) where {
    DT <: Union{MPolyRing, MPolyQuoRing},
    CT <: Union{MPolyRing, MPolyQuoRing},
    V
  }
  error("Kernel for graded -> ungraded maps is not supported: the kernel would live in a graded ring and need not be homogeneous.")
end

# Non-graded maps whose domain is a graded polynomial ring: kernel is not supported.
@attr Any function kernel(
    f::MPolyAnyMap{<:MPolyDecRing, <:NCRing, Nothing, <:Any, HomUngraded}
)
    error("Kernel for non-graded maps with graded polynomial domain is not supported: the kernel need not be a graded ideal.")
end

@attr Any function kernel(
    f::MPolyAnyMap{<:MPolyQuoRing, <:NCRing, <:Any, <:Any, HomUngraded}
)
    if _isgraded(domain(f))
        error("Kernel for non-graded maps with graded quotient domain is not supported: the kernel need not be a graded ideal.")
    end
    return invoke(kernel, Tuple{Map}, f)
end
...
# Images
################################################################################
# Allow GradedToUngraded

function image(f::MPolyAnyMap{DT, CT, Nothing, V, HomGradedToUngraded}) where {
    DT <: Union{MPolyRing, MPolyQuoRing},
    CT <: Union{MPolyRing, MPolyQuoRing},
    V
  }
  return invoke(image, Tuple{AffAlgHom}, f)
end

# Disallow UngradedToGraded
function image(f::MPolyAnyMap{DT, CT, Nothing, V, HomUngradedToGraded}) where {
    DT <: Union{MPolyRing, MPolyQuoRing},
    CT <: Union{MPolyRing, MPolyQuoRing},
    V
  }
  error("Image for ungraded -> graded maps is not supported: the image would be in a graded ring and need not be generated by homogeneous.")
end

# Non-graded maps whose codomain is a graded polynomial ring: image is not supported.
@attr Any function image(
    f::MPolyAnyMap{<:NCRing, <:MPolyDecRing, Nothing, <:Any, HomUngraded}
)
    error("Image for non-graded maps with graded polynomial codomain is not supported: the generators of the image need not be homogeneous.")
end
