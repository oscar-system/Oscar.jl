...
export quo, base_ring, modulus, gens, ngens, dim
##############################################################################
#
# quotient rings
#
...
@doc Markdown.doc"""    
    ...
    ideal(Q::MPolyQuo{T}, V::Vector{T}) where T <: MPolyElem_dec

Given a quotient ring `Q=R/I` and a vector `V` of polynomials in `R`, 
create the ideal of `Q` which is generated by the images of the entries of `V`, 
assuring, in the decorated case, that the entries of `V` are homogeneous if the base ring is graded.

    ideal(Q::MPolyQuo{T}, V::Vector{MPolyQuoElem{T}}) where T <: MPolyElem

    ideal(Q::MPolyQuo{T}, V::Vector{MPolyQuoElem{T}}) where T <: MPolyElem_dec

Given a quotient ring `Q` and a vector `V` of elements of `Q`, 
...
function ideal(Q::MPolyQuo{T}, V::Vector{T}) where T <: MPolyElem_dec
  for p in V
    Q.R == parent(p) || error("parents must match")
  end
  if isgraded(parent(V[1]))
     if !(all(ishomogenous, V))
