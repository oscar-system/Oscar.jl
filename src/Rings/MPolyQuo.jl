...
julia> Q, _ = quo(R, ideal(R, [x^2-y^3, x-y]))
(Quotient of Multivariate Polynomial Ring in x, y over Rational Field by ideal(x^2 - y^3, x - y), Map from
Multivariate Polynomial Ring in x, y over Rational Field to Quotient of Multivariate Polynomial Ring in x, y over Rational Field by ideal(x^2 - y^3, x - y) defined by a julia-function with inverse
)
...
Given a (graded) quotient ring `Q=R/I` and a vector `V` of (homogeneous) polynomials in `R`, 
create the ideal of `Q` which is generated by the images of the entries of `V`.

    ideal(Q::MPolyQuo{T}, V::Vector{MPolyQuoElem{T}}) where T <: MPolyElem

Given a (graded) quotient ring `Q` and a vector `V` of (homogeneous) elements of `Q`, 
create the ideal of `Q` which is generated by the entries of `V`.
"""
...
julia> b = ideal(Q,[x^3*y^3-x+y, x^2*y+y^2*x])
...
julia> Q, q = quo(R, ideal(R, [x^2-y^3, x-y]))
(Quotient of Multivariate Polynomial Ring in x, y over Rational Field by ideal(x^2 - y^3, x - y), Map from
Multivariate Polynomial Ring in x, y over Rational Field to Quotient of Multivariate Polynomial Ring in x, y over Rational Field by ideal(x^2 - y^3, x - y) defined by a julia-function with inverse
)
...
julia> P, p = quo(S, ideal(S, [x^2*z-y^3, x-y]))
...
Given a vector `V` collecting homogeneous elements of a graded multivariate polynomial ring or quotient ring, say `R`, return a minimal subset of these elements which still generate the subalgebra of `R` generated by all elements collected in `V`.
...
julia> V = [x,y,x^2+y^2]
3-element Array{MPolyElem_dec{fmpq,fmpq_mpoly},1}:
 x
 y
 x^2 + y^2

julia> minimal_subalgebra_generators(V)
2-element Array{MPolyElem_dec{fmpq,fmpq_mpoly},1}:
 x
 y
```
"""
function minimal_subalgebra_generators(V::Vector{S}) where S <: Union{MPolyElem{U}, MPolyQuoElem{U}} where U
    ...
    if !((typeof(parent(V[1])) <: Oscar.MPolyRing_dec) && (isgraded(parent(V[1]))))
       throw(ArgumentError("The base ring must be graded."))
    end
    if !(all(ishomogeneous, V))
       throw(ArgumentError("The input data is not homogeneous."))
    end
    W = filter(x -> true, V)
    sort!(W, lt = (x,y) -> degree(x)[1] > degree(y)[1])
    result = similar(W, 0)
    X = filter(x -> true, W)
    for i = length(W):-1:1
	deleteat!(X, i)
        Y = filter(x -> (degree(x)[1] <= degree(W[i])[1]), X)
	if (length(Y) == 0) || (!subalgebra_membership(W[i], Y)[1])
	   result = push!(result, W[i])
	   insert!(X, i, W[i])	   
	end
    end
    return result
