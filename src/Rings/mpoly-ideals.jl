...
(Multivariate Polynomial Ring in x, y over Rational Field graded by
  x -> [1]
  y -> [2], MPolyDecRingElem{fmpq, fmpq_mpoly}[x, y])

julia> J = ideal(S, [(x^2+y)^2])
ideal(x^4 + 2*x^2*y + y^2)

julia> typeof(J)
MPolyIdeal{MPolyDecRingElem{fmpq, fmpq_mpoly}}
```
"""
...
########################################################
@doc Markdown.doc"""
    absolute_primary_decomposition(I::MPolyIdeal{<:MPolyRingElem{fmpq}})
...
```
"""
function absolute_primary_decomposition(I::MPolyIdeal{<:MPolyRingElem{fmpq}})
  R = base_ring(I)
  singular_assure(I)
