...
#######################################################
@doc raw"""
    radical(I::MPolyIdeal{T}; eliminate_variables::Bool = true, factor_generators::Bool = true) where {T <: MPolyRingElem}
...
When `factor_generators` is set to `true`, the squarefree part of all generators is computed before passing to the actual radical computation. 
...
@attr MPolyIdeal{T} function radical(
  ...
  if factor_generators
    return radical(_squarefree_generator_ideal(I); eliminate_variables, factor_generators=false)
  end
  if eliminate_variables
    ...
    J = modulus(S)
    pre_res = radical(J; eliminate_variables=false, factor_generators)
...
@attr MPolyIdeal{T} function radical(
  ...
  is_one(I) && return I
  R = base_ring(I)
  factor_generators && return radical(_squarefree_generator_ideal(I); eliminate_variables, factor_generators=false)
  R_flat, iso, iso_inv = _expand_coefficient_field_to_QQ(R)
  I_flat = ideal(R_flat, iso_inv.(gens(I)))
  I_flat_rad = radical(I_flat; 
                       eliminate_variables, 
                       factor_generators=false) # This only pays off on the top level. 
  Irad = iso(I_flat_rad)
  set_attribute!(Irad, :is_radical => true)
  return Irad
end
...
function is_known_to_be_radical(I::Ideal)
  get_attribute(I, :is_radical, false) && return true
