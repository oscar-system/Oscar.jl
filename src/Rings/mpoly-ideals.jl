...
################################################################################
#
# Grassman Pluecker Ideal
#
################################################################################
@doc Markdown.doc"""
    grassman_pluecker_ideal(subspace_dimension::Int, ambient_dimension::Int)

Given an ambient dimension and a subspace dimension return the ideal
generated by the plucker relations in the Polynomial Ring over the Rationals. 

# Examples
```jldoctest
julia> grassman_pluecker_ideal(2, 4)
...
@doc Markdown.doc"""
    grassman_pluecker_ideal(subspace_dimension::Int, ambient_dimension::Int, ring::Union{MPolyRing, Nothing} = nothing)

Given an ambient dimension and a subspace dimenson dimension return the ideal
generated by the plucker relations. If the ring is not specified return the ideal 
in the multi variate polynomial ring over the rationals.
...
julia> grassman_pluecker_ideal(R, 2, 4)
ideal(x[1, 2]*x[2, 2] + 6*x[2, 1]*x[1, 3] + x[1, 1]*x[2, 3])
```
"""
function grassman_pluecker_ideal(ring::MPolyRing,
