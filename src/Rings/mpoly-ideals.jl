...
### The alternative method from Singular below is typically slower, but used for saturation_with_index: Note that
### Singular.saturation2 does not proceed iteratively and the index returned might not be the correct saturation index.
#function saturation(I::MPolyIdeal{T}, J::MPolyIdeal{T} = ideal(base_ring(I), gens(base_ring(I)))) where T
