...
(Multivariate Polynomial Ring in x, y over Rational Field graded by 
  x -> [1]
  y -> [2], Oscar.MPolyElem_dec{fmpq,fmpq_mpoly}[x, y])
...
```
"""
function eliminate(I::MPolyIdeal, l::Array{T, 1}) where T <: Union{MPolyElem, MPolyElem_dec}
  singular_assure(I)
  B = BiPolyArray(l)
...
```
"""
function ideal_membership(f::T, I::MPolyIdeal) where T <: Union{MPolyElem, MPolyElem_dec}   
  groebner_assure(I)
  Sx = base_ring(I.gb.S)
...
```
"""
function radical_membership(f::T, I::MPolyIdeal) where T <: Union{MPolyElem, MPolyElem_dec}
  singular_assure(I)                                                                                    
