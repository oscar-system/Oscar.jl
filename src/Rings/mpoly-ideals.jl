...
julia> is_monomial(ideal(R, [f2*x+y, y]))
true
```
"""
...
@doc raw"""
    is_principal(I::MPolyIdeal{T}; ordering::MonomialOrdering=default_ordering(base_ring(I))) where {T<:MPolyRingElem{<:FieldElem}}

Checks whether `I` can be generated by a single element. 
"""
@attr Bool function is_principal(I::MPolyIdeal{T}; 
    ordering::MonomialOrdering=default_ordering(base_ring(I))
  ) where {T<:MPolyRingElem{<:FieldElem}}
  if is_zero(I)
    set_attribute!(I, :principal_generator=>zero(base_ring(I)))
    return true # generated by the single element `0`
  end
  # the ideal `I` is principal if and only if a reduced Groebner basis 
  # for the ideal consists of a single element.
  if is_one(ngens(I)) 
    set_attribute!(I, :principal_generator=>only(gens(I)))
    return true
  end
  gb = groebner_basis(I; ordering, complete_reduction=true)
  !is_one(length(oscar_generators(gb))) && return false
  set_attribute!(I, :principal_generator=>only(oscar_generators(gb)))
  return true
end
