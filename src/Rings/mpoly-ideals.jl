...
Returns generators of the defining ideal for the flag variety $\text{Fl}((d_1,\dots,d_k),n)$, where $(d_1,\dots,d_k) =$`dimensions` denotes the rank, with $d_j\leq n-1$. That is, the space of $k$-step flags of linear subspaces $V_1\subset\dots\subset V_k$ in $\mathbb{C}^n$, where $\text{dim}(V_j) = d_{j}$.You can obtain the generators for the \emph{complete flag variety}$ by taking `dimensions` $=(1,\dots,n-1)$ and `n`$=n$. 

# Example
Complete flag variety $\text{Fl}((1,2,3),4)$. 
```jldoctest
julia> flag_pluecker_ideal([1,2,3],4)
Ideal generated by
...
Flag variety $\text{Fl}((1,3),4)$.
```jldoctest
julia> flag_pluecker_ideal([1,3],4)
Ideal generated by
  x[[1]]*x[[2, 3, 4]] - x[[2]]*x[[1, 3, 4]] + x[[3]]*x[[1, 2, 4]] - x[[4]]*x[[1, 2, 3]]
```
"""
function flag_pluecker_ideal(dimensions::Vector{Int},n::Int)
    ...
    gg = unique(x->x*sign(leading_coefficient(x)),[g for g in genz if !(g == 0)])
