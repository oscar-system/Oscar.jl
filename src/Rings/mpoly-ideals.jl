...
julia> quotient(I,J,ordering=lex(gens(R)))
ideal(y^5, x*y^4 + 2*y^5, x^2*y*z^2 - 2*x*y^4 + y^4*z + y^3*z^2, x^2*y^2*z - x^2*y*z^2 + x*y^4 - y^3*z^2, x^2*y^3 - x^2*y^2*z - y^5 - y^4*z, x^3*z + x*y^2*z + y^4, x^3*y + x*y^3, x^4 + x^3*z + x^2*y*z + x*y^2*z + y^4 + y^3*z)
```
"""
function quotient(I::MPolyIdeal{T}, J::MPolyIdeal{T}; ordering::MonomialOrdering=default_ordering(base_ring(I))) where T
...
function quotient(I::MPolyIdeal{T}, f::T; ordering::MonomialOrdering=default_ordering(base_ring(I))) where T
  R = base_ring(I)
  @assert R == parent(f)
  return quotient(I, ideal(R, [f]), ordering=ordering)
end
