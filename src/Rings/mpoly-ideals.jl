...
_pluecker_sgn(a::Vector{Int}, b::Vector{Int}, t::Int)::Int =
  (-1)^(count(z -> z > t, a) + count(z -> z < t, b))

@doc raw"""
    flag_pluecker_ideal(dimensions::Vector{Int},n::Int,F)

Returns generators of the defining ideal for the flag variety
$\text{Fl}((d_1,\dots,d_k),n,\mathbb{F})$, where $(d_1,\dots,d_k)
...
# Example
Complete flag variety $\text{Fl}((1,2,3),4,\mathbb{Q})$. 
```jldoctest
julia> flag_pluecker_ideal([1,2,3],4,QQ)
Ideal generated by
...
```
"""
function flag_pluecker_ideal(dimensions::Vector{Int}, n::Int, F::Field)
  ...
  filter!(!iszero, genz)
  gg = unique(x -> x * sign(leading_coefficient(x)), genz)
