...
Returns generators of the defining ideal for the flag variety
$\text{Fl}(\mathbb{F}, (d_1,\dots,d_k), n)$, where $(d_1,\dots,d_k)
=$`dimensions` denotes the rank, with $d_j\leq n-1$.  That is, the vanishing
...
the flag Plücker ideal with respect to the reverse lexographical order. The first parameter can
either be $\mathbb{F}$ or a polynomial ring R over $\mathbb{F}$. If the input is $\mathbb{F}$, the
function returns the ideal in a polynomial ring indexed by elements of $2^{[n]}$. 
Otherwise, the function returns the ideal in R.
...
(Multivariate polynomial ring in 8 variables over QQ, QQMPolyRingElem[x1, x2, x3, x4, x5
, x6, x7, x8])
...
function flag_pluecker_ideal(ring::MPolyRing, dimensions::Vector{Int}, n::Int; minimal::Bool=true)
  base_field = base_ring(ring)  
  I = flag_pluecker_ideal(base_field, dimensions, n; minimal = false) 
  
  coeffmap = c -> begin
    @assert is_integral(c)
    return base_field(numerator(c))
  end
  h = hom(base_ring(I), ring, coeffmap, gens(ring))

  !minimal && ideal(h.(gens(I)))
