...
generated by the plucker relations. If the ring is not specified return the ideal in the multi variate polynomial ring over the rationals.
...
julia> grassman_pluecker_ideal(2, 4; ring = R)
ideal(x[1, 2]*x[2, 2] + 6*x[2, 1]*x[1, 3] + x[1, 1]*x[2, 3])
```
"""
function grassman_pluecker_ideal(subspace_dimension::Int,
                                 ambient_dimension::Int;
                                 ring::R = nothing) where R <: Union{MPolyRing, Nothing}
    pluecker_ideal = convert(Polymake.ideal.pluecker_ideal(subspace_dimension, ambient_dimension))
    if R != nothing
        ...
        pluecker_ideal = ideal(ring, converted_generators)
    end
    return pluecker_ideal
end
