...
function Base.:*(I::MPolyIdeal{T}, J::MPolyIdeal{T}) where T
  ...
  # carry over any available information about the dimension
  if is_known(krull_dim, I) && is_known(krull_dim, J)
    result.dim = max(krull_dim(I), krull_dim(J))
  end
  return result
end
...
@attr MPolyIdeal{T} function radical(
  ...
  if preprocessing && eliminate_variables
    ...
    if is_known(krull_dim, I)
      res.dim = krull_dim(I) # preserve information about the dimension
    end
    return res
  end
...
function triangular_decomposition(I::MPolyIdeal; algorithm::Symbol=:lazard, ord::Vector{<:MPolyRingElem}=reverse(gens(base_ring(I))))
  @req krull_dim(I)==0 "The ideal must be zero-dimensional."
  R = base_ring(I)
...
function krull_dim(I::MPolyIdeal)
  if I.dim === nothing
    ...
    if is_zero(ngens(base_ring(I))) # Catch a boundary case
      I.dim = krull_dim(coefficient_ring(base_ring(I)))
    else
...
function is_known(::typeof(krull_dim), I::MPolyIdeal)
...
is_known(::typeof(codim), I::MPolyIdeal) = is_known(krull_dim, I)
...
@doc raw"""
    function is_principal(I::MPolyIdeal{T}) where {T<:MPolyRingElem{<:FieldElem}}

Checks whether `I` is generated by a single element. 
If not, a heuristic method is applied to see whether the generating 
set of `I` can easily be replaced by a single element. 

!!! note
    Beyond some heuristic tests there is no algorithm implemented to rigorously check whether the given ideal is principal. Hence the user must expect this method to throw an error.
"""
@attr Bool function is_principal(I::MPolyIdeal{T}) where {T<:MPolyRingElem{<:FieldElem}}
  is_zero(I) && return true # generated by the single element `0`
  is_one(ngens(I)) && return true
  g = small_generating_set(I)
  if !is_one(length(g))
    # We have more then one generator. 
    # According to Krull's principal ideal theorem the codimension of `I` must be one.
    R = base_ring(I)
    dim(I) == dim(R) - 1 || return false
    # The ideal must be equidimensional.
    all(dim(P) == dim(R) - 1 for P in minimal_primes(I)) || return false
    # At this point one has to decide whether there might be another 
    # single generator. 
    gg = reduce(gcd, g)
    if !is_unit(gg) && gg in I
      set_attribute!(I, :principal_generator=>gg)
      return true
    end
    error("no algorithm implemented to check whether the ideal $I is principal")
  end
