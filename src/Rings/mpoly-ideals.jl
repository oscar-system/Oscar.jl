...
function grassmann_pluecker_ideal(subspace_dimension::Int, ambient_dimension::Int)
  ...
  base = base_ring(I)
  base2, x = polynomial_ring(coefficient_ring(base), :x=> sort(subsets(ambient_dimension, subspace_dimension)))
  h = hom(base, base2, x)
  o = degrevlex(base2)
...
Given a ring, an ambient dimension $n$ and a subspace dimension $d$, return the ideal in the given ring
generated by the Plücker relations. If the ring is not specified return the ideal 
in a multivariate polynomial ring over the rationals with variables indexed by elements of ${[n]\choose d}$.
