...
export isbinomial, iscellular, isunital, binomial_primary_decomposition, 
       cellular_decomposition, cellular_associated_primes, cellular_minimal_associated_primes
@doc Markdown.doc"""
    isbinomial(f::MPolyElem)

Return true if `f` is a binomial, that is, `f` consists of at most 2 terms,
false otherwise.
"""
...
Return true if `I` is a binomial ideal, that is, `I` can be generated by 
polynomials consisting of at most 2 terms, false otherwise.
"""
...
Given a cellular binomial ideal `I`, return `hull(I)`, that is, the intersection 
