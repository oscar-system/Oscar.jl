...
mutable struct SAGBICandidate
    ring::QQMPolyRing
    elements::Vector{QQMPolyRingElem}
    leading_monomials::Dict{QQMPolyRingElem, Vector{QQMPolyRingElem}}
...
function _initialize_sagbi_candidate(
    ...
    for b in B
        ...
        push!(get!(leading_monomials, lm, QQMPolyRingElem[]), b)
    end
...
"""
Determine whether the monomial `m` belongs to the multiplicative
monoid generated by `generators`.

Return `nothing` if no such representation exists.
"""
function monoid_representation(
    m::QQMPolyRingElem,
    generators::Vector{QQMPolyRingElem},
)
...
monomials of `B`.
"""
function _exponent_matrix(B::Vector{QQMPolyRingElem})
...
The toric ideal `ker(QQ[y₁,…,yₙ] → QQ[x₁,…,xₘ])` of the monomial map
`yᵢ ↦ leading_monomial(bᵢ)`.
"""
function toric_ideal_of_leading_monomials(B::Vector{QQMPolyRingElem})
...
"""
function tete_a_tetes(B::Vector{QQMPolyRingElem})
    isempty(B) && return NamedTuple{(:polynomial, :a, :b), Tuple{QQMPolyRingElem, Vector{Int}, Vector{Int}}}[]
    I = toric_ideal_of_leading_monomials(B)
    R = parent(B[1])
    ts = NamedTuple{(:polynomial, :a, :b), Tuple{QQMPolyRingElem, Vector{Int}, Vector{Int}}}[]
...
@doc raw"""
    subduct(f::QQMPolyRingElem, B::SAGBICandidate) -> QQMPolyRingElem
...
@doc raw"""
    subduct(f::QQMPolyRingElem, B::Vector{QQMPolyRingElem}) -> QQMPolyRingElem
    ...
    '''
"""
subduct(f::QQMPolyRingElem, B::Vector{QQMPolyRingElem}) =
...
is_sagbi(B::Vector{QQMPolyRingElem}) =
    isempty(B) ? true : is_sagbi(_initialize_sagbi_candidate(B))

function _monic_if_nonzero(f::QQMPolyRingElem)
    if !iszero(f)
...
function _compute_all_subductions(F::Vector{QQMPolyRingElem})
    R = parent(F[1])
...
function sagbi(
        generating_set::Vector{QQMPolyRingElem};
    ...
    for f in F
        ...
        push!(get!(leading_monomials, lm, QQMPolyRingElem[]), f)
    end
