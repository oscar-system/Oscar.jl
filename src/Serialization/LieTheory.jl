...
type_params(e::T) where T <: Union{RootSpaceElem, DualRootSpaceElem} = TypeParams(root_system(e))
...
type_params(P::WeightLattice) = TypeParams(root_system(P))
...
type_params(w::WeightLatticeElem) = TypeParams(parent(w))
...
type_params(W::WeylGroup) = TypeParams(root_system(W))
...
type_params(w::WeylGroupElem) = TypeParams(parent(w))
