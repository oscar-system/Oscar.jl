...
push!(upgrade_scripts_set, UpgradeScript(
  v"0.12.2",
  function upgrade_0_12_2(s::UpgradeState, dict::AbstractDict{String, Any})
    s.nested_level += 1
    ...
    elseif contains(upgraded_dict["type"], "PolyRingElem")
      ...
      if haskey(parent_dict, "id")
        if parent_dict["type"] == "#backref"
          parent_dict = s.id_to_dict[parent_dict["id"]]
        else
          s.id_to_dict[parent_dict["id"]] = parent_dict
        end
      ...
      for parent in parents
        ...
        pushfirst!(upgraded_parents, Dict{String, Any}("id" => id, "type" => "#backref"))
        if haskey(local_refs, id)
          continue
        end
        ...
        elseif haskey(parent["data"], "base_ring")
          ...
          if haskey(base_dict, "id")
            parent["data"]["base_ring"] = Dict{String, Any}("id" => base_dict["id"], "type" => "#backref")
          end
        end
        local_refs[id] = parent
      end
      ...
      if upgraded_dict["type"] == "MPolyRingElem"
        ...
        for term in dict["data"]["terms"]
          if term["coeff"]["type"] == "#backref"
            term["coeff"] = s.id_to_dict[term["coeff"]["id"]]
          else
            s.id_to_dict[term["coeff"]["id"]] = term["coeff"]
          end
      ...
      upgraded_dict["data"] = Dict{String, Any}("terms" => terms, 
