...
###############################################################################
#
# Unimodular primitive extensions
#
###############################################################################
...
- `parity::Symbol` -> determine the parity of the unimodular primitive
  ...
  for even ones or `:both` for both. By default, the algorithm chooses the
  value of `:parity` to be `:even` or `:odd` depending on the parity of the
  inputs.
...
"""
unimodular_primitive_extensions(::ZZLat, ::ZZLat)
...
function unimodular_primitive_extensions(
  ...
  exist_only::Bool=false,
  first::Bool=false,
  parity::String=:default,
)
  @req is_integral(M) && all(is_integral, Ns) "Only available for integral lattices"
  if !isnothing(right_discriminant_action)
    continue
  elseif !isnothing(right_action)
    right_discriminant_action, _ = image(discriminant_representation(M, right_action; full=false))
  elseif first || exist_only
    qM = discriminant_group(M)
    right_discriminant_action = Oscar._orthogonal_group(qM, TorQuadModuleMap[id_hom(qM)]; check=false)
  else
    right_discriminant_action, _ = image_in_Oq(M)
  end

  results = NTuple{3, ZZLat}[]
  for N in Ns
    ok, tmp = unimodular_primitive_extensions(M, N; right_discriminant_action, exist_only, first, parity)
...
- `glue_annihilator_left::TorQuadModuleMap` -> specify an abelian group
  endomorphism $a_M$ of the discriminant group $q_M$ of $M$ such that the
  glue group $H_M\subset q_M$ associated to each primitive extension in output
...
- `glue_annihilator_right::TorQuadModuleMap` -> specify an abelian group
  endomorphism $a_N$ of the discriminant group $q_N$ of $N$ such that the
  glue group $H_N\subset q_N$ associated to each primitive extension in output
...
- `glue_exponent::IntegerUnion` -> specify a positive integer $n$ such that
  ...
  restriction.
- `glue_elementary_divisors::Vector{Vector{ZZRingElem}}` -> provide a list of
  possible elementary divisors for the glue groups associated to each primitive
...
function primitive_extensions(
  ...
  both_even = is_even(M) && is_even(N)
  if even
    !both_even && false, return NTuple{3, ZZLat}[]
    parity = :even
  elseif parity === :default
    parity = both_even ? :even : :odd
  ...
  elseif !isnothing(right_action)
    GMbar, _ = image(discriminant_representation(M, right_action; full=false))
  elseif first || exist_only || classification == :embsub || classification == :embemb
    qM = discriminant_group(M)
    GMbar = Oscar._orthogonal_group(qM, TorQuadModuleMap[id_hom(qM)]; check=false)
  else
    GMbar, _ = image_in_Oq(M)
  ...
  elseif !isnothing(left_action)
    GNbar, _ = image(discriminant_representation(N, left_action; full=false))
  elseif first || exist_only || classification == :subemb || classification == :embemb
    qN = discriminant_group(N)
    GNbar = Oscar._orthogonal_group(qN, TorQuadModuleMap[id_hom(qN)]; check=false)
  else
    GNbar, _ = image_in_Oq(N)
  ...
  return _primitive_extensions(M, N, parity, GMbar, GNbar; first, exist_only, kwargs...)
end
...
function _primitive_extensions(
  ...
  parity === :even && (!is_even(M) || !is_even(N)) && return false, results
  Fac, lgm = _local_glue_maps(qM, qN; parityi, kwargs...)
  isempty(lgm) && return results
  parity = Fac.par
  D = _gluing_ambient(qM, qN, parity)

  for x in lgm
    if first || exist_only
      z = _overlattice(x, D, parity)
      if test_overlattice(Fac, first(z))
        exist_only && return true, results
        first && return true, NTuple{3, ZZLat}[z]
      end
    end
    xMs = _split_orbit_left_group(x, GM, parity)
    for xM in xMs
      if first || exist_only
        z = _overlattice(xM, D, parity)
        if test_overlattice(Fac, first(z))
          exist_only && return true, results
          first && return true, NTuple{3, ZZLat}[z]
        end
      end
      xMNs = _split_orbit_right_group(xM, GN, parity)
      for xMN in xMNs
        if first || exist_only
          z = _overlattice(xMN, D, parity)
          if test_overlattice(Fac, first(z))
            exist_only && return true, results
            first && return true, NTuple{3, ZZLat}[z]
          end
        end
        for y in _all_glue_maps(xMN)
          z = _overlattice(y, D, parity)
          !test_overlattice(Fac, first(z)) && continue
          exist_only && return true, results
          push!(results, z)
          first && return true, results
        end
...
$\mathbb Z$-lattice $M$, return a boolean `T` and a list $V$ of representatives
of double cosets of primitive embeddings $M \hookrightarrow L$ where $L$ is a
lattice in $G$, for the right and left actions of $O(M)$ and $O(L)$
respectively.

The second output $V$ consists of triples $(L, M_1, N)$ where $L$ is a lattice
in $G$, $M_1$ is a primitive sublattice of $L$ isometric to $M$, and $N$ is
the orthogonal complement of $M_1$ in $L$.
...
- the discriminant form `q` and the signature pair `(p, n)` of $G$: one
  would therefore call
  `primitive_embeddings(q, (p, n), M; kwargs...)`.
...
function primitive_embeddings(
  ...
  if is_unimodular(G)
    ...
    for K in orths
      ok, pe = primitive_extensions(M, K; right_discriminant_action=GMbar, form_over=TorQuadModule[q], even, first, exist_only)
      !ok && continue
      exist_only && return true, results
      if !isempty(pe)
        append!(results, pe)
  ...
  R = rescale(representative(G), -1; cached=false)
  ...
  # unique in its genus)
  _, Vs = primitive_extensions(M, T; even, right_discriminant_action=GMbar)
  ...
  for (V, M2, T2) in Vs
    ...
    for K in orths
      GKbar, _ = image_in_Oq(K)
      ok, pe = _primitive_extensions_generic(V, K, GVbar, GKbar, (:plain, :plain); even, exist_only, first, form_over=[discriminant_group(GL)])
      !ok && continue
...
function _glue_stabilizers(
  ...
  if isnothing(left_discriminant_action)
    if isnothing(left_action)
      GNbar = subN ? image_centralizer_in_Oq(N)[1] : Oscar._orthogonal_group(qN, TorQuadModuleMap[id_hom(qN)]; check=false)
    else
      GNbar, _ = image(discriminant_representation(N, left_action; full=false))
    end
  else
