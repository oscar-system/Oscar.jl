...
_ray_cone(::Type{T}, C::Cone, i::Base.Integer) where T = T(view(pm_object(C).RAYS, i, :))
...
function faces(C::Cone{T}, face_dim::Int) where T<:scalar_types
   face_dim == dim(C) - 1 && return SubObjectIterator{Cone{T}}(C, _face_cone_facet, nfacets(C))
...
function _face_cone_facet(::Type{Cone{T}}, C::Cone, i::Base.Integer) where T<:scalar_types
   return Cone{T}(Polymake.polytope.Cone{_scalar_type_to_polymake(T)}(RAYS = pm_object(C).RAYS[collect(pm_object(C).RAYS_IN_FACETS[i, :]), :], LINEALITY_SPACE = pm_object(C).LINEALITY_SPACE))
end
...
_facet_cone(::Type{Polyhedron{T}}, C::Cone, i::Base.Integer) where T<:scalar_types = polyhedron(T, -view(pm_object(C).FACETS, [i], :), 0)

_facet_cone(::Type{AffineHalfspace{T}}, C::Cone, i::Base.Integer) where T<:scalar_types = AffineHalfspace{T}(-view(pm_object(C).FACETS, [i], :), 0)

_facet_cone(::Type{LinearHalfspace{T}}, C::Cone, i::Base.Integer) where T<:scalar_types = LinearHalfspace{T}(-pm_object(C).FACETS[[i], :])

_facet_cone(::Type{Cone{T}}, C::Cone, i::Base.Integer) where T<:scalar_types = cone_from_inequalities(T, -view(pm_object(C).FACETS, [i], :))
...
_lineality_cone(::Type{RayVector{T}}, C::Cone, i::Base.Integer) where T<:scalar_types = RayVector{T}(view(pm_object(C).LINEALITY_SPACE, i, :))
...
_linear_span(::Type{LinearHyperplane{T}}, C::Cone, i::Base.Integer) where T<:scalar_types = LinearHyperplane{T}(view(pm_object(C).LINEAR_SPAN, i, :))
