...
_vertex_complex(::Type{PointVector{T}}, PC::PolyhedralComplex, i::Base.Integer) where T<:scalar_types = PointVector{T}(@view pm_object(PC).VERTICES[_vertex_indices(pm_object(PC))[i], 2:end])
...
function _vertex_or_ray_polyhedron(::Type{Union{PointVector{T}, RayVector{T}}}, PC::PolyhedralComplex, i::Base.Integer) where T<:scalar_types
    A = pm_object(PC).VERTICES
    if iszero(A[_all_vertex_indices(pm_object(PC))[i],1])
        return RayVector{T}(@view pm_object(PC).VERTICES[_all_vertex_indices(pm_object(PC))[i], 2:end])
    else
        return PointVector{T}(@view pm_object(PC).VERTICES[_all_vertex_indices(pm_object(PC))[i], 2:end])
    end
end

_vertices(as::Type{Union{RayVector{T}, PointVector{T}}}, PC::PolyhedralComplex) where T<:scalar_types = SubObjectIterator{as}(PC, _vertex_or_ray_polyhedron, length(_all_vertex_indices(pm_object(PC))))

vertices_and_rays(PC::PolyhedralComplex{T}) where T<:scalar_types = _vertices(Union{PointVector{T}, RayVector{T}}, PC)

_vector_matrix(::Val{_vertex_or_ray_polyhedron}, PC::PolyhedralComplex; homogenized = false) = homogenized ? pm_object(PC).VERTICES : @view pm_object(PC).VERTICES[:, 2:end]
...
_ray_polyhedral_complex(::Type{RayVector{T}}, PC::PolyhedralComplex, i::Base.Integer) where T<:scalar_types = RayVector{T}(@view pm_object(PC).VERTICES[_ray_indices_polyhedral_complex(pm_object(PC))[i], 2:end])
...
_maximal_polyhedron(::Type{Polyhedron{T}}, PC::PolyhedralComplex, i::Base.Integer) where T<:scalar_types = Polyhedron{T}(Polymake.fan.polytope(pm_object(PC), i-1))
...
function _ith_polyhedron(::Type{Polyhedron{T}}, PC::PolyhedralComplex, i::Base.Integer; f_dim::Int = -1, f_ind::Vector{Int64} = Vector{Int64}()) where T<:scalar_types
    ...
    return Polyhedron{T}(Polymake.polytope.Polytope{_scalar_type_to_polymake(T)}(VERTICES = pm_object(PC).VERTICES[collect(pface),:], LINEALITY_SPACE = pm_object(PC).LINEALITY_SPACE))
end
...
_lineality_complex(::Type{RayVector{T}}, PC::PolyhedralComplex, i::Base.Integer) where T<:scalar_types = RayVector{T}(@view pm_object(PC).LINEALITY_SPACE[i, 2:end])
