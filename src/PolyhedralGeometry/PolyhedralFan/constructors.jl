...
# Arguments
- `Rays::RayCollection`: Rays generating the cones of the fan; encoded row-wise as representative vectors.
...
```
"""
function PolyhedralFan{T}(Rays::RayCollection, Incidence::IncidenceMatrix; non_redundant::Bool = false) where T<:scalar_types
   if non_redundant
...
function PolyhedralFan{T}(Rays::RayCollection, LS::RayCollection, Incidence::IncidenceMatrix; non_redundant::Bool = false) where T<:scalar_types
   if non_redundant
      return PolyhedralFan{T}(Polymake.fan.PolyhedralFan{scalar_type_to_polymake[T]}(
         RAYS = Rays,
         LINEALITY_SPACE = LS,
...
function PolyhedralFan{T}(Rays::RayCollection, LS::RayCollection, Incidence::Matrix{Bool}) where T<:scalar_types
   PolyhedralFan(Rays, LS, IncidenceMatrix(Polymake.IncidenceMatrix(Incidence)))
end
function PolyhedralFan{T}(Rays::RayCollection, Incidence::Matrix{Bool}) where T<:scalar_types
