...
Given a vector `r` which is not a zero vector, returns the primitive generator `s` of `r` together with the rational number `q` such that `qr = s`, meaning the integer point on the ray generated by `r` that is closest to the origin.
