...
```
"""
rays(PF::_FanLikeType) = lineality_dim(PF) == 0 ? _rays(PF) : _empty_subobjectiterator(RayVector{get_scalar_type(PF)}, pm_object(PF))
_rays(PF::_FanLikeType) = SubObjectIterator{RayVector{get_scalar_type(PF)}}(pm_object(PF), _ray_fan, _nrays(PF))
