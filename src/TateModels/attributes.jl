...
we should find that the discriminant vanishes to order 2 on ``W = {w = 0}``. Let us verify this.
...
```
"""
@attr Vector{Tuple{MPolyIdeal{MPolyElem_dec{fmpq, fmpq_mpoly}}, MPolyIdeal{MPolyElem_dec{fmpq, fmpq_mpoly}}}} function singular_loci(t::GlobalTateModel)
    B = irrelevant_ideal(toric_base_space(t))
    primes = primary_decomposition(ideal([discriminant(t)]))
