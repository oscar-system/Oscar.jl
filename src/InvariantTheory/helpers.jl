...
function add_base_element!(
  C::PowerProductCache{S,T}, f::T, remember_exponents::Bool=true
) where {S,T}
  @assert !iszero(f)
...
function extend!(
  C::PowerProductCache{S,T}, d::Int, i::Int, remember_exponents::Bool=true
) where {S,T<:MPolyRingElem}
  @assert d >= 0
  ...
  for j in 1:length(C.power_products[dd])
...
function all_power_products_of_degree!(
  C::PowerProductCache{S,T}, d::Int, remember_exponents::Bool=true
) where {S,T<:MPolyRingElem}
  @assert d >= 0
  ...
  if d == 0
    ...
    if remember_exponents
      C.exponent_vectors[one(C.ring)] = zeros(Int, length(C.base))
    else
      nothing
    end
...
function generators_for_given_degree!(
  C::PowerProductCache{S,T}, module_gens::Vector{T}, d::Int, remember_exponents::Bool=true
) where {S,T<:MPolyRingElem}
...
function enumerate_monomials(
  polys::Vector{PolyRingElemT}
) where {PolyRingElemT<:MPolyRingElem}
...
function polys_to_smat(
  polys::Vector{PolyRingElemT}, monomial_to_column::Dict{Vector{Int},Int}; copy=true
) where {PolyRingElemT<:MPolyRingElem}
