...
"""
    ...
    matrix_group(R::Ring, V::AbstractVector{T}; check::Bool=true) where T<:Union{MatElem,MatGroupElem}

Return the matrix group generated by matrices in `V`.
If `V` is empty then the base ring `R` and the degree `m` must be given.

When a ring `R` is given without `m`, the degree is inferred from `V`
(which must be non-empty) and the matrix entries are coerced into `R`.
...
julia> g = matrix_group(QQ, [identity_matrix(ZZ, 3)])
...
function matrix_group(F::Ring, V::AbstractVector{<:Union{MatElem,MatGroupElem}}; check::Bool=true)
   @req !isempty(V) "Cannot determine the degree from an empty list of generators; use matrix_group(R, m, V) instead"
   if all(v -> base_ring(v) === F, V)
      return matrix_group(F, nrows(V[1]), V; check)
   end
   W = [map_entries(F, matrix(v)) for v in V]
   return matrix_group(F, nrows(W[1]), W; check)
end
