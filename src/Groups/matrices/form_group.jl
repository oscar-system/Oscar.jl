...
function _isometry_group_via_decomposition(L::ZLat; closed = true, direct=true)
  ...
  G1 = _with_small_generating_set(G1)
  G2 = _with_small_generating_set(G2)
  ...
  G = _with_small_generating_set(G)
...
function _with_small_generating_set(G::MatrixGroup)
  g = GAP.Globals.SmallGeneratingSet(G.X)
  S = GAP.Globals.SubgroupNC(G.X, g)
  return _as_subgroup_bare(G, S)
end
