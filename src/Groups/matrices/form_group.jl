...
"""
function _isometry_group_via_decomposition(L::ZLat)
  ...
  sv1 = fmpq_mat[v*basis_matrix(L) for v in shortest_vectors(L)]
  # compute the ZZ-span of sv ... probably this can be done much more (memory) efficient
  # using some kind of basis exchange lemma
  h =  hnf(reduce(vcat, sv1))  # !hnf?
  ...
  # recursion
  O2, sv2 = _isometry_group_via_decomposition(M2)

  # short vectors to be used for a permutation representation

  #=
  if order(H1)==1
    # no glue
    # somehow avoids an infinite recursion in gap ... or not?
    sv = append!(sv1, sv2)
    gensO1 = matrix.(gens(O1))
    gensO = append!(gensO1, matrix.(gens(O2)))
    G =  matrix_group(gensO)
    set_nice_monomorphism(G, sv)
    return G, sv
  end
  =#

  # the stabilizers lead to many generators
  # first project to the discriminant_group and then lift?
  # or use SmallGeneratingSet set? ... but this may die for very big groups.
  @vprint :Lattice 3 "Computing stabilizers \n"
  G1, _ = stabilizer(O1, cover(H1), on_lattices)
  G2, _ = stabilizer(O2, cover(H2), on_lattices)
  # We might want to call GAP.SmallGeneratingSet?
  set_nice_monomorphism(G1, sv1)
  set_nice_monomorphism(G2, sv2)
  ...
  @vprint :Lattice 2 "Computing the kernel part 2\n"
...
function set_nice_monomorphism(G::MatrixGroup, short_vectors)
  phi = action_homomorphism(gset(G, on_fmpz_mat, short_vectors))
