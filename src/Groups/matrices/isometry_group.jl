...
function _howell_is_fast(S::ZZLat, L::ZZLat)
  ...
  # keep only the relevant elementary_divisors
  ed = [gcd(i,n) for i in ed]
  filter!(!isone, ed)
...
function Hecke._assert_has_automorphisms_ZZLat(L::ZZLat;
  ...
  @vprintln :Isometry 3 "Computing orthogonal group algorithm=$(algorithm) of $(gram_matrix(L))"  
...
function _isometry_group_via_decomposition(
  ...
  if _direct_is_faster(M1primitive)
    ...
    if M1primitive == M1
      ...
      @vprint :Isometry 3 "Computing overlattice stabilizers index=$(index(M1primitive,M1))\n"
  ...
  if _howell && _howell_is_fast(M1primitive+M2, L)
    # Cook up O1 x O2
    @vprint :Isometry 3 "stabilizer of primitive extension in diagonal \n"
    r1 = rank(M1primitive)
    r2 = rank(M2) 
   ...
   @vprint :Isometry 3 "stabilizer of primitive extension via gluing\n"
    ...
    @vtime :Isometry 3 gensS = stabilizer_in_diagonal_action(L, M1primitive, M2, O1amb, O2amb; check=false, is_finite_known=(true, true))
...
function _overlattice_stabilizer(G::MatGroup{ZZRingElem,ZZMatrix}, S::ZZLat, L::ZZLat, howell::Bool=false)
  ...
  BL = numerator(_BL)
  if is_prime(n)
    @vprintln :Isometry 5 "stabilizer via finite field with n=$n and index $(index(L,S))"
