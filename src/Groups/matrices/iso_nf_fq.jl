...
function _isomorphic_group_over_finite_field(matrices::Vector{<:MatElem{T}}; check::Bool = true, min_char::Int = 3) where T <: Union{ZZRingElem, QQFieldElem, AbsSimpleNumFieldElem}
...
function good_reduction(matrices::Vector{<:MatElem{T}}, p::Int = 2) where T <: Union{ZZRingElem, QQFieldElem, AbsSimpleNumFieldElem}
   while true
      p = next_prime(p)
...
function _reduce(M::MatElem{AbsSimpleNumFieldElem}, OtoFq)
  e = extend(OtoFq, Hecke.nf(domain(OtoFq)))
  return map_entries(e, M)
end

function _reduce(M::MatElem{QQFieldElem}, Fp)
...
function test_modulus(matrices::Vector{<:MatElem{T}}, p::Int) where T <: Union{ZZRingElem, QQFieldElem}
   Fp = GF(p)
...
# an isomorphism or, if it is not injective, then the group generated by
# matrices cannot be finite.
function test_modulus(matrices::Vector{T}, p::Int) where T <: MatElem{AbsSimpleNumFieldElem}
