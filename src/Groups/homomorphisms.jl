...
#T change this: use IsomorphismPcpGroup if G is infinite!
       f = fun(GapObj(G))::GapObj
       @req f !== GAP.Globals.fail "Could not convert group into a group of type $T"
       # The image of `f` can be a *subgroup* of a full pc group.
       # In this situation, we have to switch to a full pc group.
       C = GAPWrap.ImagesSource(f)
       if !_is_full_pc_group(C)
         ...
         # Remember `CC`: GAP computes `ImagesSource` of the composition as
         # the subgroup generated by the images of `gens(G)`, and those need
         # not generate `CC` as a full pc group.
         C = CC
       end
     end
     H = T(C)
