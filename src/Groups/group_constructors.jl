...
function quaternion_group(::Type{T}, n::IntegerUnion) where T <: GAPGroup
  ...
  @req iszero(mod(n, 4)) "n must be a multiple of 4"
...
@doc raw"""
    heisenberg_group(R::Ring, n::Int = 3)

Return the (generalized) Heisenberg group consisting of $n\times n$ matrices
over $R$. Note that $n$ must be odd, and currently $R$ must be either a finite
field or the integers.

# Examples
```jldoctest
julia> G = heisenberg_group(GF(3))
...
julia> G = heisenberg_group(GF(3), 5)
Matrix group of degree 5 over Finite field of characteristic 3

julia> factor(order(G))
1 * 3^7

julia> G = heisenberg_group(GF(3,2))
Matrix group of degree 3 over Finite field of degree 2 over GF(3)

julia> factor(order(G))
1 * 3^6

julia> G = heisenberg_group(ZZ)
Matrix group of degree 3 over Integer ring

julia> is_finite(G)
false
```
"""
function heisenberg_group(R::Ring, n::Int = 3)
  @req n > 0 "n must be positive"
  @req is_odd(n) "n must be odd"

  n == 1 && return matrix_group(R, matrix(R, 1, 1, [one(R)]))

  if R === ZZ
    G = _heisenberg_group_helper(R, n, [one(R)])

    # TODO: set this as the "nice monomorphism" output:
    # PcGroup(GAP.Globals.HeisenbergPcpGroup((n-1)>>1)::GapObj)
  else
    @req R isa Field "R must be the integers or a finite field"
    @req is_finite(R) "R must be the integers or a finite field"
    G = _heisenberg_group_helper(R, n, basis(R))

    # TODO: set a "nice monomorphism" into a pc-group. One can write
    # this down explicitly, and even an effective isomorphism. If
    # someone really needs this, let's talk.
  end

  return G
end

# constructing the generators is in a separate helper to
# ensure type stability
function _heisenberg_group_helper(R::Ring, n::Int, B)
  gens = dense_matrix_type(R)[]
  for x in B
    for i in 2:n-1
    ...
    # the matrices with entries in the top right corner
    # are generated by the above generators, so we don't
    # need them here.
    #g = identity_matrix(R, n)
    #g[1,n] = x
    #push!(gens, g)
