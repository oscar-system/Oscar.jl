...
@doc raw"""
     group(g::PermGroupElem, gs::PermGroupElem...)
     group(gs::AbstractVector{PermGroupElem})

     group(g::MatElem, gs::MatElem...; check::Bool=true)
     group(gs::AbstractVector{MatElem}; check::Bool=true)

     group(g::PcGroupElem, gs::PcGroupElem...)
     group(gs::AbstractVector{PermGroupElem})

     group(g::SubPcGroupElem, gs::SubPcGroupElem...)
     group(gs::AbstractVector{<:SubPcGroupElem})
     
Construct the group generated by the given generators.

For permutation generators this delegates to [`permutation_group`].
For matrix generators this delegates to [`matrix_group`] and forwards `check`.
"""
...
group(g::PcGroupElem, gs::PcGroupElem...) = group(PcGroupElem[g, gs...])
group(gs::AbstractVector{<:PcGroupElem}) = sub(parent(gs[1]), PcGroupElem[gs...])[1]

group(g::SubPcGroupElem, gs::SubPcGroupElem...) = group(SubPcGroupElem[g, gs...])
group(gs::AbstractVector{<:SubPcGroupElem}) = sub(parent(gs[1]), SubPcGroupElem[gs...])[1]
