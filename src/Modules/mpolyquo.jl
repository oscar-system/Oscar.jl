...
#    p = Hecke.ComplexOfMorphisms(SparseFPModule, pushfirst!(SparseFPModuleHom[map(p, i) for i in collect(range(p))[1:end-1]], phi), check=false, seed = -2)
#    i = i+1
#    K, inc = kernel(phi)
#  end
#  #end_map = hom(FreeMod(R, 0), K, elem_type(K)[])
#  p = Hecke.ComplexOfMorphisms(SparseFPModule, vcat(SparseFPModuleHom[inc], SparseFPModuleHom[map(p, i) for i in collect(range(p))[1:end-1]]), check=false, seed = -2)
#  return p
#end
