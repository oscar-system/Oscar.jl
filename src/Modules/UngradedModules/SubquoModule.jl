...
and relations `q`.
"""
function SubquoModule(F::FreeMod{T}, g::Vector{FreeModElem{T}}, q::Vector{FreeModElem{T}}) where {T<:AdmissibleOFPModuleRingElem} 
...
function show_morphism_as_map(f::OFPModuleHom, print_non_zero_only = false)
...
```
"""
function cokernel(f::OFPModuleHom{T1, T2}) where {T1, T2}
  return quo(codomain(f), image(f)[1])::Tuple{SubquoModule{elem_type(base_ring_type(T2))}, OFPModuleHom}
end

function cokernel_object(f::OFPModuleHom{T1, T2}) where {T1, T2}
...
```
"""
function annihilator(N::OFPModule{T}) where T
...
@doc raw"""
    function krull_dim(M::OFPModule)
...
```
"""
@attr Union{Int,NegInf} function krull_dim(M::OFPModule)
  ann = annihilator(M)
