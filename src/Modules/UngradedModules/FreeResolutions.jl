...
!!! note
    In the next note, we explain what the different options stand for. In doing so, we will not distinguish
    between a homomorphism of free modules over `base_ring(M)` and the corresponding polynomial matrix.
    Also, we somewhat abuse our notation by saying that a polynomial matrix is minimal if all its constant
...
julia> B = [Z Z Z Z Z O; w^2-x*z w*x-y*z x^2-w*y x*y-z^2 y^2-w*z Z];
...
julia> FM1[1]
Graded free module R^1([0]) + R^5([-2]) of rank 6 over R
