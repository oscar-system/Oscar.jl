...
@doc raw"""
    sub(F::FreeMod{T}, V::Vector{<:FreeModElem{T}}) where {T}
...
"""
function sub(F::FreeMod{T}, V::Vector{<:FreeModElem{T}}) where {T}
  s = SubquoModule(F, V)
...
@doc raw"""
    sub(F::FreeMod{T}, A::MatElem{T}) where {T} 
...
@doc raw"""
    sub(F::FreeMod{T}, M::SubquoModule{T}) where T
...
If only the submodule itself is desired, use `submodule` instead.
"""
function sub(F::FreeMod{T}, s::SubquoModule{T}) where T
...
@doc raw"""
    sub(M::SubquoModule{T}, V::Vector{<:SubquoModuleElem{T}}) where T
...
If only the submodule itself is desired, use `submodule` instead.
"""
function sub(M::SubquoModule{T}, V::Vector{<:SubquoModuleElem{T}}) where T
...
```
"""
function sub(M::ModuleFP{T}, V::Vector{<:ModuleFPElem{T}}) where T
