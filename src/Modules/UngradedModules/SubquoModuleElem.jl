...
@doc raw"""
    quo(F::FreeMod{T}, V::Vector{<:FreeModElem{T}}) where T
...
"""
function quo(F::FreeMod{T}, V::Vector{<:FreeModElem{T}}) where T
...
@doc raw"""
    quo(F::FreeMod{T}, A::MatElem{T}) where {T}
...
@doc raw"""
    quo(F::FreeMod{T}, O::Vector{<:SubquoModuleElem{T}}) where T
...
"""
function quo(F::FreeMod{T}, O::Vector{<:SubquoModuleElem{T}}) where T
  S = SubquoModule(F, basis(F))
...
@doc raw"""
    quo(S::SubquoModule{T}, O::Vector{<:FreeModElem{T}}) where T
...
elements of `S`.
"""
function quo(F::SubquoModule{T}, O::Vector{<:FreeModElem{T}}) where T
  if length(O) > 0
...
@doc raw"""
    quo(S::SubquoModule{T}, V::Vector{<:SubquoModuleElem{T}}) where T
...
If one is only interested in the actual object `M`, but not the map, use `quo_object` instead.
"""
function quo(M::SubquoModule{T}, V::Vector{<:SubquoModuleElem{T}}) where T
  return quo(M, [repres(x) for x = V])
end

@doc raw"""
    quo(M::ModuleFP{T}, V::Vector{<:ModuleFPElem{T}}) where T
...
```
"""
function quo(M::ModuleFP{T}, V::Vector{<:ModuleFPElem{T}}) where T
...
"""
function quo(M::SubquoModule{T}, U::SubquoModule{T}) where T
...
function quo(M::SubquoModule{T}, U::SubquoModule{T}) where {T<:MPolyRingElem}
  if isdefined(M, :quo) && isdefined(U, :quo)
    @assert M.quo == U.quo
  else
    @assert !isdefined(M, :quo) && !isdefined(U, :quo)
...
@doc raw"""
    quo(F::FreeMod{R}, T::SubquoModule{R}) where R
...
"""
function quo(F::FreeMod{R}, T::SubquoModule{R}) where R
