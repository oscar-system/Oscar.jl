...
```
"""
function prune(M::ModuleFP)
  return prune_atomic(M)
end

# generic fallback
function prune_atomic(M::ModuleFP)
...
function prune_atomic(M::ModuleFP{T}) where {T<:Union{MPolyRingElem, MPolyQuoRingElem}} # The case that can be handled by Singular
