...
function presentation(SQ::SubquoModule;
  ...
  M = Hecke.ComplexOfMorphisms(SparseFPModule, SparseFPModuleHom[h_G_F, h_F_SQ, h_SQ_Z], check = false, seed = -2)
...
function _presentation_graded(SQ::SubquoModule)
  ...
  M = Hecke.ComplexOfMorphisms(SparseFPModule, SparseFPModuleHom[F1_to_F0, F0_to_SQ, SQ_to_Z], check=false, seed = -2)
...
function presentation(F::FreeMod; minimal = false)
  ...
  M = Hecke.ComplexOfMorphisms(SparseFPModule, SparseFPModuleHom[hom(Z, F, Vector{elem_type(F)}()), hom(F, F, gens(F)), hom(F, Z, Vector{elem_type(Z)}([zero(Z) for i=1:ngens(F)]))], check = false, seed = -2)
  set_attribute!(M, :show => Hecke.pres_show)
  return M
end

@doc raw"""
    presentation(M::SparseFPModule; minimal = false)
...
```
"""
function presentation(M::SparseFPModule; minimal=false)
...
```
"""
function prune_with_map(M::SparseFPModule)
...
function prune_with_map(M::SparseFPModule{T}) where {T<:Union{MPolyRingElem, MPolyQuoRingElem}} # The case that can be handled by Singular
...
function _presentation_minimal(SQ::SparseFPModule{T};
  ...
  # compile the presentation complex
  CC = Hecke.ComplexOfMorphisms(SparseFPModule, SparseFPModuleHom[F1_to_F0, F0_to_SQ, SQ_to_Z], check=false, seed = -2)
