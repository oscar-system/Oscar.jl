...
function presentation(SQ::SubquoModule;
  ...
  M = Hecke.ComplexOfMorphisms(OFPModule, OFPModuleHom[h_G_F, h_F_SQ, h_SQ_Z], check = false, seed = -2)
...
function _presentation_graded(SQ::SubquoModule)
  ...
  if ngens(SQ) == ngens(F) && all(repres(v) == g for (v, g) in zip(gens(SQ), gens(F)))
    ...
    M = Hecke.ComplexOfMorphisms(OFPModule, OFPModuleHom[phi, aug, SQ_to_Z], check=false, seed = -2)
    set_attribute!(M, :show => Hecke.pres_show)
    return M
  end
  ...
  # compile the presentation complex
  M = Hecke.ComplexOfMorphisms(OFPModule, OFPModuleHom[F1_to_F0, F0_to_SQ, SQ_to_Z], check=false, seed = -2)
...
@doc raw"""
    presentation(M::OFPModule; minimal = false)
...
```
"""
function presentation(M::OFPModule; minimal=false)
...
IIf `M` is positively graded, return an isomorphic module `N` 
which is generated by a minimal number of elements.
...
```
"""
function prune(M::OFPModule)
  return prune_atomic(M)
end

# generic fallback
function prune_atomic(M::OFPModule)
...
function prune_atomic(M::OFPModule{T}) where {T<:Union{MPolyRingElem, MPolyQuoRingElem}} # The case that can be handled by Singular
  ...
  if iszero(krnel)
    if is_graded(M)
      F0 = pm[0]
      return SubquoModule(F0, gens(F0))
    end
    return M
  end
  ...
  s_mod_new, p = Singular.prune_projection(s_mod)
...
If `M` is positively graded, return an isomorphic module `N` 
which is generated by a minimal number of elements. Also, 
return an isomorphism `N` $\overset{\cong{\rightarrow}$ `M`.
...
```
"""
function prune_with_map(M::OFPModule)
  return prune_with_map_atomic(M)
end

# generic fallback
function prune_with_map_atomic(M::OFPModule)
  # TODO: take special care of graded modules 
  # by stripping off the grading and rewrapping it afterwards.
  N, b = simplify(M)
  return N, b
...
# MpolyRing and MPolyQuoRing case
function prune_with_map_atomic(M::OFPModule{T}) where {T<:Union{MPolyRingElem, MPolyQuoRingElem}} # The case that can be handled by Singular
...
function _presentation_minimal(SQ::OFPModule{T};
  ...
  # compile the presentation complex
  CC = Hecke.ComplexOfMorphisms(OFPModule, OFPModuleHom[F1_to_F0, F0_to_SQ, SQ_to_Z], check=false, seed = -2)
  set_attribute!(CC, :show => Hecke.pres_show)
  return CC
end

function prune(F::FreeMod)
  return F
end
