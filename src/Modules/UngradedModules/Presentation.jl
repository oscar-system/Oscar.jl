...
function presentation(SQ::SubquoModule)
  ...
  @show "new call to presentation???????????????????????????????????????????????????????????????????????? ####################################################################################"
  @show ambient_free_module(SQ)
  @show gens(ambient_free_module(SQ))
  @show degree.(gens(ambient_free_module(SQ)))
  @show gens(SQ)
  @show degree.(gens(SQ))
  @show relations(SQ)
  @show degree.(relations(SQ))
  R = base_ring(SQ)
  if is_graded(SQ)
  ...
  if is_generated_by_standard_unit_vectors(SQ.sub)
    ...
    #q = elem_type(F)[]
    @show c
    @show is_homogeneous.(c)

    for x = c
      @show degree(x)
      @show is_homogeneous(x)
      b = sparse_row(R)
      #e = zero(SQ.F)
      for (i,v) = x.coords
        if i>ngens(SQ)
