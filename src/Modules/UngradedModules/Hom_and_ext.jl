...
function hom(F::FreeMod, G::OFPModule)
  R = base_ring(F)
  ...
  function _elem_to_hom1(v::OFPModuleElem)
  ...
  function _hom_to_elem1(phi::OFPModuleHom)
    @assert domain(phi) === F
...
function hom(M::SubquoModule, N::OFPModule)
  R = base_ring(M)
  ...
  function _elem_to_hom2(v::OFPModuleElem)
    w = inc(iso(v)) 
  ...
  function _hom_to_elem2(phi::OFPModuleHom)
    # phi : M --> N
    @assert domain(phi) === M
...
function hom(
    phi::FreeModuleHom{<:FreeMod, <:FreeMod}, N::OFPModule; 
    domain::OFPModule=hom(Oscar.codomain(phi), N)[1],
    codomain::OFPModule=hom(Oscar.domain(phi), N)[1]
  )
  R = base_ring(N)
...
```
"""
function element_to_homomorphism(f::OFPModuleElem)
  H = f.parent
...
@doc raw"""
    homomorphism_to_element(H::OFPModule, a::OFPModuleHom)
...
```
"""
function homomorphism_to_element(H::OFPModule, a::OFPModuleHom)
  to_hom_map = get_attribute(H, :module_to_hom_map)
...
"""
function multiplication_induced_morphism(F::FreeMod, H::OFPModule)
  @assert rank(F) == 1
