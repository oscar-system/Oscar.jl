...
```
"""
function fitting_ideal(M::SparseFPModule{T}, i::Int) where T <: MPolyRingElem
...
```
"""
function is_flat(M::SparseFPModule{T}) where T <: MPolyRingElem
 error("not implemented for the given type of module.")
end
...
```
"""
function non_flat_locus(M::SparseFPModule{T}) where T <: MPolyRingElem
 error("not implemented for the given type of module.")
end
...
```
"""
function is_regular_sequence(V::Vector{T}, M::SparseFPModule{T}) where T <: MPolyRingElem
 error("not implemented for the given type of module.")
end
...
```
"""
function koszul_homology(V::Vector{T}, F::SparseFPModule{T}, i::Int) where T <: MPolyRingElem
  R = base_ring(F)
...
function _koszul_homology(V::Vector{T}, F::SparseFPModule{T}, i::Int) where T <: MPolyRingElem
  r = length(V)
  R = base_ring(F)
  if iszero(r)
    iszero(i) && return F
...
function _koszul_homology_graded(V::Vector{T},F::SparseFPModule{T}, i::Int) where T <: MPolyRingElem
  r = length(V)
  R = base_ring(F)
  if iszero(r)
    iszero(i) && return F
...
```
"""
function depth(I::Ideal{T}, F::SparseFPModule{T}) where T <: RingElem
...
function depth(I::MPolyIdeal{T}, F::SparseFPModule{T}) where T <: MPolyRingElem{<:FieldElem}
...
function _depth_from_below(I::Ideal{T}, F::SparseFPModule{T}) where T <: RingElem
  R = base_ring(F)
  R === base_ring(I) || error("rings are incompatible")
  f = small_generating_set(I)
  n = length(f)
...
function koszul_complex(V::Vector{T}, M::SparseFPModule{T}) where T <: MPolyRingElem
  r = length(V)
  iszero(r) && error("list must not be empty")
  R = base_ring(M)
  @assert all(x->parent(x)===R, V) "parent mismatch"
