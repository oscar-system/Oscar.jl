...
function (flat_map::RingFlattening)(F::FreeMod{T}) where {T <: FlattableRingElemType}
  ...
  end::Tuple{<:OFPModule, <:OFPModuleHom, <:OFPModuleHom}
end
...
function free_resolution(
  ...
  return get!(flat_counterparts(flat), comp) do
    ...
    comp_up = ComplexOfMorphisms(OFPModule, reverse(res_maps), typ=:chain, seed=-1, check=false)
    comp_up.complete = true
    return FreeResolution(comp_up)
  end::FreeResolution
end

function (phi::RingFlattening)(f::OFPModuleHom)
