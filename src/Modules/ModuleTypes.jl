...
# we expect the `OFPModule` framework to be functional and/or working.
# It can gradually be extended, but should not be considered to be 
# visible or accessible to the outside world. 
const AdmissibleOFPModuleRingElem = Union{RingElem, PBWAlgQuoElem, PBWAlgElem}
const AdmissibleOFPModuleRing = Union{Ring, PBWAlgQuo, PBWAlgRing}
...
"""
abstract type OFPModule{T <: AdmissibleOFPModuleRingElem} <: AbstractAlgebra.Module{T} end

@doc raw"""
    AbstractFreeMod{T} <: OFPModule{T}

The abstract supertype of all finitely generated free modules.
"""
abstract type AbstractFreeMod{T <: AdmissibleOFPModuleRingElem} <: OFPModule{T} end

@doc raw"""
    AbstractSubQuo{T} <: OFPModule{T}

The abstract supertype of all finitely presented subquotient modules.
"""
abstract type AbstractSubQuo{T <: AdmissibleOFPModuleRingElem} <: OFPModule{T} end


@doc raw"""
    OFPModuleElem{T} <: ModuleElem{T}

The abstract supertype of all elements of finitely presented modules.
"""
abstract type OFPModuleElem{T <: AdmissibleOFPModuleRingElem} <: ModuleElem{T} end

@doc raw"""
    AbstractFreeModElem{T} <: OFPModuleElem{T}

The abstract supertype of all elements of finitely generated free modules.
"""
abstract type AbstractFreeModElem{T <: AdmissibleOFPModuleRingElem} <: OFPModuleElem{T} end

@doc raw"""
    AbstractSubQuoElem{T} <: OFPModuleElem{T}

The abstract supertype of all elements of subquotient modules.
"""
abstract type AbstractSubQuoElem{T <: AdmissibleOFPModuleRingElem} <: OFPModuleElem{T} end

abstract type OFPModuleHomDummy end

@doc raw"""
    OFPModuleHom{T1, T2, RingMapType}
...
"""
abstract type OFPModuleHom{T1, T2, RingMapType} <: Map{T1, T2, Hecke.HeckeMap, OFPModuleHomDummy} end

parent(f::OFPModuleHom) = Hecke.MapParent(domain(f), codomain(f), "homomorphisms")
...
"""
@attributes mutable struct FreeMod{T <: AdmissibleOFPModuleRingElem} <: AbstractFreeMod{T}
  R::NCRing
  n::Int
  ...
  # low cost).
  incoming::WeakKeyIdDict{<:OFPModule, <:Tuple{<:SMat, <:Any}}
  outgoing::WeakKeyIdDict{<:OFPModule, <:Tuple{<:SMat, <:Any}}

  function FreeMod{T}(n::Int, R::AdmissibleOFPModuleRing, S::Vector{Symbol}) where T <: AdmissibleOFPModuleRingElem
    r = new{elem_type(R)}()
    r.n = n
    r.R = R
    r.S = S
    ...
    r.incoming = WeakKeyIdDict{OFPModule, Tuple{SMat, Any}}()
    r.outgoing = WeakKeyIdDict{OFPModule, Tuple{SMat, Any}}()
    return r
  end
  
  function FreeMod{T}(n::Int, R::AdmissibleOFPModuleRing, symbol_fun::Function) where T <: AdmissibleOFPModuleRingElem
...
@doc raw"""
    FreeModElem{T <: AdmissibleOFPModuleRingElem} <: AbstractFreeModElem{T}
...
```
"""
mutable struct FreeModElem{T <: AdmissibleOFPModuleRingElem} <: AbstractFreeModElem{T}
...
"""
@attributes mutable struct ModuleGens{T <: AdmissibleOFPModuleRingElem} # T is the type of the elements of the ground ring.
  ...
  function ModuleGens{T}(O::Vector{<:FreeModElem}, F::FreeMod{T}) where {T <: AdmissibleOFPModuleRingElem}
    r = new{T}()
    r.O = O
    r.F = F
    r.isGB = false
  ...
  function ModuleGens{T}(O::Vector{<:FreeModElem}, F::FreeMod{T}, SF::Singular.FreeMod) where {T <: AdmissibleOFPModuleRingElem}
    r = new{T}()
    r.O = O
    r.SF = SF
    r.F = F
  ...
  function ModuleGens{S}(F::FreeMod{S}, s::Singular.smodule) where {S <: AdmissibleOFPModuleRingElem} # FreeMod is necessary due to type S
    r = new{S}()
    r.F = F
...
@doc raw"""
    SubModuleOfFreeModule{T <: AdmissibleOFPModuleRingElem} <: OFPModule{T}
...
"""
@attributes mutable struct SubModuleOfFreeModule{T <: AdmissibleOFPModuleRingElem} <: OFPModule{T}
  F::FreeMod{T}
...
@doc raw"""
    SubquoModule{T <: AdmissibleOFPModuleRingElem} <: AbstractSubQuo{T}
...
Moreover, canonical incoming and outgoing morphisms are stored if the corresponding
option is set in suitable functions.
`SubquoModule{T}` is a subtype of `OFPModule{T}`.
"""
@attributes mutable struct SubquoModule{T <: AdmissibleOFPModuleRingElem} <: AbstractSubQuo{T}
  ...
  function SubquoModule{R}(F::FreeMod{R}) where {R}
    ...
    return new{R}(F,
                  Dict(),  # groebner_basis
                  WeakKeyIdDict{OFPModule, Tuple{SMat, Any}}(), # incoming
                  WeakKeyIdDict{OFPModule, Tuple{SMat, Any}}(), # outgoing
                  )
  end
end
...
@doc raw"""
    SubquoModuleElem{T <: AdmissibleOFPModuleRingElem} <: AbstractSubQuoElem{T} 
...
```
"""
mutable struct SubquoModuleElem{T <: AdmissibleOFPModuleRingElem} <: AbstractSubQuoElem{T} 
  ...
  function SubquoModuleElem{R}(v::SRow{R}, SQ::SubquoModule) where {R <: AdmissibleOFPModuleRingElem}
  ...
  function SubquoModuleElem{R}(a::FreeModElem{R}, SQ::SubquoModule; is_reduced::Bool=false) where {R <: AdmissibleOFPModuleRingElem}
...
mutable struct SubQuoHom{
  ...
  inverse_isomorphism::OFPModuleHom
  ...
  function SubQuoHom{T1,T2,RingMapType}(D::SubquoModule, C::OFPModule, im::Vector;
                                        check::Bool=true
                                       ) where {T1,T2,RingMapType}
    ###@assert is_graded(D) == is_graded(C)
    @assert length(im) == ngens(D)
...
@doc raw"""
    FreeMod_dec{T <: CRingElem_dec} <: OFPModule_dec{T}
...
Moreover, canonical incoming and outgoing morphisms are stored if the corresponding
option is set in suitable functions.
`FreeMod_dec{T}` is a subtype of `OFPModule{T}`.
"""
...
const OFPModule_dec{T} = Union{FreeMod_dec{T}} # SubquoDecModule{T} will be included
const OFPModuleElem_dec{T} = Union{FreeModElem_dec{T}} # SubquoDecModuleElem{T} will be included


@doc raw"""
    FreeModuleHom{T1, T2, RingMapType} <: OFPModuleHom{T1, T2, RingMapType} 

Data structure for morphisms where the domain is a free module (`FreeMod`).
`T1` and `T2` are the types of domain and codomain respectively.
`FreeModuleHom` is a subtype of `OFPModuleHom`.
...
@attributes mutable struct FreeModuleHom{
    T1 <: AbstractFreeMod,
    T2 <: OFPModule,
    RingMapType <: Any} <: OFPModuleHom{T1, T2, RingMapType} 
  header::MapHeader{T1, T2}
  ring_map::RingMapType
  d::FinGenAbGroupElem
...
struct FreeModuleHom_dec{
  ...
  function FreeModuleHom_dec(F::FreeMod_dec{T}, G::OFPModule_dec{T}, a::Vector) where {T}
    f = FreeModuleHom(F,G,a)
    r = new{typeof(F), typeof(G), Nothing}(f, f.header)
    return r
  end

  function FreeModuleHom_dec(F::FreeMod_dec{T}, G::OFPModule_dec{T}, mat::MatElem{T}) where {T}
    f = FreeModuleHom(F,G,mat)
    r = new{typeof(F), typeof(G), Nothing}(f, f.header)
    return r
  end

  function FreeModuleHom_dec(F::FreeMod_dec, G::OFPModule_dec, a::Vector, h::RingMapType) where {RingMapType}
    f = FreeModuleHom(F,G,a,h)
    r = new{typeof(F), typeof(G), RingMapType}(f, f.header)
    return r
  end

  function FreeModuleHom_dec(F::FreeMod_dec, G::OFPModule_dec{T}, mat::MatElem{T}, h::RingMapType) where {T, RingMapType}
