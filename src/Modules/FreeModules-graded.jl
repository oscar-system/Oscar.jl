...
function gens(F::SubQuo_dec)
  return collect(F.sub)
end

function gen(F::SubQuo_dec, i::Int)
  return F.sub[Val(:O), i]
end
...
mutable struct SubQuoHom_dec{T1, T2} <: Map{T1, T2, Hecke.HeckeMap, SubQuoHom_dec}
  ...
  function SubQuoHom_dec(D::SubQuo_dec, C::SubQuo_dec, im::Array{<:SubQuoElem_dec, 1})
    first = true
...
hom(D::SubQuo_dec, C::SubQuo_dec, A::Array{<:SubQuoElem_dec, 1}) = SubQuoHom_dec(D, C, A)
