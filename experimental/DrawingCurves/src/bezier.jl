...
Return the Bernstein basis polynomial $B_{i,n}(t) = \binom{n}{i} \cdot t^i \cdot (1-t)^{n-i}$.
...
function bernstein_basis_polynomial(i::Int, n::Int)
  R, _ = polynomial_ring(ZZ, :t; cached=false)
  bernstein_basis_polynomial(R, i, n)
end

"""
    bernstein_polynomial(R::QQPolyRing, coeffs)

Return the Bernstein polynomial which is the linear combination of the
Bernstein basis polynomials with the given coefficients.
...
function bernstein_polynomial(coeffs::Vector{QQFieldElem})
  R, _ = polynomial_ring(QQ, :t; cached=false)
...
Return a Bernstein polynomial of degree `n` which approximates the polynomial
`f` on the interval [0,1].
The sequence of these polynomials converges uniformly to `f` on [0,1] if `n`
goes to infinity.
...
function bezier_curve(p::Vector{Vector{QQFieldElem}})
  ...
  d = length(p[1])
  for k in 1:d
    bernstein_polynomial([p[i][k] for i in 1:n])
  end
end
...
function _draw_edge_sequence_bernstein(T::MatrixElem)
  return function _draw_edge_sequence_bernstein(
    ...
    pts100 = [_Point(p1x + dist * i//100, evaluate(f2, i//100)) for i in 0:100]
    for i in 1:(length(pts100) - 1)
      _draw_edge_tikz(io, T * pts100[i], T * pts100[i + 1], scale; color="blue")
    end
  end
end
