...
function get_scale(IG::_IsotopyGraph, random_transform)::QQFieldElem
...
function add_edge!(
  ...
  add_edge!(IG.G, IG.pair2node[p0], IG.pair2node[p1])
  if nosequence
...
function _assemble_isotopy_graph!(
  ...
  for i in 1:(length(svecs) - 1)
    ...
    x1 = x0 + dist / 3
    x2 = x3 - dist / 3
    y1 = root_finder(projy(evaluate(f, [Rxy(x1), y])); solver_precision)
    y2 = root_finder(projy(evaluate(f, [Rxy(x2), y])); solver_precision)
    ...
    if length(y1) == length(y2)
      ...
      @vprintln :DrawingCurves 2 "Incoherency between critical points detected."
    end
  end

  # Collect some points before the first and after the last one.
  dist = QQ(xmax - xmin)//20
  if dist == 0
    dist = QQ(1)
  end
  ...
  for (coeff, pos) in ((1, 1), (-1, length(svecs)))
    x1 = xcoords[pos]
    x0 = x1 - coeff * dist
    y0 = root_finder(projy(evaluate(f, [Rxy(x0), y])); solver_precision)
    y1 = svecs[pos]
