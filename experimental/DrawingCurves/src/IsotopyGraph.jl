...
_get_edges_with_singularity(na::Int, nb::Int, si::Int) = _get_edges_with_singularity(collect(1:na), collect(1:nb), si)
...
function _IsotopyGraph()
   ...
   edge4sequences = Vector{Vector{Int}}()
   nosequenceedges = Vector{Vector{Int}}()
   return _IsotopyGraph(G, node2pair, pair2node, singularNodes, ytangentNodes, node2coordinates, edge4sequences, nosequenceedges)
end
...
function add_edge!(IG::_IsotopyGraph, p0::Vector{Int}, p1::Vector{Int}; nosequence::Bool=false)
...
function _assemble_isotopy_graph!(IG::_IsotopyGraph, f, singcoords, stypes, svecs, sindices, random_transform, selected_precision, root_finder)
   ...
   for i in 1:length(svecs)-1
      ...
      if length(y1) == length(y2)
         ...
         end_edges = _get_edges_with_singularity(length(svecs[i+1]), length(y2), sindices[i+1])
