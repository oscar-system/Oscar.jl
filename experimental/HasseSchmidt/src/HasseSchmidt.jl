...
Return a list of Hasse-Schmidt derivatives of `f`, each with a multiindex
`[a_1, ..., a_n]`, where `a_i` describes the number of times `f` was derived
...
8-element Vector{Tuple{Vector{Int64}, ZZMPolyRingElem}}:
 ([0, 5], 3)
 ([0, 4], 15*y)
 ([0, 3], 30*y^2)
 ([2, 0], 5)
...
Return a list of Hasse-Schmidt derivatives of lift of `f`, each with a
multiindex `[a_1, ..., a_n]`, where `a_i` describes the number of times `f`
was derived w.r.t. the `i`-th variable.
...
5-element Vector{Tuple{Vector{Int64}, ZZMPolyRingElem}}:
 ([0, 4], 2)
 ([0, 3], 8*y)
 ([0, 2], 12*y^2)
 ([0, 1], 8*y^3)
...
Return a list of Hasse-Schmidt derivatives of numerator of `f`, each with a
multiindex `[a_1, ..., a_n]`, where `a_i` describes the number of times `f`
was derived w.r.t. the `i`-th variable.
...
julia> Oscar._hasse_derivatives(f)
6-element Vector{Tuple{Vector{Int64}, QQMPolyRingElem}}:
 ([0, 0, 5], 2)
 ([0, 0, 4], 10*z)
 ([0, 0, 3], 20*z^2)
 ([0, 0, 2], 20*z^3)
...
Return a list of Hasse-Schmidt derivatives of lifted numerator of `f`, each
with a multiindex `[a_1, ..., a_n]`, where `a_i` describes the number of times
`f` was derived w.r.t. the `i`-th variable.
...
julia> Oscar._hasse_derivatives(f)
4-element Vector{Tuple{Vector{Int64}, QQMPolyRingElem}}:
 ([0, 3, 0], 4)
 ([0, 2, 0], 12*y)
 ([0, 1, 0], 12*y^2)
 ([0, 0, 0], 4*y^3)
