...
function blow_up(i::AbstractVarietyMap; symbol::String = "e")
  ...
  # by E-H, Prop. 13.12, ABl is generated by j_push(AE) and  f_pull(AX) as a K-algebra;
  ...
  # 3) relation from AE: ∑ g_pull(c_k(N)) ⋅ ζ^(d-k) = 0, see EH Thm. 9.6

  cN = total_chern_class(N)[0:rN] # cN[k] = c_k ₋₁(N)
  push!(Rels, sum([j_push_g_pull(cN[k+1]) * (-ev[end])^(rN-k) for k in 0:rN]))

  # 4) j_push(x) ⋅ j_push(y) = -j_push(x⋅y⋅ζ) # rule for multiplication, EH, Prop. 13.12
  ...
  # we have ctop(Q) = ∑ g_pull(c_{k-1}(N)) ⋅ ζ^(d-k), EH p. 477
