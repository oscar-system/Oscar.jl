...
Given an inclusion $i\colon Z \hookrightarrow X$, construct the extended Chow ring
of the variety $X^+$ (which is also the variety $X$, except with more classes)
whose Chow ring $A^*(X^+)$ is the $A^*(X)$-algebra generated by the pushforward classes
from $A^*(Z)$.

Return a morphism $j\colon Z \to X^+$.
"""
