...
Given an inclusion $i\colon Z \hookrightarrow X$, construct the extended variety
$X^+$ whose Chow ring $A^*(X^+)$ is the $A^*(X)$-algebra generated by the
push-forward classes from $A^*(Z)$.
