...
function blowup(i::AbstractVarietyMap; symbol::String="e")
  ...
  RBl, ev, xv = graded_polynomial_ring(
  ...
  alpha = sum(
    sum(
      (binomial(ZZ(rN - j), ZZ(k)) - binomial(ZZ(rN - j), ZZ(k + 1))) * zeta^k for
      k in 0:(rN - j)
    ) * pullback(g, chern_class(N, j)) for j in 0:rN
  )
  Bl.T.chern = simplify(
