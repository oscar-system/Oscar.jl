...
codimension, that is, we work with the graded group ${\rm Z}^\ast(X) = \bigoplus^{\dim(X)}_{c=0} {\rm Z}^c(X)$, where
${\rm Z}^c(X)$ consists of the cycles of codimension $c$ on $X$.

A useful cycle theory is obtained by considering an *adequate equivalence relation*,
given on the cycle groups ${\rm Z}^\ast(X)$ of all varieties $X$ (see [Sam60](@cite), [Mur14](@cite)). Such a
relation $\sim$ is compatible with the group structure and  grading on ${\rm Z}^\ast(X)$. It gives rise to a suitable concept
of *moving cycles* and, thus, to a well-defined intersection product on cycle classes which makes the quotient group
${\rm C}_{\sim}^\ast(X) := {\rm Z}^\ast(X)/\sim$ into a graded ring (*intersection ring*). The construction of such rings is functorial
...
More precisely, for each $d$, building the group ${\rm C}_{\sim}^d(X)$ of cycle classes of dimension $d$ gives rise to a covariant
functor from varieties to groups via $f_{\ast}$, while building ${\rm C}_{\sim}^\ast(X)$ gives rise to a contravariant functor from varieties
to rings via $f^{\ast}$. Moreover, we have the *projection formula*

$f_\ast(f^\ast(\beta)\cdot \alpha) = f_\ast(\alpha)\cdot \beta \;\text{ for all }\; \alpha\in{\rm C}_{\sim}^\ast(X), \beta\in{\rm C}_{\sim}^\ast(Y)$
...
$\dim(X)$. Note that this part is isomorphic to $\mathbb Z$: It is generated by the class of $X$ in ${\rm C}_{\sim}^\ast(X)$,
...
$\deg =\int_X : {\rm C}_{\sim}^{\dim(X)}(X) \rightarrow \mathbb Z$

which sends the class of a point of $X$ to 1, and which extends to a homomorphism on all of ${\rm C}_{\sim}^\ast(X)$ :

$\int_X : {\rm C}_{\sim}^\ast(X)\rightarrow \mathbb Z,  \quad \alpha = \sum _{c = 0}^{\dim(X)} \alpha_c\mapsto \int \alpha_{\dim(X)}.$

In particular, we have the intersection pairings

${\rm C}_{\sim}^c(X)\times{\rm C}_{\sim}^{\dim(X)-c}(X)\rightarrow{\rm C}_{\sim}^{\dim(X)}(X) \overset{\deg}\longrightarrow \mathbb Z.$

The finest adequate equivalence relation is *rational equivalence* which is the  most common relation to work with.
The resulting intersection ring of $X$ is usually denoted by ${\rm A}^\ast(X)$ and called
the *Chow ring* of $X$. Note that such rings are often difficult to handle. For example, while ${\rm A}^0(X)$ is isomorphic
to $\mathbb Z$ as pointed out above, it may happen that the group ${\rm A}^{\dim(X)}$ is not even finitely generated.
...
We will write ${\rm N}^\ast(X)$ for the numerical intersection ring, or numerical Chow ring, of $X$.

!!! note
    In this chapter, we abuse our notation in that the name *Chow ring* always refers to a ring of type ${\rm N}^\ast(X)_{\mathbb Q}$.
...
${\rm N}^\ast(X)_{\mathbb Q} = {\rm N}^\ast(X) \otimes_{\mathbb Z} {\mathbb Q} = \bigoplus^{\dim(X)}_{c=0}{\rm N}^c(X)_{\mathbb Q}$

and extend notions such as integral or intersection pairing to this situation. The graded pieces ${\rm N}^c(X)_{\mathbb Q}$ are
then finite-dimensional $\mathbb Q$-vector spaces, their dimensions $\beta_c(X) = \dim{\rm N}^c(X)_{\mathbb Q}$ are
...
${\rm N}^c(X)_{\mathbb Q}\times{\rm N}^{\dim(X)-c}(X)_{\mathbb Q}\rightarrow{\rm N}^{\dim(X)}(X)_{\mathbb Q} \overset{\deg}\longrightarrow \mathbb Q$

are non-degenerate by the very definition of numerical equivalence, we have $\beta_c(X) = \beta_{\dim(X)-c}(X)$ for each $c$.
In particular, ${\rm N}^0(X)_{\mathbb Q}$ and ${\rm N}^{\dim(X)}_{\mathbb Q} $ are both 1-dimensional $\mathbb Q$-vector spaces,
...
Let $X$ be a variety. Write ${\rm K}^0(X)$ for the *Grothendieck group of vector bundles on $X$*. That is,
${\rm K}^0(X)$ is the free abelian group generated by the isomorphism classes of vector bundles on $X$,
...
The tensor product turns ${\rm K}^0(X)$ into a ring, the *Grothendieck ring of vector bundles on $X$*.
By Grothendieck--Riemann--Roch, the Chern character defines a ring isomorphism

$\text{ch}:{\rm K}^0(X)_{\mathbb Q}\overset{\simeq}\longrightarrow {\mathrm A}^*(X) _{\mathbb Q}.$

Since rational equivalence is (much) finer than numerical equivalence, we obtain a ring epimorphism

$\text{ch}: {\rm K}^0(X)_{\mathbb Q}\overset{\simeq}\longrightarrow{\rm A}^*(X) _{\mathbb Q}\twoheadrightarrow{\rm N}^*(X)_{\mathbb Q}.$

Write ${\rm K}_0(X)$ for the *Grothendieck group of coherent sheaves on $X$*.
Then, since $X$ is supposed to be smooth, the natural map ${\rm K}^0(X)\rightarrow{\rm K}_0(X)$ is an isomorphism
(every coherent sheaf on $X$ has a finite resolution by locally free sheaves). We may, thus, speak of coherent sheaves
given by *virtual vector bundles*, that is, by elements of ${\rm K}^0(X)$. Hence, we can use the concept of abstract
