...
work with is rational equivalence which yields intersection rings called Chow rings. In contrast to this usual practice, following
the authors of `Schubert`, we consider numerical equivalence, allowing rational coefficients when forming cycles. That is, if
$N^\ast(X)$ is the numerical intersection ring of a variety $X$, we consider the ring

$N^\ast(X)_{\mathbb Q} = N^\ast(X) \otimes_{\mathbb Z} {\mathbb Q}.$

We grade this ring by the codimension of cycle classes. In particular, as $\mathbb Q$-vector spaces, $N^{0}(X)_{\mathbb Q}\cong\mathbb Q$
is generated by the fundamental class $\left[X\right]$ of $X$, and $N^{\dim(X)}(X)_{\mathbb Q}$ is formed by the 0-dimensional cycle classes.
Note that we have a well-defined *degree homomorphism* , or *integral*,

$\deg =\int_X : N^{\dim(X)}(X)_{\mathbb Q}  \rightarrow \mathbb Q$
...
!!! note
    In this chapter, the name *Chow ring* always refers to a ring of type $N^\ast(X)_{\mathbb Q}$.
