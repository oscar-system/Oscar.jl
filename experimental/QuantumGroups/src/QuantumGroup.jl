...
@doc raw"""
    quantum_group(fam::Symbol, rk::Int; bilinear_form::ZZMatrix=Oscar.bilinear_form(R),
      w0::Vector{<:Integer}=word(longest_element(weyl_group(R))),
    ) -> QuantumGroup
"""
function quantum_group(fam::Symbol, rk::Int;
  ...
  return quantum_group(root_system(fam, rk); bilinear_form=bilinear_form, w0=w0)
end
...
function deepcopy_internal(U::QuantumGroup)
  return QuantumGroup(
    deepcopy_internal(U.algebra),
    deepcopy_internal(U.root_system),
    deepcopy_internal(U.bilinear_form),
    deepcopy_internal(U.w0),
  )
end
