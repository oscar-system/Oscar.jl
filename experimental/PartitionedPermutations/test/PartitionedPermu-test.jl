@testset "PartitionedPermutation" begin
    ...
    @testset "PartitionedPermutation Product" begin
        ...
        @test length(factorize(partitioned_permutation(perm(symmetric_group(3), [2, 1, 3]), [1, 1, 2]))) == 2
        @test length(factorize(partitioned_permutation(perm(symmetric_group(4), [2, 1, 4, 3]), [1, 1, 2, 2]))) <
              length(factorize(partitioned_permutation(perm(symmetric_group(4), [1, 2, 3, 4]), [1, 2, 3, 4])))
        @test length(factorize(partitioned_permutation(perm(symmetric_group(3), [2, 1, 3]), [1, 1, 2]))) <
              length(factorize(partitioned_permutation(perm(symmetric_group(3), [1, 2, 3]), [1, 2, 3])))
    end
end
