@testset "PartitionedPermutation" begin
    ...
    @testset "PartitionedPermutation Product" begin
        ...
        @test length(factorization_partitioned_permutation(partitioned_permutation(perm(symmetric_group(3), [2, 1, 3]), [1, 1, 2]))) == 2
        @test length(factorization_partitioned_permutation(partitioned_permutation(perm(symmetric_group(4), [2, 1, 4, 3]), [1, 1, 2, 2]))) <
              length(factorization_partitioned_permutation(partitioned_permutation(perm(symmetric_group(4), [1, 2, 3, 4]), [1, 2, 3, 4])))
        @test length(factorization_partitioned_permutation(partitioned_permutation(perm(symmetric_group(3), [2, 1, 3]), [1, 1, 2]))) <
              length(factorization_partitioned_permutation(partitioned_permutation(perm(symmetric_group(3), [1, 2, 3]), [1, 2, 3])))
    end
end
