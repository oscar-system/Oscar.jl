...
function _get_irreducible_ideal(kQ::MonoidAlgebra, J::IndecInj; minimal::Bool=true)
  ...
  # The lattice points of Algorithm 3.9 in [HM05] generate W, but most of
  # them are redundant: if v - w in Q for two of them, then x^v lies in the
  # ideal generated by x^w. Turning a point into a monomial costs a normal
  # form in the quotient ring, so by default only the minimal points are
  # used. The non-normal case (Algorithm 3.15) seeds an iteration with the
  # generators of the intersection with k[Q] and finishes sooner with the
  # redundant set, hence the keyword.
  minimal && (points = _minimal_points(kQ, points))
  G_W = [monomial_basis(kQ.algebra, ZZRingElem.(v))[1] for v in points]
  return ideal(kQ, G_W)
end

# The points of B (a finite subset of Q, without repetitions) that are
# minimal for the partial order w <= v iff v - w in Q. Since Q is pointed
# this is a partial order, every point of the finite set B lies above a
# minimal one, and x^v = x^(v-w) x^w, so the minimal points generate the
# same monomial ideal as B. They are in fact its unique minimal monomial
# generating set. Only used for normal Q, where membership in Q is a check
# of the facet inequalities of the cone.
