...
and every summand $k\{a + F - Q\}$ of the terms $J^0, \dots, J^i$ of the minimal
...
function degrees_of_bass_numbers_bound(M::SubquoModule{<:MonoidAlgebraElem}, i::Int)
  ...
  # Every summand k{a + F - Q} of J^j has a in deg(g) - a_jl + Q + ZF, where
  # G_j = sum_l k[Q](-a_jl) resolves k[Q]/P_F (shift bound of the paper).
  D = Set{Vector{Int}}()
  for p in faces(kQ)
    ...
    # length 0 would mean a complete resolution, hence max(i, 1)
    free_res = free_resolution(RpF; length=max(i, 1))
    for j in 0:i
...
# Coefficient vectors lambda_i in ker(C) with <lambda_i, c_{b_j}> = delta_ij for
# the socle elements b_j of each ZF-class (Algorithm 1 of the paper). `degs`,
# `c_bs`, `C_rows`: degrees, coordinate vectors and relation rows of the b_j.
...
!!! note
    The monoid algebra $k[Q]$ must be normal.
"""
function _get_irreducible_ideal(kQ::MonoidAlgebra, J::IndecInj; minimal::Bool=true)
  B = Set{Vector{Int}}()
  for h in hyperplanes(kQ)
    if is_subset(J.face.poly, h.hyperplane)
      union!(B, generators_W_H(kQ, h, J.vector))
    end
  end
  points = sort!(collect(B))   # sorted for reproducible generator lists

  # The minimal points generate W and each point costs a normal form. The
  # non-normal case (Algorithm 3.15 in [HM05]) is faster with all of them.
  minimal && (points = _minimal_points(kQ, points))
  G_W = [monomial_basis(kQ.algebra, ZZRingElem.(v))[1] for v in points]
  return ideal(kQ, G_W)
end

# Points of B minimal for w <= v iff v - w in Q. They generate the same
# monomial ideal as B. Normal Q only, where membership is a facet check.
function _minimal_points(kQ::MonoidAlgebra, B::Vector{Vector{Int}})
  @assert is_normal(kQ)
  A_f, b_f = halfspace_matrix_pair(facets(cone(kQ)))
  A_m = Matrix(A_f)
  b_v = Vector(b_f)
  in_Q = v -> all(A_m*v .<= b_v)
  return [v for v in B if !any(w -> w != v && in_Q(v - w), B)]
end
...
function _get_irreducible_ideal_unsaturated(kQ::MonoidAlgebra, J::IndecInj)
  ...
  #compute irreducible ideal in kQsat, with the redundant generators of
  #Algorithm 3.9, which seed the iteration below better than the minimal ones
  V = _get_irreducible_ideal(kQsat, J; minimal=false)
...
The module is first shifted so that every summand of the terms $J^0, \dots, J^i$ of its minimal
...
function injective_resolution(M::SubquoModule{<:MonoidAlgebraElem}, i::Int; shift::Symbol=:bound, check::Bool=true)
  ...
  # The bound strategies control the summands of J^0, ..., J^i at every face.
  # The exact strategies go through the maximal ideal and need the Bass
  # numbers up to degree i + dim Q by [HM05, Lemma 4.5].
  depth_m = i + ambient_dimension(affine_semigroup(kQ))

  #compute irreducible resolution of shifted module
  if shift === :bound
    a_shift = compute_shift_bound(M, i)
  elseif shift === :helm_miller
    a_shift = compute_shift(M, depth_m)
  elseif shift === :milp
    a_shift = compute_shift_milp(M, depth_m)
  elseif shift === :milp_bound
    a_shift = compute_shift_milp_bound(M, i)
  else
...
# Rank of M over Frac(k[Q]): 0 for torsion modules, otherwise from the
# presentation matrix, which may be unimplemented over MPolyQuoRing.
...
# Canonical representative of b modulo the lattice ZF spanned by the columns
# of A, via HNF reduction.
...
function graded_bass_numbers(M::SubquoModule{<:MonoidAlgebraElem}, p::FaceQ, i::Int)
  ...
  # F = Q: R_{p_F} = Frac(k[Q]) is a field, so mu^0 = rank(M) and mu^j = 0 for
  # j > 0. This also avoids mod_quotient(-, (0)), which crashes hom(FreeMod, FreeMod).
  if is_zero(p.prime)
...
module `M` as an `InjMod` together with the scalar matrix of the embedding
$M \hookrightarrow E(M)$, whose entry $(i, j)$ is the coefficient with which the
$i$-th generator of `M` maps into the $j$-th summand of $E(M)$, as
[`augmentation_map`](@ref) returns it for a resolution.
"""
function injective_hull(M::SubquoModule{<:MonoidAlgebraElem})
  ...
  a_shift = compute_shift_bound(M, 0)
  M_a = twist(M, -G(a_shift))
  J, _lambda = irreducible_hull(M_a, 0)
  lambda = _scalar_matrix(kQ, _lambda)
