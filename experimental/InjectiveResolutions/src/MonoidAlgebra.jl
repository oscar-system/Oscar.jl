...
function Base.show(io::IO, ::MIME"text/plain", S::AffineSemigroup)
  println(io, "Affine semigroup in ambient dimension ", ambient_dimension(S), " generated by")
  print(io, "  ", join(string.(gens(S)), "\n  "))
end
...
@doc raw"""
    ...
    affine_semigroup(V::Vector{Vector{Int}})

Create the affine semigroup generated by the columns of the integer matrix `M`,
respectively by the integer vectors in `V`.
"""
...
Return the monoid algebra $k[Q]$ over the field `k`, where the affine semigroup $Q$ is
generated by the columns of `M_Q`, by the vectors in `V_Q`, or is given as `Q`.
The monoid algebra is represented as the quotient of a polynomial ring with one
variable $x_i$ of degree $v_i$ for each generator $v_i$ of $Q$ by the toric ideal.
...
# detailed printing fits on one line, so one method serves all print modes
function Base.show(io::IO, kQ::MonoidAlgebra)
  if is_terse(io)
    print(io, "Monoid algebra")
  else
    io = pretty(io)
    print(io, "Monoid algebra over ", Lowercase(), coefficient_ring(kQ),
...
function Base.show(io::IO, ::MIME"text/plain", I::MonoidAlgebraIdeal)
  io = pretty(io)
  println(io, "Ideal over ", Lowercase(), base_ring(I))
  if is_empty(gens(I))
    print(io, "generated by 0")
  else
    print(io, "generated by ", join(string.(gens(I)), ", "))
  end
end
...
function is_in_aZF(a::Vector{Int}, F::FaceQ, p::Vector{Int})
  F.A === nothing && return a == p
  # p - a lies in the lattice spanned by the columns of F.A iff the integer
  # linear system F.A * x = p - a is solvable
  return can_solve(matrix(ZZ, F.A), matrix(ZZ, length(p), 1, p .- a); side=:right)
end
...
function _generates_lattice(kQ::MonoidAlgebra)
