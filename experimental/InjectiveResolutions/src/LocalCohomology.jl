...
function local_cohomology_all(M::SubquoModule{T}, I::MonoidAlgebraIdeal, i::Integer) where {T<:MonoidAlgebraElem}
  ...
  for j in 1:i
    # cochain_maps[1] is the embedding M -> J^0, cochain_maps[k + 2] is d^k
    _phi = get_scalar_matrix(kQ, inj_res.cochain_maps[j + 1])
    _psi = get_scalar_matrix(kQ, inj_res.cochain_maps[j + 2])
