...
@attributes mutable struct FamilyOfSpaces
  ...
  dim::Int
  FamilyOfSpaces(
    coordinate_ring::MPolyDecRing{QQFieldElem,QQMPolyRing}, grading::Matrix{Int64}, dim::Int
  ) = new(
    coordinate_ring, grading, dim
  )
end
...
@attributes mutable struct ClosedSubschemeModel <: AbstractFTheoryModel
  ...
  # jacbian_fibration
  # mirror_dual
  ClosedSubschemeModel(base_space::FTheorySpace, ambient_space::FTheorySpace) = new(
    base_space, ambient_space
  )
end
...
@attributes mutable struct CompleteIntersectionModel <: AbstractFTheoryModel
  ...
  # zero section
  CompleteIntersectionModel(
    base_space::FTheorySpace, ambient_space::FTheorySpace, defining_ideal::MPolyIdeal
  ) = new(
    base_space, ambient_space, defining_ideal
  )
end
