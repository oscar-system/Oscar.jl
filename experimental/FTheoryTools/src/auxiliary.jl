...
function _ambient_space(
  base::NormalToricVariety,
  fiber_amb_space::NormalToricVariety,
  ...
  fiber_twist_divisor_classes_coeffs = [
    divisor_class(D).coeff for D in fiber_twist_divisor_classes
  ]
  ...
  a_cones = [
    hcat([b for b in b_cones[i:i, :]], [c for c in f_cones[j:j, :]]) for
    i in 1:nrows(b_cones), j in 1:nrows(f_cones)
  ]
  a_space = normal_toric_variety(
    IncidenceMatrix(vcat(a_cones...)), a_rays; non_redundant=true
  )
  ...
  a_space_class_group = free_abelian_group(
    ncols(b_grades) + torsion_free_rank(class_group_with_map(fiber_amb_space)[1])
  )

  # Compute grading of Cox ring of a_space
  a_space_grading = zero_matrix(
    ZZ, torsion_free_rank(a_space_divisor_group), torsion_free_rank(a_space_class_group)
  )
  a_space_grading[1:nrows(b_grades), 1:ncols(b_grades)] = b_grades
  a_space_grading[
    (1 + nrows(b_rays)):(nrows(b_rays) + nrows(f_grades)),
    (1 + ncols(b_grades)):(ncols(b_grades) + ncols(f_grades)),
  ] = f_grades
  ...
  set_attribute!(
    a_space, :map_from_torusinvariant_weil_divisor_group_to_class_group, a_space_grading
  )
...
function _weierstrass_sections(
  ...
  return [
    generic_section(anticanonical_bundle(base)^4; rng),
    generic_section(anticanonical_bundle(base)^6; rng),
  ]
end
...
function _tate_polynomial(ais::Vector{<:MPolyRingElem}, S::MPolyRing)
  ...
  return x^3 - y^2 - x * y * z * a1 + x^2 * z^2 * a2 - y * z^3 * a3 + x * z^4 * a4 +
         z^6 * a6
end
...
_count_factors(poly::QQMPolyRingElem) = mapreduce(
  p -> p[end], +, absolute_primary_decomposition(ideal([poly]))
)

_string_from_factor_count(poly::QQMPolyRingElem, string_list::Vector{String}) =
  string_list[_count_factors(
    poly
  )]

function _kodaira_type(
  id::MPolyIdeal{<:MPolyRingElem},
  ords::Tuple{Int64,Int64,Int64},
  w::WeierstrassModel;
  ...
  elseif d_ord == 6 && f_ord >= 2 && g_ord >= 3
    ...
    kod_type = _string_from_factor_count(
    ...
    if !is_base_space_fully_specified(w)
      ...
      concrete_data = merge(
        Dict(
          string(base_coords_symbols[i]) => generic_section(
            KBar^grading[1, i] *
            prod(hyperplane_bundle^grading[j, i] for j in 2:length(grading[:, 1]));
            rng,
          ) for i in eachindex(base_coords_symbols)
        ),
        Dict("base" => concrete_base),
      )
      ...
      list_of_sections = [
        concrete_data[string(base_coords_symbols[i])] for
        i in eachindex(base_coords_symbols)
      ]
    ...
    if f_ord == 0 && g_ord == 0
      ...
      for i in eachindex(gauge2s)
        push!(
          quotients,
          quotient(ideal([9 * g2s[i], gauge2s[i]]), ideal([2 * f2s[i], gauge2s[i]])),
        )
      end

      kod_type = if all(is_radical, quotients)
        "Non-split I_$d_ord"
      else
        "Split I_$d_ord"
      end
    elseif d_ord == 4 && g_ord == 2 && f_ord >= 2
      ...
      for i in eachindex(gauge2s)
        push!(
          quotients, quotient(ideal([g2s[i]]), ideal([gauge2s[i]^2])) + ideal([gauge2s[i]])
        )
      end
      ...
      if d_ord % 2 == 0
        for i in eachindex(gauge2s)
          push!(
            quotients,
            quotient(
              ideal([4//81 * (d2s[i] * f2s[i]^2) / gauge2s[i]^(d_ord + 4), gauge2s[i]]),
              ideal([g2s[i]^2 / gauge2s[i]^6, gauge2s[i]]),
            ),
          )
        end
      else
        for i in eachindex(gauge2s)
          push!(
            quotients,
            quotient(
              ideal([2//729 * (d2s[i] * f2s[i]^3) / gauge2s[i]^(d_ord + 6), gauge2s[i]]),
              ideal([g2s[i]^3 / gauge2s[i]^9, gauge2s[i]]),
            ),
          )
        end
      end
    ...
    elseif d_ord == 8 && g_ord == 4 && f_ord >= 3
      ...
      for i in eachindex(gauge2s)
        push!(
          quotients, quotient(ideal([g2s[i]]), ideal([gauge2s[i]^4])) + ideal([gauge2s[i]])
        )
      end
...
function _construct_generic_sample(
  base_grading::Matrix{Int64}, base_vars::Vector{String}, d::Int
)
  base_space = family_of_spaces(
    polynomial_ring(QQ, base_vars; cached=false)[1], base_grading, d
  )
  ...
  ambient_space = family_of_spaces(
    coordinate_ring_ambient_space, ambient_space_grading, d + 2
  )
  return [coordinate_ring(base_space), base_space, ambient_space]
end

function _construct_generic_sample(
  base_grading::Matrix{Int64},
  base_vars::Vector{String},
  d::Int,
  ...
  coordinate_ring_ambient_space, _ = polynomial_ring(
    QQ, base_vars, coordinate_names(fiber_ambient_space); cached=false
  )
  ...
  D_block = hcat(
    [
      [
        Int(fiber_twist_divisor_classes[k, l]) for k in 1:nrows(fiber_twist_divisor_classes)
      ] for l in 1:ncols(fiber_twist_divisor_classes)
    ]...,
  )
  ambient_space_grading = [base_grading D_block; z_block w']
  ambient_space = family_of_spaces(
    coordinate_ring_ambient_space, ambient_space_grading, d + dim(fiber_ambient_space)
  )
