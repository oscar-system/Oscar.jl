...
```
"""
function put_over_concrete_base(m::AbstractFTheoryModel, concrete_data::Dict{String, <:Any}; completeness_check::Bool = true, rng::AbstractRNG = Random.default_rng())
  ...
  if is_empty(model_section_parametrization(m))
    ...
    if m isa WeierstrassModel
      new_model_secs["f"] = generic_section(anticanonical_bundle(concrete_data["base"])^4; rng)
      new_model_secs["g"] = generic_section(anticanonical_bundle(concrete_data["base"])^6; rng)
    else
      new_model_secs["a1"] = generic_section(anticanonical_bundle(concrete_data["base"]); rng)
      new_model_secs["a2"] = generic_section(anticanonical_bundle(concrete_data["base"])^2; rng)
      new_model_secs["a3"] = generic_section(anticanonical_bundle(concrete_data["base"])^3; rng)
      new_model_secs["a4"] = generic_section(anticanonical_bundle(concrete_data["base"])^4; rng)
      new_model_secs["a6"] = generic_section(anticanonical_bundle(concrete_data["base"])^6; rng)
    end
    ...
    if m isa WeierstrassModel
      ...
      if haskey(parametrization, "f")
        ...
        if !is_zero(new_sec)
          @req degree(new_sec) == degree(generic_section(anticanonical_bundle(concrete_data["base"])^4; rng)) "Degree mismatch"
        end
      ...
      if haskey(parametrization, "g")
        ...
        if !is_zero(new_sec)
          @req degree(new_sec) == degree(generic_section(anticanonical_bundle(concrete_data["base"])^6; rng)) "Degree mismatch"
        end
      ...
      if haskey(parametrization, "a1")
        ...
        if !is_zero(new_sec)
          @req degree(new_sec) == degree(generic_section(anticanonical_bundle(concrete_data["base"]); rng)) "Degree mismatch"
        end
      ...
      if haskey(parametrization, "a2")
        ...
        if !is_zero(new_sec)
          @req degree(new_sec) == degree(generic_section(anticanonical_bundle(concrete_data["base"])^2; rng)) "Degree mismatch"
        end
      ...
      if haskey(parametrization, "a3")
        ...
        if !is_zero(new_sec)
          @req degree(new_sec) == degree(generic_section(anticanonical_bundle(concrete_data["base"])^3; rng)) "Degree mismatch"
        end
