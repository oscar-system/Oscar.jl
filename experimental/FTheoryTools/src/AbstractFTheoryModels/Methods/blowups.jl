...
function resolve(m::AbstractFTheoryModel, resolution_index::Int)
  ...
  if has_attribute(m, :arxiv_id) && resolution_index == 1 && arxiv_id(m) == "1511.03209"
    ...
    # Additional blowup 1: Additional blowup 1: Ambient space has the following
    # rays x: [0,0,0,-3,1] y: [0,0,0,2,-1] z: [0,0,0,0,1] We add the ray m1:
    # (0,0,0,1,0). This looks like y + z = 2 * m1. So naively, I think of this
    # as blowing up y^2 = z^2 = 0 and introducing the variable m1. For the
    # strict transform, we thus do y^2 -> y^2 * m1 z^2 -> z^2 * m1 y * z -> y *
    # z * m1 `let`: the three blow-ups below reuse the same names, and a
    # variable captured while being assigned in more than one of them would be
    ...
    model_bl = let
      ...
      set_attribute!(new_model, :partially_resolved, true)
      new_model
    end
    # Additional blowup 2:
    # Additional blowup 2:
    ...
    # x^3 -> x^3 * m2^2
    # z^3 -> z^3 * m2
    # `let`: see the first blow-up above
    model_bl2 = let
