...
_is_nontrivial(id::MPolyIdeal{T}, irr::MPolyIdeal{T}) where {T<:MPolyRingElem} =
  !is_one(id) && !is_one(saturation(id, irr))
...
function analyze_fibers(
  ...
  base_to_ambient_ring_map = hom(
    base_coords, ambient_coords, gens(ambient_coords)[1:(end - 3)]
  )

  # Resolved model
  # NOTE: _blowup_global_sequence was removed on September 4, 2025. This entire function needs to be rewritten!
  strict_transform, exceptionals, crepant, res_irr, res_sri, res_lin, res_S, res_S_gens, res_ring_map = _blowup_global_sequence(
    hypersurface_ideal, centers, irr, sri, lin
  )
  if !crepant
    @warn "The given sequence of blowups is not crepant"
  end

  loci_fiber_intersections = Tuple{
    MPolyIdeal{QQMPolyRingElem},
    Vector{Tuple{Tuple{Int64,Int64},Vector{MPolyIdeal{QQMPolyRingElem}}}},
  }[]
  for locus in interesting_singular_loci
    ...
    potential_components = map(
      last, primary_decomposition(strict_transform + res_ring_map(ungraded_locus))
    )

    # Filter out the trivial loci among the potential components
    components = filter(
      component -> _is_nontrivial(component, res_irr), potential_components
    )
    ...
    for i in 1:(length(components) - 1)
      for j in (i + 1):length(components)
        intersection = filter(
          candidate_locus -> _is_nontrivial(candidate_locus, res_irr),
          map(last, primary_decomposition(components[i] + components[j])),
        )
