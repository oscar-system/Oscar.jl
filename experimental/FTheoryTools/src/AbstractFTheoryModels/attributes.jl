...
function chern_class(m::AbstractFTheoryModel, k::Int; check::Bool = true)
  ...
  if !has_attribute(m, :chern_classes)
    cs = Vector{Any}(nothing, dim(ambient_space(m)))
  ...
  cs = get_attribute(m, :chern_classes)
  if cs[k+1] != nothing
    return cs[k+1]
  end
  ...
  set_attribute!(m, :chern_classes, cs)
  return get_attribute(m, :chern_classes)[k+1]
end
...
julia> h = chern_classes(qsm_model, 4; check = false);
