...
function sophisticated_intersection_product(
  v::NormalToricVariety,
  indices::NTuple{4,Int64},
  hypersurface_equation::MPolyDecRingElem{QQFieldElem,QQMPolyRingElem},
  ...
  if length(gs_reduced) == 1 && length(remaining_vars) == 2
    ...
    if length(mons_list) == 1
      ...
      if number_of_zeros == length(list_of_exps) - 1
        if gs_reduced[1] == remaining_vars[1] * remaining_vars[2]
          if (mons_list[1] == remaining_vars[1] && reduced_scaling_relations[1, 2] != 0) ||
            (mons_list[1] == remaining_vars[2] && reduced_scaling_relations[1, 1] != 0)
  ...
  if length(gs_reduced) == 1 && length(remaining_vars) == 2
    if gs_reduced[1] == remaining_vars[1] * remaining_vars[2]
      mons_list = collect(monomials(pt_reduced))
      if Set(mons_list) == Set([
        remaining_vars[1] * remaining_vars[2], remaining_vars[1]^2, remaining_vars[2]^2
      ])
        if reduced_scaling_relations[1, 1] != 0 || reduced_scaling_relations[1, 2] != 0
          inter_dict[indices] = ZZ(2)
          return ZZ(2)
...
function intersection_from_equivalent_cycle(
  v::NormalToricVariety,
  indices::NTuple{4,Int64},
  hypersurface_equation::MPolyDecRingElem{QQFieldElem,QQMPolyRingElem},
  inter_dict::Dict{NTuple{4,Int64},ZZRingElem},
  ...
  for k in 1:length(tuple_list)
    if !is_zero(coeffs_list[k])
      intersect_numb +=
        coeffs_list[k] * sophisticated_intersection_product(
          v, tuple_list[k], hypersurface_equation, inter_dict, s_inter_dict, data; rng
        )
    end
  end
...
function _reduce_hypersurface_equation(
  v::NormalToricVariety, p_hyper::MPolyRingElem, indices::NTuple{4,Int64}, data::NamedTuple
)
  ...
  prepared_scaling_relations = zero_matrix(
    ZZ, length(data.scalings[1]), length(set_to_one_list) + length(remaining_vars_pos)
  )
  for k in 1:(length(set_to_one_list) + length(remaining_vars_pos))
    col = if k <= length(set_to_one_list)
      data.scalings[set_to_one_list[k]]
    else
      data.scalings[remaining_vars_pos[k - length(set_to_one_list)]]
    end
  ...
  reduced_scaling_relations = prepared_scaling_relations[
    (length(set_to_one_list) + 1):nrows(prepared_scaling_relations),
    (length(set_to_one_list) + 1):ncols(prepared_scaling_relations),
  ]
...
function _rationally_equivalent_cycle(
  v::NormalToricVariety,
  indices::NTuple{4,Int64},
  data::NamedTuple;
  rng::AbstractRNG=Random.default_rng(),
  ...
  power_variable = if isempty(positions)
    indices[rand(rng, 1:length(indices))]
  else
    indices[rand(rng, positions)]
  end
  ...
  simpler_matrix = matrix(
    QQ, data.linear_relations[[other_variables..., power_variable], :]
  )
  ...
  employed_relation =
    -sum((data.linear_relations[:, k] .* (A[k, 1] + A_offset[k, 1])) for k in 1:nrows(A))
