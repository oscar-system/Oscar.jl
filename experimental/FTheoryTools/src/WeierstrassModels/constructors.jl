...
function weierstrass_model(
  ...
  return weierstrass_model(
    base, Dict("f" => f, "g" => g), Dict{String,MPolyRingElem}(); completeness_check
  )
end

function weierstrass_model(base::NormalToricVariety,
  explicit_model_sections::Dict{
    String,<:Union{MPolyRingElem,MPolyDecRingElem{QQFieldElem,QQMPolyRingElem}}
  },
  ...
  # construct the model
  pw = _weierstrass_polynomial(
    explicit_model_sections["f"],
    explicit_model_sections["g"],
    coordinate_ring(ambient_space),
  )
...
weierstrass_model_over_projective_space(d::Int; rng::AbstractRNG=Random.default_rng()) =
  weierstrass_model(
    projective_space(NormalToricVariety, d); completeness_check=false, rng=rng
  )
...
weierstrass_model_over_hirzebruch_surface(r::Int; rng::AbstractRNG=Random.default_rng()) =
  weierstrass_model(
    hirzebruch_surface(NormalToricVariety, r); completeness_check=false, rng=rng
  )
...
weierstrass_model_over_del_pezzo_surface(b::Int; rng::AbstractRNG=Random.default_rng()) =
  weierstrass_model(
    del_pezzo_surface(NormalToricVariety, b); completeness_check=false, rng=rng
  )
...
function weierstrass_model(
  ...
  @req (
    (parent(weierstrass_f) == auxiliary_base_ring) &&
    (parent(weierstrass_g) == auxiliary_base_ring)
  ) "All Weierstrass sections must reside in the provided auxiliary base ring"
  ...
  (S, auxiliary_base_space, auxiliary_ambient_space) = _construct_generic_sample(
    auxiliary_base_grading, gens_base_names, d
  )
