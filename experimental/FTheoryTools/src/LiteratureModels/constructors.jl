...
function literature_model(;
  ...
  return _literature_model(
    ...
    defining_classes=defining_classes,
...
function _literature_model(
        ...
        x -> _nested_string_map(
          s -> replace(
            ...
            r"\(([^(),]+)\)" =>
              dim -> string(
                "(",
                Oscar.eval_poly(string.(match(r"\(([^(),]+)\)", dim).captures[1]), ZZ),
                ")",
              ),
          ),
          x,
  ...
  if !isnothing(base_space) && dim(base_space) > 0
    ...
    for (key, value) in defining_classes_provided
      if (
        value isa ToricDivisor || value isa ToricDivisorClass || value isa ToricLineBundle
      ) == false
        error(
          "Construction of literature models over concrete bases currently requires defining classes (and model sections) to be provided as toric divisor (classes) or line bundles"
        )
      end
    end
    @req all(
      k -> haskey(defining_classes_provided, k),
      model_dict["model_data"]["defining_classes"],
    ) "Not all defining classes are specified"
    ...
    # Construct the model
    model = _construct_literature_model_over_concrete_base(
      model_dict, base_space, defining_classes_provided, completeness_check, rng
    )
...
function _find_model(
  doi::String, arxiv_id::String, version::String, equation::String, type::String
)
...
function _process_candidates(candidate_files::Vector{String})
  ...
  @req(length(candidate_files) == 1,
    begin
      ...
      equations = map(
        d -> get(d["arxiv_data"]["model_location"], "equation", nothing), dicts
      )
      types = map(d -> get(d["model_descriptors"], "type", nothing), dicts)
      strings = [
        "doi: $(dois[i]), arxiv_id: $(ids[i]), version: $(versions[i]), equation: $(equations[i]), type: $(types[i])"
...
function _construct_literature_model_over_concrete_base(
  ...
  cfs_mat = matrix(
    ...
    transpose(
      hcat(
        [
          [eval_poly(weight, ZZ) for weight in vec] for vec in
  ...
  cl_of_secs = Dict(
    tune_sec_names[k] =>
      sum(cfs[l, k] * Kbar_and_defng_cls_as_divisor_classes[l] for l in 1:nrows(cfs)) for
    k in 1:length(tune_sec_names)
  )
  ...
  for (key, value) in cl_of_secs
    ...
    if model_dict["arxiv_data"]["id"] == "1109.3454" && key == "w" && dim(base_space) == 3
      if torsion_free_rank(class_group_with_map(base_space)[1]) == 1 &&
        degree(toric_line_bundle(cl_of_secs["w"])) == 1
  ...
  map = hom(
    auxiliary_ring, coordinate_ring(base_space), [model_sections[k] for k in tune_sec_names]
  )
  ...
  elseif model_dict["model_descriptors"]["type"] == "hypersurface"
    ...
    max_cones = IncidenceMatrix([
      [a for a in b] for b in model_dict["model_data"]["fiber_ambient_space_max_cones"]
    ])
    fas = normal_toric_variety(max_cones, rays; non_redundant=true)
    fiber_amb_coordinates =
      string.(
        model_dict["model_data"]["fiber_ambient_space_coordinates"]
      )
    set_coordinate_names(fas, fiber_amb_coordinates)

    # Extract the base divisor classes of the fiber coordinates
    fiber_twist_matrix = transpose(
      matrix(ZZ, (hcat(model_dict["model_data"]["fiber_twist_matrix"]...)))
    )
    ...
    fiber_twist_divisor_classes = [
      sum([
        fiber_twist_matrix[l, k] * Kbar_and_defng_cls_as_divisor_classes[l] for
        l in 1:nrows(fiber_twist_matrix)
      ]) for k in 1:ncols(fiber_twist_matrix)
    ]

    # Compute the hypersurface equation and its parametrization
    auxiliary_ambient_ring, _ = polynomial_ring(
      QQ, vcat(tune_sec_names, fiber_amb_coordinates); cached=false
    )
    parametrized_hypersurface_equation = eval_poly(
      model_dict["model_data"]["hypersurface_equation"], auxiliary_ambient_ring
    )
    base_coordinates = string.(gens(coordinate_ring(base_space)))
    auxiliary_ambient_ring2, _ = polynomial_ring(
      QQ, vcat(base_coordinates, fiber_amb_coordinates); cached=false
    )
    images1 = [
      eval_poly(string(model_sections[k]), auxiliary_ambient_ring2) for k in tune_sec_names
    ]
...
function _construct_literature_model_over_arbitrary_base(
  ...
  if model_dict["model_descriptors"]["type"] == "tate"
    ...
    model = global_tate_model(
      auxiliary_base_ring, auxiliary_base_grading, base_dim, [a1, a2, a3, a4, a6]
    )
  ...
  elseif model_dict["model_descriptors"]["type"] == "hypersurface"
    ...
    ambient_ring, _ = polynomial_ring(
      QQ, vcat(auxiliary_base_vars, fiber_amb_coordinates); cached=false
    )
    p = eval_poly(model_dict["model_data"]["hypersurface_equation"], ambient_ring)

    # Create the model
    model = hypersurface_model(
      auxiliary_base_vars, auxiliary_base_grading, base_dim, fas, fiber_twist_matrix, p
    )
...
function _set_all_attributes(
  ...
  set_attribute!(
    model, :model_description => model_dict["model_descriptors"]["description"]
  )

  set_attribute!(model, :paper_authors => string.(model_dict["paper_metadata"]["authors"]))
  set_attribute!(
    model, :paper_buzzwords => string.(model_dict["paper_metadata"]["buzzwords"])
  )
  ...
  set_attribute!(
    model,
    :arxiv_model_equation_number => model_dict["arxiv_data"]["model_location"]["equation"],
  )
  set_attribute!(
    model, :arxiv_model_page => model_dict["arxiv_data"]["model_location"]["page"]
  )
  set_attribute!(
    model, :arxiv_model_section => model_dict["arxiv_data"]["model_location"]["section"]
  )
  ...
  if haskey(model_dict["journal_data"], "report_numbers")
    set_attribute!(
      model,
      :journal_report_numbers => string.(model_dict["journal_data"]["report_numbers"]),
    )
  end
  ...
  set_attribute!(
    ...
    :journal_model_equation_number =>
      model_dict["journal_data"]["model_location"]["equation"],
  )
  set_attribute!(
    model, :journal_model_page => model_dict["journal_data"]["model_location"]["page"]
  )
  set_attribute!(
    model, :journal_model_section => model_dict["journal_data"]["model_location"]["section"]
  )
  ...
  if haskey(model_dict, "birational_models")
    set_attribute!(
      ...
      :birational_literature_models =>
        [str[6:(end - 5)] for str in model_dict["birational_models"]],
    )
  end
  ...
  if haskey(model_dict, "associated_models")
    set_attribute!(
      ...
      :associated_literature_models =>
        [str[6:(end - 5)] for str in model_dict["associated_models"]],
    )
  end
  ...
  if haskey(model_dict, "birational_models")
    for m in model_dict["birational_models"]
      ...
      model_type = birational_data["model_descriptors"]["type"]
      model_index = parse(Int, birational_data["model_index"])
  ...
  if haskey(model_dict["model_data"], "resolutions")
    set_attribute!(
      model,
      :resolutions => [
        ([string.(c) for c in r[1]], string.(r[2])) for
        r in model_dict["model_data"]["resolutions"]
      ],
    )
  end
  ...
  if haskey(model_dict["model_data"], "weighted_resolutions")
    set_attribute!(
      ...
      :weighted_resolutions => [
        ([(string.(c[1]), Int.(c[2])) for c in r[1]], string.(r[2])) for
        r in model_dict["model_data"]["weighted_resolutions"]
      ],
    )
  end
  ...
  if haskey(model_dict["model_descriptors"], "global_gauge_group_quotients")
    set_attribute!(
      model,
      :global_gauge_group_quotient => map(
        k -> string.(k), model_dict["model_descriptors"]["global_gauge_group_quotients"]
      ),
    )
  end
  ...
  if base_space(model) isa NormalToricVariety &&
    ...
    if haskey(model_data, "zero_section_class")
      ...
      set_attribute!(
        model,
        :zero_section_class => cohomology_class(divs[index]; completeness_check=false),
      )
    end
    ...
    if haskey(model_data, "exceptional_classes")
      ...
      set_attribute!(
        model, :exceptional_divisor_indices => exceptional_divisor_indices::Vector{Int}
      )
      set_attribute!(
        model,
        :exceptional_classes => [
  ...
  if haskey(
    ...
    set_attribute!(
      model, :classes_of_tunable_sections_in_basis_of_Kbar_and_defining_classes, D
    )
  end
  ...
  if haskey(model_dict["model_descriptors"], "gauge_algebra")
    ...
    set_attribute!(
      model,
      :gauge_algebra =>
        direct_sum(C, LieAlgebra{elem_type(C)}[_construct(g) for g in algebras]),
    )
  end
end
