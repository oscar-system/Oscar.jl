...
@attr WeierstrassModel function weierstrass_model(t::GlobalTateModel)
  ...
  parametrization = tunable_section_parametrization(t)
  param_keys = collect(keys(parametrization))
  new_tunable_section_parametrization = Dict{String, MPolyRingElem}()
  if length(param_keys) > 0
    ...
    new_tunable_section_parametrization = Dict("f" => param_f, "g" => param_g)

  end
  
  # Compute Weierstrass model
  model = WeierstrassModel(new_explicit_model_sections, new_tunable_section_parametrization, pw, base_space(t), ambient_space(t))
