...
function global_tate_model(base::NormalToricVariety,
  ...
  model = GlobalTateModel(explicit_model_sections, tunable_section_parametrization, pt, base, ambient_space)
...
function global_tate_model(auxiliary_base_ring::MPolyRing, auxiliary_base_grading::Matrix{Int64}, d::Int, ais::Vector{T}) where {T<:MPolyRingElem}
  ...
  model = GlobalTateModel(explicit_model_sections, tunable_section_parametrization, pt, auxiliary_base_space, auxiliary_ambient_space)
  set_attribute!(model, :partially_resolved, false)
  return model
end
