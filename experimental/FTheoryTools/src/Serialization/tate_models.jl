...
function save_object(s::SerializerState, gtm::GlobalTateModel)
  ...
  save_data_dict(s) do
    for (data, key) in [
        ...
        (tunable_section_parametrization(gtm), :tunable_section_parametrization),
    ...
    if has_attribute(gtm, :well_quantized_ambient_space_models_of_g4_fluxes)
      res = well_quantized_ambient_space_models_of_g4_fluxes(gtm, check = false)
    ...
    if has_attribute(gtm, :well_quantized_and_vertical_ambient_space_models_of_g4_fluxes)
      res = well_quantized_and_vertical_ambient_space_models_of_g4_fluxes(gtm, check = false)
    ...
    if has_attribute(gtm, :well_quantized_and_vertical_and_no_non_abelian_gauge_group_breaking_ambient_space_models_of_g4_fluxes)
      res = well_quantized_and_vertical_and_no_non_abelian_gauge_group_breaking_ambient_space_models_of_g4_fluxes(gtm, check = false)
...
function load_object(s::DeserializerState, ::Type{<: GlobalTateModel}, params::Tuple{NormalToricVariety, NormalToricVariety, MPolyDecRing})
  ...
  tunable_section_parametrization = haskey(s, :tunable_section_parametrization) ? load_typed_object(s, :tunable_section_parametrization) : Dict{String, MPolyRingElem}()
  defining_classes = haskey(s, :defining_classes) ? load_typed_object(s, :defining_classes) : Dict{String, ToricDivisorClass}()
  model = GlobalTateModel(explicit_model_sections, tunable_section_parametrization, pt, base_space, amb_space)
  ...
  # as this saves memory. If known, recompute the corresponding cohomology classes and set the attribute.
