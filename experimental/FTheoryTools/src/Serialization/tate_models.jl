...
function save_object(s::SerializerState, gtm::GlobalTateModel)
  ...
  save_data_dict(s) do
    ...
    if has_attribute(gtm, :_ambient_space_divisors_to_be_considered)
      attrs_dict[:_ambient_space_divisors_to_be_considered] = _ambient_space_divisors_to_be_considered(gtm)
    end

    # Do we know ambient space models for g4-fluxes?
    if has_attribute(gtm, :ambient_space_models_of_g4_fluxes)
      ambient_space_flux_candidates_basis = ambient_space_models_of_g4_fluxes(gtm)
      g4_flux_tuple_list = Vector{Tuple{Int64, Int64}}()
      for k in 1:length(ambient_space_flux_candidates_basis)
        ...
        push!(g4_flux_tuple_list, Tuple(sort(idxs)))
      end
      attrs_dict[:g4_flux_tuple_list] = g4_flux_tuple_list
    end
...
function load_object(s::DeserializerState, ::Type{<: GlobalTateModel}, params::Tuple{NormalToricVariety, NormalToricVariety, MPolyDecRing})
  ...
  if haskey(attrs_data, :_ambient_space_divisors_to_be_considered)
    set_attribute!(model, :_ambient_space_divisors_to_be_considered, attrs_data[:_ambient_space_divisors_to_be_considered])
  end
