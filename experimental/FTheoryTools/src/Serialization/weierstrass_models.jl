...
function save_object(s::SerializerState, w::WeierstrassModel)
  ...
  save_data_dict(s) do
    ...
    if has_attribute(w, :_ambient_space_divisors_to_be_considered)
      attrs_dict[:_ambient_space_divisors_to_be_considered] = _ambient_space_divisors_to_be_considered(w)
    end

    # Do we know ambient space models for g4-fluxes?
    if has_attribute(w, :ambient_space_models_of_g4_fluxes)
      ambient_space_flux_candidates_basis = ambient_space_models_of_g4_fluxes(w)
