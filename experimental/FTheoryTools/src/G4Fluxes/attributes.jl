...
function _g4_flux_coordinate_computer(gf::G4Flux)
  ...
  if all(is_integer, up[1:ncols(gff_mat_int)])
    int_coeffs = matrix(ZZ, [[up[k] for k in 1:ncols(gff_mat_int)]])::ZZMatrix
    rat_coeffs = matrix(QQ, [[up[ncols(gff_mat_int) + k] for k in 1:ncols(gff_mat_rat)]])::QQMatrix
  ...
  int_coeffs = matrix(ZZ, [[good_up[k] for k in 1:ncols(gff_mat_int)]])::ZZMatrix
  rat_coeffs = matrix(QQ, [[good_up[ncols(gff_mat_int) + k] for k in 1:ncols(gff_mat_rat)]])::QQMatrix
  set_attribute!(gf, :int_combination, int_coeffs)
  set_attribute!(gf, :rat_combination, rat_coeffs)
  return get_attribute(gf, :int_combination)::ZZMatrix
end
