...
```
"""
@attr Dict{Tuple{Int64,Int64},Vector{Tuple{QQFieldElem,Tuple{Int64,Int64}}}} function converter_dict_h22_ambient(
  ...
  ignored_sets = Set([
    Tuple(sort(Vector{Int}(Polymake.row(mnf, i)))) for i in 1:Polymake.nrows(mnf)
  ])
  ...
  for k in 1:length(good_positions)
    for l in k:length(good_positions)
      my_tuple = (
        min(good_positions[k], good_positions[l]), max(good_positions[k], good_positions[l])
      )
      if !(my_tuple in ignored_sets)
  ...
  a1 = sort(
    unique(
      vcat(
        [[u[2] for u in k] for k in filter(x -> x != 0, collect(values(converter_dict)))]...
      ),
    ),
  )
  ...
  if length(remaining_relations) != 0
    ...
    for k in new_good_positions
      matching_keys = [
        key for (key, value) in dict_of_filtered_quadratic_elements if value == k
      ]
      if length(matching_keys) != 1
        println(
          "Verification failed: Expected exactly one key for value $k, but found $(length(matching_keys))"
        )
        return false, []
      end
        ...
        for (ikey, ivalue) in converter_dict
          ...
          if position_of_key !== nothing
            ...
            elseif length(new_tuple_list) == 0 && length(new_coeff_list) == 0
              converter_dict[ikey] = [
                (coeff_in_question * k[1], k[2]) for
                k in relation_to_be_applied if k[1] != 0
              ]
            elseif relation_to_be_applied == 0
              converter_dict[ikey] = [
                (new_coeff_list[a], new_tuple_list[a]) for a in 1:length(new_tuple_list)
              ]
            else
              for a in 1:length(relation_to_be_applied)
                if relation_to_be_applied[a][2] in new_tuple_list
                  position_of_tuple = findfirst(
                    k -> k == relation_to_be_applied[a][2], new_tuple_list
                  )
                  new_coeff_list[position_of_tuple] +=
                    coeff_in_question * relation_to_be_applied[a][1]
                else
              ...
              converter_dict[ikey] = [
                (new_coeff_list[a], new_tuple_list[a]) for
                a in 1:length(new_tuple_list) if new_coeff_list[a] != 0
              ]
            end
          end
...
```
"""
@attr Vector{Tuple{Int64,Int64}} function basis_of_h22_ambient_indices(
  m::AbstractFTheoryModel; completeness_check::Bool=true
)
  cdh22 = converter_dict_h22_ambient(m; completeness_check)
  return unique(
    vcat([[u[2] for u in k] for k in filter(x -> x != 0, collect(values(cdh22)))]...)
  )
end
...
```
"""
@attr Vector{CohomologyClass} function basis_of_h22_ambient(
  m::AbstractFTheoryModel; completeness_check::Bool=true
)
  ...
  return [
    cohomology_class(v, MPolyQuoRingElem(c_ds[mt[1]] * c_ds[mt[2]], S)) for
    mt in basis_indices
  ]
end
...
```
"""
@attr Vector{Tuple{Int64,Int64}} function gens_of_h22_hypersurface_indices(
  m::AbstractFTheoryModel; completeness_check::Bool=true
)
  ...
  for a in length(filtered_h22_basis_indices):-1:1
    ...
    for my_exps in sr_ideal_pos
      ...
      if (len_my_exps == 2 && inter_len == 1) || (len_my_exps == 3 && inter_len == 2)
        delete!(
          remaining_vars_list,
          my_exps[findfirst(idx -> !(idx in vanishing_vars_pos), my_exps)],
        )
      end
    end
...
```
"""
@attr Vector{CohomologyClass} function gens_of_h22_hypersurface(
  m::AbstractFTheoryModel; completeness_check::Bool=true
)
...
```
"""
@attr Dict{Tuple{Int64,Int64},Vector{Tuple{QQFieldElem,Tuple{Int64,Int64}}}} function converter_dict_h22_hypersurface(
  m::AbstractFTheoryModel; completeness_check::Bool=true
)
...
```
"""
@attr Vector{G4Flux} function chosen_g4_flux_gens(
  m::AbstractFTheoryModel; completeness_check::Bool=true
)
...
@attr Vector{Tuple{Int64,Int64}} function _ambient_space_divisor_pairs_to_be_considered(
...
@attr Vector{Tuple{Int64,Int64}} function _ambient_space_base_divisor_pairs_to_be_considered(
  m::AbstractFTheoryModel
)
  gS = gens(coordinate_ring(ambient_space(m)))
  mnf = Oscar._minimal_nonfaces(ambient_space(m))
