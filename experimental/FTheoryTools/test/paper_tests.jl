...
weier_f = 1//48*(-(28*x1*x2^5 + 169*x3^6)^2 + 24*(2*x1^3*(x2 + x3)^9 + 13*x1^2*x2^4*x3^6))
weier_g = 1//864*(216*x1^4*x2^8*x3^6 + (28*x1*x2^5 + 169*x3^6)^3 - 36*(28*x1*x2^5 + 169*x3^6)*(2*x1^3*(x2 + x3)^9 + 13*x1^2*x2^4*x3^6))
...
@testset "FTheoryToolsPaper Section 3.4" begin
  ...
  @test rank(grading_group(cox_ring(ambient_space(h)))) == rank(grading_group(cox_ring(ambient_space(t))))
end
...
@testset "FTheoryToolsPaper Section 3.5.2 Non-Toric Resolution Part 3" begin
  ...
  @test typeof(tate_ideal_sheaf(t2_res)) == Oscar.StrictTransformIdealSheaf{CoveredScheme{QQField}, AbsAffineScheme, Ideal, Map}
end
...
@testset "FTheoryToolsPaper Section 4.2 Part 1" begin
  ...
  @test global_gauge_group_quotient(foah11_B3)[1][1] == "diagonal_matrix(root_of_unity(C,3),3)"
end
