...
@testset "Attributes of Weierstrass models over concrete base spaces" begin
  ...
  @test parent(weierstrass_section_g(weierstrass_P3)) == coordinate_ring(base_space(weierstrass_P3))
  @test parent(weierstrass_polynomial(weierstrass_P3)) == coordinate_ring(ambient_space(weierstrass_P3))
  ...
  @test toric_variety(calabi_yau_hypersurface(weierstrass_P3)) == ambient_space(weierstrass_P3)
...
@testset "Error messages in Weierstrass models over concrete base spaces" begin
  @test_throws ArgumentError weierstrass_model(P3, section_f, section_g; completeness_check = false)
end
...
@testset "Saving and loading Weierstrass model over concrete base space" begin
  mktempdir() do path
    test_save_load_roundtrip(path, literature_model_P2) do loaded
      ...
      @test is_base_space_fully_specified(literature_model_P2) == is_base_space_fully_specified(loaded)
      @test explicit_model_sections(literature_model_P2) == explicit_model_sections(loaded)
      @test model_section_parametrization(literature_model_P2) == model_section_parametrization(loaded)
...
@testset "Saving and loading another Weierstrass model over concrete base space" begin
  mktempdir() do path
    test_save_load_roundtrip(path, another_weierstrass_P3) do loaded
      ...
      @test is_base_space_fully_specified(another_weierstrass_P3) == is_base_space_fully_specified(loaded)
      @test is_partially_resolved(another_weierstrass_P3) == is_partially_resolved(loaded)
      @test explicit_model_sections(another_weierstrass_P3) == explicit_model_sections(loaded)
      @test model_section_parametrization(another_weierstrass_P3) == model_section_parametrization(loaded)
...
weierstrass_generic = weierstrass_model(auxiliary_base_ring, auxiliary_base_grading, 3, f, g)

@testset "Attributes of Weierstrass models over generic base space" begin
  @test parent(weierstrass_section_f(weierstrass_generic)) == coordinate_ring(base_space(weierstrass_generic))
  @test parent(weierstrass_section_g(weierstrass_generic)) == coordinate_ring(base_space(weierstrass_generic))
  @test parent(weierstrass_polynomial(weierstrass_generic)) == coordinate_ring(ambient_space(weierstrass_generic))
...
@testset "Error messages in Weierstrass models over generic base space" begin
  @test_throws ArgumentError weierstrass_model(auxiliary_base_ring, auxiliary_base_grading, 3, f, section_f)
