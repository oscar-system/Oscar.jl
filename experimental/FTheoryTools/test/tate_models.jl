...
@testset "Attributes of global Tate models over concrete base space" begin
  ...
  @test is_base_space_fully_specified(tate_P3) == is_base_space_fully_specified(weierstrass_model(tate_P3))
...
@testset "Error messages in global Tate models over concrete base space" begin
  @test_throws ArgumentError global_tate_model(P3, [section_a2, section_a3, section_a4, section_a6]; completeness_check = false)
  @test_throws ArgumentError global_tate_model(P3, [section_a1, section_a2, section_a3, section_a4, section_a6]; completeness_check = false)
end
  ...
  @testset "Saving and loading global Tate model over concrete base space" begin
    mktempdir() do path
      test_save_load_roundtrip(path, literature_tate_P3) do loaded
        ...
        @test is_base_space_fully_specified(literature_tate_P3) == is_base_space_fully_specified(loaded)
  ...
  @testset "Saving and loading another global Tate model over concrete base space" begin
    mktempdir() do path
      test_save_load_roundtrip(path, another_tate_P3) do loaded
        ...
        @test is_base_space_fully_specified(another_tate_P3) == is_base_space_fully_specified(loaded)
        ...
        @test model_section_parametrization(another_tate_P3) == model_section_parametrization(loaded)
...
aux_ring_star0, (a1pp, a2pp, a3pp, a4pp, a6pp, vp, wp, mp) = QQ[:a1pp, :a2pp, :a3pp, :a4pp, :a6pp, :vp, :wp, :mp]
...
tate_generic_nonminimal = global_tate_model(
  ...
  3,
  [a1p * (v^2 + w), a2p * (v^2 + w)^2, a3p * (v^2 + w)^3, a4p * (v^2 + w)^4, a6p * (v^2 + w)^6],
)

@testset "Attributes of global Tate models over generic base space" begin
  @test parent(tate_section_a1(tate_generic_I5s)) == coordinate_ring(base_space(tate_generic_I5s))
  @test parent(tate_section_a2(tate_generic_I5s)) == coordinate_ring(base_space(tate_generic_I5s))
  @test parent(tate_section_a3(tate_generic_I5s)) == coordinate_ring(base_space(tate_generic_I5s))
  @test parent(tate_section_a4(tate_generic_I5s)) == coordinate_ring(base_space(tate_generic_I5s))
  @test parent(tate_section_a6(tate_generic_I5s)) == coordinate_ring(base_space(tate_generic_I5s))
  @test parent(tate_polynomial(tate_generic_I5s)) == coordinate_ring(ambient_space(tate_generic_I5s))
  @test parent(discriminant(tate_generic_I5s)) == coordinate_ring(base_space(tate_generic_I5s))
  ...
  @test is_base_space_fully_specified(tate_generic_I5s) == is_base_space_fully_specified(weierstrass_model(tate_generic_I5s))
end

@testset "Error messages in global Tate models over generic base space" begin
  @test_throws ArgumentError global_tate_model(aux_ring_tate, [1 2 3 4 6 0 0; 0 -1 -2 -3 -5 1 2], 3, [a1p])
  @test_throws ArgumentError global_tate_model(aux_ring_tate, [1 2 3 4 6 0 0; 0 -1 -2 -3 -5 1 2], 3, [a1p, a2p * v, a3p * v^2, a4p * v^3, section_a6])
  @test_throws ArgumentError global_tate_model(aux_ring_tate, [1 2 3 4 6 0 0; 0 -1 -2 -3 -5 1 2], -1, [a1p, a2p * v, a3p * v^2, a4p * v^3, a6p * v^5])
end

@testset "Singular loci of global Tate models over generic base space" begin
  @test [k[2:3] for k in singular_loci(tate_generic_I1; rng = our_rng)] == [((0, 0, 1), "I_1"), ((0, 0, 1), "I_1")]
  @test [k[2:3] for k in singular_loci(tate_generic_nonminimal; rng = our_rng)] == [((0, 0, 1), "I_1"), ((4, 6, 12), "Non-minimal")]
end
