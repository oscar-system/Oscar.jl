...
@testset "Advanced intersection theory and QSM-fluxes" begin
  for k in 1:5000
    qsm_model = try
      literature_model(; arxiv_id="1903.00009", model_parameters=Dict("k" => k), rng=our_rng)
    catch e
      continue
    end
    ...
    kbar_poly =
      polynomial(cohomology_class(anticanonical_bundle(ambient_space(qsm_model)))).f
    ...
    for (k, v) in sampled_dict
      desired_class = CohomologyClass(
        ambient_space(qsm_model),
        coho_R(gs[k[1]] * gs[k[2]] * gs[k[3]] * gs[k[4]] * kbar_poly),
        true,
      )
