...
@testset "Test defining data for literature Tate model over concrete base" begin
  ...
  @test is_base_space_fully_specified(t1) ==
    is_base_space_fully_specified(weierstrass_model(t1))
  @test is_smooth(ambient_space(t1)) == false
  @test toric_variety(calabi_yau_hypersurface(t1)) == ambient_space(t1)
  @test sum(D["a43"] .* [Kbar, toric_divisor_class(w)]) ==
    classes_of_model_sections(t1)["a43"]
end
...
# Toric resolution preserves base data and lazily rebuilds ambient divisor classes.
zero_section_index_t1 = findfirst(==(:z), symbols(coordinate_ring(ambient_space(t1))))::Int
...
@testset "Test resolving literature Tate model over concrete base" begin
  @test !has_attribute(t2, :zero_section_class)
  @test !has_attribute(t2, :exceptional_classes)
  ...
  @test classes_of_tunable_sections_in_basis_of_Kbar_and_defining_classes(t2) === D
  @test classes_of_model_sections(t2) === classes_of_model_sections(t1)
  @test discriminant(t2) === discriminant(t1)
  @test singular_loci(t2) === singular_loci(t1)
  @test !has_attribute(t2, :weierstrass_model)
  @test !has_attribute(t2, :generating_sections)
  @test has_attribute(t2, :zero_section_class)
  ...
  @test length(exceptional_divisor_indices(t2)) ==
    length(exceptional_divisor_indices(t1)) + 5
...
t3 = literature_model(; arxiv_id="1109.3454", equation="3.1", rng=our_rng)
...
@testset "Basic tests for literature Tate model over arbitrary base" begin
  ...
  @test is_base_space_fully_specified(t3) ==
    is_base_space_fully_specified(weierstrass_model(t3))
end
...
@testset "Test error messages for literature Tate model over arbitrary base" begin
  ...
  @test_throws ArgumentError literature_model(
    arxiv_id="1212.2949", equation="3.2", rng=our_rng
  )
end

t4 = literature_model(;
  arxiv_id="1212.2949", equation="3.2", model_parameters=Dict("k" => 5), rng=our_rng
)
t5 = literature_model(;
  arxiv_id="1212.2949", equation="3.42", model_parameters=Dict("k" => 4), rng=our_rng
)
t6 = literature_model(;
  arxiv_id="1212.2949", equation="4.1", model_parameters=Dict("k" => 3), rng=our_rng
)
t7 = literature_model(;
  arxiv_id="1212.2949", equation="4.23", model_parameters=Dict("k" => 4), rng=our_rng
)
t8 = literature_model(; arxiv_id="1212.2949", equation="5.1", rng=our_rng)
t9 = literature_model(; arxiv_id="1212.2949", equation="5.7", rng=our_rng)
t10 = literature_model(; arxiv_id="1212.2949", equation="5.13", rng=our_rng)
...
t4b = literature_model(;
  arxiv_id="1212.2949",
  equation="3.2",
  model_parameters=Dict("k" => 2),
  base_space=B3,
  model_sections=Dict("ζ0" => toric_line_bundle(ζ0)),
  rng=our_rng,
)
t5b = literature_model(;
  arxiv_id="1212.2949",
  equation="3.42",
  model_parameters=Dict("k" => 2),
  base_space=B3,
  model_sections=Dict("ζ0" => toric_divisor_class(ζ0)),
  rng=our_rng,
)
t6b = literature_model(;
  arxiv_id="1212.2949",
  equation="4.1",
  model_parameters=Dict("k" => 2),
  base_space=B3,
  defining_classes=Dict("ζ0" => toric_line_bundle(ζ0)),
  rng=our_rng,
)
t7b = literature_model(;
  arxiv_id="1212.2949",
  equation="4.23",
  model_parameters=Dict("k" => 2),
  base_space=B3,
  defining_classes=Dict("ζ0" => toric_divisor_class(ζ0)),
  rng=our_rng,
)
t8b = literature_model(;
  arxiv_id="1212.2949",
  equation="5.1",
  base_space=B3,
  defining_classes=Dict("ζ0" => ζ0),
  rng=our_rng,
)
t9b = literature_model(;
  arxiv_id="1212.2949",
  equation="5.7",
  base_space=B3,
  defining_classes=Dict("ζ0" => ζ0),
...
w2 = literature_model(;
  arxiv_id="1208.2695", equation="B.19", completeness_check=false, rng=our_rng
)
...
w3 = literature_model(; arxiv_id="1507.05954", equation="A.1", rng=our_rng)
...
w5 = literature_model(;
  ...
  completeness_check=false,
  base_space=B2,
  defining_classes=Dict("s8" => b2, "a1" => b, "a2" => b, "a3" => b),
  rng=our_rng,
)
...
b = torusinvariant_prime_divisors(B2)[1]
w6 = literature_model(
  3; base_space=B2, defining_classes=Dict("b" => b), completeness_check=false, rng=our_rng
)
...
@testset "Test error messages for literature Weierstrass model over arbitrary base" begin
  @test_throws ArgumentError literature_model(-1, rng=our_rng)
  @test_throws ArgumentError literature_model(0, rng=our_rng)
  @test_throws ArgumentError literature_model(205, rng=our_rng)
end

h = literature_model(; arxiv_id="1507.05954", equation="3.4", rng=our_rng)
...
foah1 = literature_model(;
  arxiv_id="1408.4808", equation="3.4", type="hypersurface", rng=our_rng
)
foah2 = literature_model(;
  arxiv_id="1408.4808", equation="3.12", type="hypersurface", rng=our_rng
)
foah3 = literature_model(;
  arxiv_id="1408.4808", equation="3.54", type="hypersurface", rng=our_rng
)
foah4 = literature_model(;
  arxiv_id="1408.4808", equation="3.17", type="hypersurface", rng=our_rng
)
foah5 = literature_model(;
  arxiv_id="1408.4808", equation="3.73", type="hypersurface", rng=our_rng
)
foah6 = literature_model(;
  arxiv_id="1408.4808", equation="3.82", type="hypersurface", rng=our_rng
)
foah7 = literature_model(;
  arxiv_id="1408.4808", equation="3.96", type="hypersurface", rng=our_rng
)
foah8 = literature_model(;
  arxiv_id="1408.4808", equation="3.106", type="hypersurface", rng=our_rng
)
foah9 = literature_model(;
  arxiv_id="1408.4808", equation="3.118", type="hypersurface", rng=our_rng
)
foah10 = literature_model(;
  arxiv_id="1408.4808", equation="3.130", type="hypersurface", rng=our_rng
)
foah11 = literature_model(;
  arxiv_id="1408.4808", equation="3.142", type="hypersurface", rng=our_rng
)
foah12 = literature_model(;
  arxiv_id="1408.4808", equation="3.155", type="hypersurface", rng=our_rng
)
foah13 = literature_model(;
  arxiv_id="1408.4808", equation="3.181", type="hypersurface", rng=our_rng
)
foah14 = literature_model(;
  arxiv_id="1408.4808", equation="3.168", type="hypersurface", rng=our_rng
)
foah15 = literature_model(;
  arxiv_id="1408.4808", equation="3.190", type="hypersurface", rng=our_rng
)
foah16 = literature_model(;
  arxiv_id="1408.4808", equation="3.203", type="hypersurface", rng=our_rng
)
...
@testset "Test hypersurface form of models in F-theory on all toric hypersurfaces, defined over arbitrary base" begin
  ...
  @test model_description(foah1) ==
    "F-theory hypersurface model with fiber ambient space F_1"
  @test model_description(foah2) ==
    "F-theory hypersurface model with fiber ambient space F_2"
  @test model_description(foah3) ==
    "F-theory hypersurface model with fiber ambient space F_3"
  @test model_description(foah4) ==
    "F-theory hypersurface model with fiber ambient space F_4"
  @test model_description(foah5) ==
    "F-theory hypersurface model with fiber ambient space F_5"
  @test model_description(foah6) ==
    "F-theory hypersurface model with fiber ambient space F_6"
  @test model_description(foah7) ==
    "F-theory hypersurface model with fiber ambient space F_7"
  @test model_description(foah8) ==
    "F-theory hypersurface model with fiber ambient space F_8"
  @test model_description(foah9) ==
    "F-theory hypersurface model with fiber ambient space F_9"
  @test model_description(foah10) ==
    "F-theory hypersurface model with fiber ambient space F_10"
  @test model_description(foah11) ==
    "F-theory hypersurface model with fiber ambient space F_11"
  @test model_description(foah12) ==
    "F-theory hypersurface model with fiber ambient space F_12"
  @test model_description(foah13) ==
    "F-theory hypersurface model with fiber ambient space F_13"
  @test model_description(foah14) ==
    "F-theory hypersurface model with fiber ambient space F_14"
  @test model_description(foah15) ==
    "F-theory hypersurface model with fiber ambient space F_15"
  @test model_description(foah16) ==
    "F-theory hypersurface model with fiber ambient space F_16"
...
foah2_B3 = literature_model(;
  ...
  type="hypersurface",
  base_space=B3,
  defining_classes=Dict("b7" => Kbar, "b9" => Kbar),
  completeness_check=false,
  rng=our_rng,
)
foah3_B3 = literature_model(;
  arxiv_id="1408.4808",
  equation="3.54",
  type="hypersurface",
  base_space=B3,
...
@testset "Test hypersurface form of models in F-theory on all toric hypersurfaces, defined over concrete base" begin
  ...
  @test model_description(foah1_B3) ==
    "F-theory hypersurface model with fiber ambient space F_1"
  @test model_description(foah2_B3) ==
    "F-theory hypersurface model with fiber ambient space F_2"
  @test model_description(foah3_B3) ==
    "F-theory hypersurface model with fiber ambient space F_3"
  @test model_description(foah4_B3) ==
    "F-theory hypersurface model with fiber ambient space F_4"
  @test model_description(foah5_B3) ==
    "F-theory hypersurface model with fiber ambient space F_5"
  @test model_description(foah6_B3) ==
    "F-theory hypersurface model with fiber ambient space F_6"
  @test model_description(foah7_B3) ==
    "F-theory hypersurface model with fiber ambient space F_7"
  @test model_description(foah8_B3) ==
    "F-theory hypersurface model with fiber ambient space F_8"
  @test model_description(foah9_B3) ==
    "F-theory hypersurface model with fiber ambient space F_9"
  @test model_description(foah10_B3) ==
    "F-theory hypersurface model with fiber ambient space F_10"
  @test model_description(foah11_B3) ==
    "F-theory hypersurface model with fiber ambient space F_11"
  @test model_description(foah12_B3) ==
    "F-theory hypersurface model with fiber ambient space F_12"
  @test model_description(foah13_B3) ==
    "F-theory hypersurface model with fiber ambient space F_13"
  @test model_description(foah14_B3) ==
    "F-theory hypersurface model with fiber ambient space F_14"
  @test model_description(foah15_B3) ==
    "F-theory hypersurface model with fiber ambient space F_15"
  @test model_description(foah16_B3) ==
    "F-theory hypersurface model with fiber ambient space F_16"
  @test parent(explicit_model_sections(foah1_B3)["s7"]) ==
    coordinate_ring(base_space(foah1_B3))
  @test parent(explicit_model_sections(foah2_B3)["b7"]) ==
    coordinate_ring(base_space(foah2_B3))
  @test parent(explicit_model_sections(foah3_B3)["s7"]) ==
    coordinate_ring(base_space(foah3_B3))
  @test parent(explicit_model_sections(foah4_B3)["d4"]) ==
    coordinate_ring(base_space(foah4_B3))
  @test parent(explicit_model_sections(foah5_B3)["s7"]) ==
    coordinate_ring(base_space(foah5_B3))
  @test parent(explicit_model_sections(foah6_B3)["s7"]) ==
    coordinate_ring(base_space(foah6_B3))
  @test parent(explicit_model_sections(foah7_B3)["s7"]) ==
    coordinate_ring(base_space(foah7_B3))
  @test parent(explicit_model_sections(foah8_B3)["s7"]) ==
    coordinate_ring(base_space(foah8_B3))
  @test parent(explicit_model_sections(foah9_B3)["s7"]) ==
    coordinate_ring(base_space(foah9_B3))
  @test parent(explicit_model_sections(foah10_B3)["s5"]) ==
    coordinate_ring(base_space(foah10_B3))
  @test parent(explicit_model_sections(foah11_B3)["s5"]) ==
    coordinate_ring(base_space(foah11_B3))
  @test parent(explicit_model_sections(foah12_B3)["s7"]) ==
    coordinate_ring(base_space(foah12_B3))
  @test parent(explicit_model_sections(foah13_B3)["s1"]) ==
    coordinate_ring(base_space(foah13_B3))
  @test parent(explicit_model_sections(foah14_B3)["s7"]) ==
    coordinate_ring(base_space(foah14_B3))
  @test parent(explicit_model_sections(foah15_B3)["s7"]) ==
    coordinate_ring(base_space(foah15_B3))
  @test parent(explicit_model_sections(foah16_B3)["s7"]) ==
    coordinate_ring(base_space(foah16_B3))
  @test string(hypersurface_equation_parametrization(foah1_B3)) ==
...
foah1_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.4", type="weierstrass", rng=our_rng
)
foah2_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.12", type="weierstrass", rng=our_rng
)
foah3_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.54", type="weierstrass", rng=our_rng
)
foah4_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.17", type="weierstrass", rng=our_rng
)
foah5_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.73", type="weierstrass", rng=our_rng
)
foah6_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.82", type="weierstrass", rng=our_rng
)
foah7_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.96", type="weierstrass", rng=our_rng
)
foah8_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.106", type="weierstrass", rng=our_rng
)
foah9_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.118", type="weierstrass", rng=our_rng
)
foah10_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.130", type="weierstrass", rng=our_rng
)
foah11_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.142", type="weierstrass", rng=our_rng
)
foah12_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.155", type="weierstrass", rng=our_rng
)
foah13_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.181", type="weierstrass", rng=our_rng
)
foah14_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.168", type="weierstrass", rng=our_rng
)
foah15_weier = literature_model(;
  arxiv_id="1408.4808", equation="3.190", type="weierstrass", rng=our_rng
)
...
@testset "Test Weierstrass form of models in F-theory on all toric hypersurfaces, defined over arbitrary base" begin
  ...
  @test model_description(foah1_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_1"
  @test model_description(foah2_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_2"
  @test model_description(foah3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_3"
  @test model_description(foah4_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_4"
  @test model_description(foah5_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_5"
  @test model_description(foah6_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_6"
  @test model_description(foah7_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_7"
  @test model_description(foah8_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_8"
  @test model_description(foah9_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_9"
  @test model_description(foah10_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_10"
  @test model_description(foah11_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_11"
  @test model_description(foah12_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_12"
  @test model_description(foah13_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_13"
  @test model_description(foah14_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_14"
  @test model_description(foah15_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_15"
  @test model_description(foah16_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_16"
end
...
foah2_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.12",
  type="weierstrass",
  base_space=B3,
...
foah3_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.54",
  type="weierstrass",
  base_space=B3,
...
foah4_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.17",
  type="weierstrass",
  base_space=B3,
...
foah5_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.73",
  type="weierstrass",
  base_space=B3,
...
foah7_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.96",
  type="weierstrass",
  base_space=B3,
...
foah8_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.106",
  type="weierstrass",
  base_space=B3,
...
foah9_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.118",
  type="weierstrass",
  base_space=B3,
...
foah10_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.130",
  type="weierstrass",
  base_space=B3,
...
foah11_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.142",
  type="weierstrass",
  base_space=B3,
...
foah12_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.155",
  type="weierstrass",
  base_space=B3,
...
foah14_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.168",
  type="weierstrass",
  base_space=B3,
...
foah15_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.190",
  type="weierstrass",
  base_space=B3,
...
foah16_B3_weier = literature_model(;
  arxiv_id="1408.4808",
  equation="3.203",
  type="weierstrass",
  base_space=B3,
...
@testset "Test Weierstrass form of models in F-theory on all toric hypersurfaces, defined over concrete base" begin
  ...
  @test model_description(foah1_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_1"
  @test model_description(foah2_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_2"
  @test model_description(foah3_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_3"
  @test model_description(foah4_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_4"
  @test model_description(foah5_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_5"
  @test model_description(foah6_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_6"
  @test model_description(foah7_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_7"
  @test model_description(foah8_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_8"
  @test model_description(foah9_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_9"
  @test model_description(foah10_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_10"
  @test model_description(foah11_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_11"
  @test model_description(foah12_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_12"
  @test model_description(foah13_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_13"
  @test model_description(foah14_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_14"
  @test model_description(foah15_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_15"
  @test model_description(foah16_B3_weier) ==
    "F-theory weierstrass model dual to hypersurface model with fiber ambient space F_16"
  @test parent(explicit_model_sections(foah1_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah1_B3_weier))
  @test parent(explicit_model_sections(foah2_B3_weier)["b7"]) ==
    coordinate_ring(base_space(foah2_B3_weier))
  @test parent(explicit_model_sections(foah3_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah3_B3_weier))
  @test parent(explicit_model_sections(foah4_B3_weier)["d4"]) ==
    coordinate_ring(base_space(foah4_B3_weier))
  @test parent(explicit_model_sections(foah5_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah5_B3_weier))
  @test parent(explicit_model_sections(foah6_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah6_B3_weier))
  @test parent(explicit_model_sections(foah7_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah7_B3_weier))
  @test parent(explicit_model_sections(foah8_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah8_B3_weier))
  @test parent(explicit_model_sections(foah9_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah9_B3_weier))
  @test parent(explicit_model_sections(foah10_B3_weier)["s5"]) ==
    coordinate_ring(base_space(foah10_B3_weier))
  @test parent(explicit_model_sections(foah11_B3_weier)["s5"]) ==
    coordinate_ring(base_space(foah11_B3_weier))
  @test parent(explicit_model_sections(foah12_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah12_B3_weier))
  @test parent(explicit_model_sections(foah13_B3_weier)["s1"]) ==
    coordinate_ring(base_space(foah13_B3_weier))
  @test parent(explicit_model_sections(foah14_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah14_B3_weier))
  @test parent(explicit_model_sections(foah15_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah15_B3_weier))
  @test parent(explicit_model_sections(foah16_B3_weier)["s7"]) ==
    coordinate_ring(base_space(foah16_B3_weier))
  @test [k[2:3] for k in singular_loci(foah1_B3_weier; rng=our_rng)] == [((0, 0, 1), "I_1")]
  @test [k[2:3] for k in singular_loci(foah11_B3_weier; rng=our_rng)] ==
    [((0, 0, 1), "I_1"), ((0, 0, 2), "Non-split I_2"), ((0, 0, 3), "Split I_3")]
end
