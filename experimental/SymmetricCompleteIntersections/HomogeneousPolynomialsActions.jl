...
function action_on_ideal(rep::LinRep, I::S) where S <: MPolyIdeal{<: MPolyElem_dec}
  ...
  gene = minimal_generating_set(I)
  d = degree(gene[1])
  @req all(f -> degree(f) == d, gene) "All generators of I should have same total degree"
  ...
  M = [reverse((RdtoR\f).v) for f in gene]
  M = reduce(vcat, M)
...
function symmetric_complete_intersections(prep::ProjRep, d::Int, t::Int; j = nothing, check::Bool = true)
  ...
  M = moduli_space_of_submodules(rd, t)
...
function symmetric_complete_intersections(G::Oscar.GAPGroup, n::Int, d::Int, t::Int)
  pfr = projectively_faithful_representations(G, n)
...
function _induced_action_symplectic_form(rep::LinRep, I::T) where T <: MPolyIdeal{<: MPolyElem_dec}
  @assert sum(degree.(gens(I))) == ngens(parent(I)) + 1
  repI = action_on_ideal(rep, I)
  mr = matrix_representation(rep)
  mrI = matrix_representation(repI)
  e = exponent(underlying_group(rep))
  F, _ = cyclotomic_field(e, cached = false)
  coll = [ F(data(det(mr[i])//det(mrI[i]))) for i in 1:length(mr)]
  mg = matrix_group(mr)
  img = matrix_group(coll)
  return hom(mg, img, mr, coll)
end
...
function _is_symplectic_action(prep::ProjRep, I) 
  f = _induced_action_symplectic_form(linear_lift(prep), I)
...
function _is_purely_non_symplectic_action(prep::ProjRep, I)
  f = _induced_action_symplectic_form(linear_lift(prep))
