@attributes mutable struct LieAlgebraExteriorPowerModule{C<:RingElement} <:
                           LieAlgebraModule{C}
  ...
  function LieAlgebraExteriorPowerModule{C}(
    ...
    return get_cached!(
      LieAlgebraExteriorPowerModuleDict, (inner_mod, power), cached
    ) do
...
const LieAlgebraExteriorPowerModuleDict = CacheDictType{
  Tuple{LieAlgebraModule,Int},LieAlgebraExteriorPowerModule
}()
...
parent_type(::Type{LieAlgebraExteriorPowerModuleElem{C}}) where {C<:RingElement} =
  LieAlgebraExteriorPowerModule{C}

elem_type(::Type{LieAlgebraExteriorPowerModule{C}}) where {C<:RingElement} =
  LieAlgebraExteriorPowerModuleElem{C}

parent(v::LieAlgebraExteriorPowerModuleElem{C}) where {C<:RingElement} = v.parent

base_ring(V::LieAlgebraExteriorPowerModule{C}) where {C<:RingElement} =
...
function transformation_matrix_by_basisindex(
  V::LieAlgebraExteriorPowerModule{C}, i::Int
) where {C<:RingElement}
