@attributes mutable struct LieAlgebraTensorPowerModule{C<:RingElement} <:
                           LieAlgebraModule{C}
...
const LieAlgebraTensorPowerModuleDict = CacheDictType{
  Tuple{LieAlgebraModule,Int},LieAlgebraTensorPowerModule
}()
...
parent_type(::Type{LieAlgebraTensorPowerModuleElem{C}}) where {C<:RingElement} =
  LieAlgebraTensorPowerModule{C}

elem_type(::Type{LieAlgebraTensorPowerModule{C}}) where {C<:RingElement} =
  LieAlgebraTensorPowerModuleElem{C}
...
function transformation_matrix_by_basisindex(
  V::LieAlgebraTensorPowerModule{C}, i::Int
) where {C<:RingElement}
  if V.transformation_matrix_cache[i] === nothing
    y = transformation_matrix_by_basisindex(V.inner_mod, i)::dense_matrix_type(C)
    V.transformation_matrix_cache[i] = sum(
      reduce(kronecker_product, (j == i ? y : one(y) for j in 1:(V.power))) for
      i in 1:(V.power)
    )
  end
