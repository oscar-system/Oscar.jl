@attributes mutable struct LieAlgebraSymmetricPowerModule{C<:RingElement} <:
                           LieAlgebraModule{C}
  ...
  function LieAlgebraSymmetricPowerModule{C}(
    inner_mod::LieAlgebraModule{C}, power::Int; cached::Bool=true
  ) where {C<:RingElement}
    return get_cached!(
      LieAlgebraSymmetricPowerModuleDict, (inner_mod, power), cached
    ) do
      ind_map = collect(
      ...
      new{C}(
        inner_mod, power, ind_map, transformation_matrix_cache
      )
    end::LieAlgebraSymmetricPowerModule{C}
  end
end

const LieAlgebraSymmetricPowerModuleDict = CacheDictType{
  Tuple{LieAlgebraModule,Int},LieAlgebraSymmetricPowerModule
}()
...
parent_type(::Type{LieAlgebraSymmetricPowerModuleElem{C}}) where {C<:RingElement} =
  LieAlgebraSymmetricPowerModule{C}

elem_type(::Type{LieAlgebraSymmetricPowerModule{C}}) where {C<:RingElement} =
  LieAlgebraSymmetricPowerModuleElem{C}

parent(v::LieAlgebraSymmetricPowerModuleElem{C}) where {C<:RingElement} = v.parent

base_ring(V::LieAlgebraSymmetricPowerModule{C}) where {C<:RingElement} =
  base_ring(V.inner_mod)
...
function transformation_matrix_by_basisindex(
  V::LieAlgebraSymmetricPowerModule{C}, i::Int
) where {C<:RingElement}
