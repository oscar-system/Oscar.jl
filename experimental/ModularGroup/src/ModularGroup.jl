...
function _perm_group(G::ModularGroup)
  if isnothing(G.perm_group_cache)
    G.perm_group_cache, _ = sub(symmetric_group(index(G)), [s_right_action(G), t_right_action(G)])
  end
  return G.perm_group_cache
end

# cache the homomorphism from SL2Z to P
function _coset_action_hom(G::ModularGroup)
  if isnothing(G.coset_action_hom_cache)
    SL2Z, _, _ = _SL2Z_fp()
    P = _perm_group(G)
    G.coset_action_hom_cache = hom(SL2Z, P, [s_right_action(G), t_right_action(G)])
  end
  return G.coset_action_hom_cache
end
