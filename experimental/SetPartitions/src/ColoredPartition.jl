...
Construct a `ColoredPartition` from `partition` with colors given by `upper_colors` and `lower_colors`.
...
function colored_partition(
    ...
    return colored_partition(set_partition(upper_points, lower_points), upper_colors, lower_colors)
end
...
The tensor product of two colored partitions is given by their horizontal concatenation.
See also Section 1.2 in [TW18](@cite) and `tensor_product(::SetPartition, ::SetPartition)`.
...
See also Section 1.2 in [TW18](@cite) and `compose_count_loops(::SetPartition, ::SetPartition)`.
...
function compose_count_loops(p::ColoredPartition, q::ColoredPartition)
    ...
    @req p.color_upper_points == q.color_lower_points "p upper and q lower colors are different in composition"
...
to the lower points or vice verca and inverts its color. See also Section 1.2 in [TW18](@cite)
and `rotate(::SetPartition, ::Bool, ::Bool)`.
