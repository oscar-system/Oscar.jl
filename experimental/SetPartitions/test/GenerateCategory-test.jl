@testset "GenerateCategory test" begin
    @testset "Classical Partitions" begin
        ...
        for partition in nonc2
            @test is_pair(partition) && is_noncrossing(partition) && SetPartitions.size(partition) == 10
        end

        nonc = construct_category([SetPartition([1], [1]), SetPartition([], [1, 1]), SetPartition([1], [1, 1])], 6)
        @test length(nonc) <= 924
        for partition in nonc
            @test is_noncrossing(partition) && SetPartitions.size(partition) == 6
        end
        ...
        for partition in balanced
            ...
            @test is_balanced(partition) && SetPartitions.size(partition) == 6
        end
    ...
    @testset "Colored Partitions" begin
        ...
        for partition in nonc2
            @test is_pair(partition) && is_noncrossing(partition) && partition isa ColoredPartition && partition != ColoredPartition(SetPartition([1], [1]), [0], [1]) && SetPartitions.size(partition) == 6
        end
        ...
        for partition in nonc2
            @test is_pair(partition) && is_noncrossing(partition) && partition isa ColoredPartition && SetPartitions.size(partition) == 4
        end
    end
    ...
    @testset "Spatial Partitions" begin
        ...
        @test length(P2) <= 105
        for partition in P2
            @test is_pair(partition) && partition isa SpatialPartition && SetPartitions.size(partition) == 6
        end
        ...
        for partition in nonc2
            @test is_pair(partition) && is_noncrossing(partition.partition) && SetPartitions.size(partition) == 8 && partition.partition in nonc2classic
        end
    end
end
