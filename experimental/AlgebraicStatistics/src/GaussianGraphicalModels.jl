...
@attr function parameter_ring(GM::GaussianGraphicalModel{Directed, Nothing}; cached=false)
  G = graph(GM)
...
function parameter_ring_gens(GM::GaussianGraphicalModel{Directed, Nothing})
  G = graph(GM)
  R = parameter_ring(GM)
...
function concentration_matrix(M::GaussianGraphicalModel{Undirected, Nothing})
  ...
  K = zero_matrix(parameter_ring(M), n_vertices(G), n_vertices(G))
