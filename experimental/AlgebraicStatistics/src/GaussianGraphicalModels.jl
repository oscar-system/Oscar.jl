...
random_variables(GR::GaussianRing) = collect(1:nrows(covariance_matrix(GR)))

@doc raw"""
    ci_statements(GR::GaussianRing)

Return all the `CIStmt` objects which can be formed on the `random_variables(GR)`.

```jldoctest
julia> GR = gaussian_ring(3)
Gaussian ring over Rational field in 6 variables
s[1, 1], s[1, 2], s[1, 3], s[2, 2], s[2, 3], s[3, 3]

julia> ci_statements(GR)
...
```
"""
ci_statements(GR::GaussianRing) = ci_statements(random_variables(GR))

@doc raw"""
    ci_ideal(GR::GaussianRing, stmts::Vector{CIStmt})
...
julia> ci_ideal(GR, [CI"1,2|", CI"1,2|3"])
Ideal generated by
  s[1, 2]
  s[1, 2]*s[3, 3] - s[1, 3]*s[2, 3]
```
"""
function ci_ideal(GR::GaussianRing, stmts::Vector{CIStmt})
  eqs = Any[]
  A = covariance_matrix(GR)
  for stmt in stmts
    k = length(stmt.K)
    B = A[vcat(stmt.I, stmt.K), vcat(stmt.J, stmt.K)]
    append!(eqs, minors(B, k+1))
  end
  return ideal(GR.ring, eqs)
end

@doc raw"""
    ci_polynomial(GR::GaussianRing, stmt::CIStmt)
...
julia> ci_polynomial(GR, [CI"1,2|3"])
s[1, 2]*s[3, 3] - s[1, 3]*s[2, 3]
```
"""
...
ci_polynomial(GR::GaussianRing, stmt::CIStmt) = ci_polynomial(covariance_matrix(GR), stmt)

@doc raw"""
    ci_structure(GR::GaussianRing; strategy::Symbol)
...
1. `parametrization(GR)` returning a parametrized covariance matrix. The CI structure is determined by evaluating its subdeterminants.
2. `vanishing_ideal(GR)` returning the model's vanishing ideal. In this case, the subdeterminant for each CI statement is checked for contained in the vanishing ideal.
...
function ci_structure(GR::GaussianRing; strategy::Symbol)
  ...
  elseif strategy == :ideal || hasmethod(vanishing_ideal, Tuple{typeof(GR)})
    A = covariance_matrix(GR)
    I = vanishing_ideal(GR)
    return [stmt for stmt in ci_statements(GR) if ci_polynomial(A, stmt) in I]
  end
