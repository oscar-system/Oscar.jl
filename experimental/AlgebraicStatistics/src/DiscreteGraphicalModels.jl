...
function parametrization(M::DiscreteGraphicalModel{Graph{Undirected}, L}) where L
  ...
  images = elem_type(R)[]
  sorted_keys = sort(collect(keys(td)); by=x ->x[2])
  for p in gens(S)
    s = gen_index(S, p) # get label of the variable
    push!(images, prod(td[k] for k in sorted_keys if k[2] == s[sort(collect(k[1]))]))
  end
  hom(S, R, images)
end
...
```
"""
function parametrization(M::DiscreteGraphicalModel{Graph{Directed}})
  G = graph(M)
  n = n_vertices(G)
  S, pd = model_ring(M)
