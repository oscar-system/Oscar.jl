...
@doc raw"""
    map_word(w::WeylGroupElem, genimgs::Vector; init = nothing)

If `init` is `nothing` and `word(w) = [UInt(`$i_1$`), ..., UInt(`$i_n$`)]`, then return the product $R_1 R_2 \cdots R_n$
with $R_j =$ `genimgs[`$i_j$`]`.
...
```
"""
function map_word(w::WeylGroupElem, genimgs::Vector; init=nothing)
  @req length(genimgs) == number_of_generators(parent(w)) "Length of vector of images does not equal rank of Weyl group"
  return Oscar.map_word([Int(i) for i in word(w)], genimgs, init=init)
end

@doc raw"""
    parabolic_subgroup(W::WeylGroup, vec::Vector{<:Integer}, w::WeylGroupElem) -> WeylGroup, Map{WeylGroup, WeylGroup}

Returns two objects: The subgroup `U` of `W` generated by `[inv(w)*u*w for u in gens(W)[vec]]`, whose generators are set to be this vector and which is itself a `WeylGroup`, and the embedding of `U` into `W`. The elements of `vec` must be pairwise distinct integers in `1:number_of_generators(W)` and `vec` must be non-empty.
"""
function parabolic_subgroup(W::WeylGroup, vec::Vector{<:Integer}, w::WeylGroupElem=one(W))
  @req length(unique(vec)) == length(vec) "Elements of vector are not pairwise distinct"
...
Returns a triple `(U, emb, proj)` that describes a factor of `W`, that is, a product of irreducible factors.
Here the parabolic subgroup `U` and its embedding into `W` are as returned by [`parabolic_subgroup`](@ref)`(W, vec)`
and `proj` is the projection map from `W` onto `U`.
If `U` is not a factor of `W`, an error occurs.
"""
function parabolic_subgroup_with_projection(W::WeylGroup, vec::Vector{<:Integer}; check_factor::Bool=true)
  if check_factor
    ...
    for i in vec
      for j in setdiff(1:number_of_generators(W), vec)
        @req coxeter_matrix_entry_from_cartan_matrix(cm, i, j) == 2 "Input vector does not describe a factor of the Weyl group, so there is no projection homomorphism"
      end;
    end
  end
