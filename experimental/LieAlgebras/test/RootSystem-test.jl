@testset "LieAlgebras.RootSystem" begin
  ...
  @testset "property tests" begin
    function root_system_property_tests(R::RootSystem, rk::Int, npositive_roots::Int)
      @test rank(R) == rk
      @test num_simple_roots(R) == rk
      @test num_positive_roots(R) == npositive_roots
      @test num_roots(R) == 2 * npositive_roots

      @test length(simple_roots(R)) == num_simple_roots(R)
      @test length(positive_roots(R)) == num_positive_roots(R)
      @test length(negative_roots(R)) == num_positive_roots(R)
      @test length(roots(R)) == num_roots(R)
      ...
      @test length(simple_coroots(R)) == num_simple_roots(R)
      @test length(positive_coroots(R)) == num_positive_roots(R)
      @test length(negative_coroots(R)) == num_positive_roots(R)
      @test length(coroots(R)) == num_roots(R)
