...
function (fac::TensorProductFactory{<:OFPModule})(dc::AbsDoubleComplexOfMorphisms, i::Int, j::Int)
  M = fac.C1[i]
  N = fac.C2[j]
...
function (fac::VerticalTensorMapFactory{<:OFPModuleHom})(dc::AbsDoubleComplexOfMorphisms, i::Int, j::Int)
  dom = dc[i, j]
...
function (fac::HorizontalTensorMapFactory{<:OFPModuleHom})(dc::AbsDoubleComplexOfMorphisms, i::Int, j::Int)
  dom = dc[i, j]
...
morphism_type(::Type{T}) where {T<:OFPModule} = OFPModuleHom

zero(phi::OFPModuleHom) = hom(domain(phi), codomain(phi), [zero(codomain(phi)) for i in 1:ngens(domain(phi))])
zero_morphism(dom::OFPModule, cod::OFPModule) = hom(dom, cod, [zero(cod) for i in 1:ngens(dom)])
...
function (fac::TensorProductChainFactory{T})(HC::AbsHyperComplex, i::Tuple) where {T<:OFPModule}
  d = length(i)
...
function (fac::TensorProductMapFactory{T})(HC::AbsHyperComplex, p::Int, i::Tuple) where {T<:OFPModuleHom}
  d = length(i)
  v = collect(i)
...
function _tensor_product(v::OFPModule...)
...
function zero_map(M::OFPModule, N::OFPModule)
...
function tensor_product(M::OFPModule{T}, Ms::OFPModule{T}...) where {U<:MPolyComplementOfPrimeIdeal, T<:MPolyLocRingElem{<:Any, <:Any, <:Any, <:Any, U}}
  R = base_ring(M)
