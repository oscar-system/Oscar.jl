...
#this function sorts the list of polynomials by total degree, this helps later to reduce the list of basisminors
...
function selfprojecting_realization_matrix(M::Matroid, Bas::Vector{Int}, F::Ring;Ideal::Union{Ideal,Nothing} = nothing)
  ...
  if isnothing(Ideal) #this way the ideal needs only be computed once!
...
function selfprojecting_realization_space(m::Matroid;
  ...
  M = selfprojecting_realization_matrix(goodM, goodB, RS.ground_ring; Ideal = I) #does not return a tuple of ring and matrix like it does for realization_space 
  if M == nothing 
