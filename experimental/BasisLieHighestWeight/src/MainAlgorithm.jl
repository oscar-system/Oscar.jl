...
function basis_coordinate_ring_kodaira_compute(
  ...
  for i in 2:degree
    ...
    for k in 1:(i - 1)
      ...
      if (
        length(set_mon_sum) == GAP.Globals.DimensionOfHighestWeightModule(
    ...
    if length(set_mon_sum) == GAP.Globals.DimensionOfHighestWeightModule(
      ...
      println(
        ...
        length(set_mon_sum),
        " and one needs ",
        GAP.Globals.DimensionOfHighestWeightModule(
          L.lie_algebra_gap, GAP.Obj(Int.(i * highest_weight))
        ),
      )
      set_mon = compute_monomials(
  ...
  println(
    "The number of monomials in the basis of ",
    degree * highest_weight,
    " that are not products of basis elements in smaller degree are ",
  )
