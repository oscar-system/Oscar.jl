...
# save all highest weights, for which the Minkowski-sum did not suffice to gain all monomials
no_minkowski = Set{Vector{ZZRingElem}}()
set_mon_k = [Set{ZZMPolyRingElem}() for _ in 1:degree] #monomial basis of the module k*highest_weight
set_mon_missing = [Set{ZZMPolyRingElem}() for _ in 1:degree] #store the monomials that are not products of basis monomials of smaller degree
# start recursion over highest_weight
set_mon = compute_monomials(
  L,
  birational_sequence,
...
list_of_generators = zeros(Int, degree)
union!(set_mon_k[1], set_mon)
set_mon_missing[1] = set_mon
...
for i in 2:degree
  ...
  for k in 1:i-1
    ...
    if (length(set_mon_sum) == GAP.Globals.DimensionOfHighestWeightModule(
      L.lie_algebra_gap, GAP.Obj(Int.(i*highest_weight))))
      break
    end
  end  
  if length(set_mon_sum) == GAP.Globals.DimensionOfHighestWeightModule(
    L.lie_algebra_gap, GAP.Obj(Int.(i*highest_weight)))
    #println(i*highest_weight, " is generated by smaller weights")
    union!(set_mon_k[i], set_mon_sum)
    #set_mon_k[i] = set_mon_sum
  else 
    println(" for ", i*highest_weight, " it is ", length(set_mon_sum), 
    " and one needs ", GAP.Globals.DimensionOfHighestWeightModule(
      L.lie_algebra_gap, GAP.Obj(Int.(i*highest_weight))))
    set_mon = compute_monomials(
      L,
      birational_sequence,
    ...
    #end 
    #println("for ", i, " we added ", length(set_mon_missing[i]), " monomials ")
    union!(set_mon_k[i], set_mon)
    #println(" weights up to " , i*highest_weight, " are neded ")
  end  
end  

#println(list_of_generators) 
println("The number of monomials in the basis of ", degree*highest_weight, " that are not products of basis elements in smaller degree are ")
for s in set_mon_missing
  print(length(s), "  ")
end
println(" ") 
