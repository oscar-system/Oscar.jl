...
function monomial_from_degrees(
  ...
  if length(degs) != length(vars)
    throw(
      ArgumentError(
        "Length of degree vector must match the number of variables in the polynomial ring!"
      ),
    )
  end
...
function is_monomial(
  p::AbstractAlgebra.Generic.LaurentMPolyWrapRing{ZZRingElem,ZZMPolyRing}
)
...
function demazure_operator_monom(
  ...
  beta::Int,
  beta_wi::Vector{Int},
  e_lambda::AbstractAlgebra.Generic.LaurentMPolyWrap{
    ZZRingElem,
    ZZMPolyRingElem,
    AbstractAlgebra.Generic.LaurentMPolyWrapRing{ZZRingElem,ZZMPolyRing},
  },
)
