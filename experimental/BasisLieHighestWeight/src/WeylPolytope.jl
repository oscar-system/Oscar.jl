...
Given a birational sequence, the action reads from right to left. If for the given highest_weight, the right most root vector acts by zero because it 
is in the annihilator of the highest_weight vector, then we do not need to consider this coordinate at all when determining the possible monomials.
This extends then further to the left to all coordinates while the annihilator is then the annihilator of the sudmodule generated by all root vectors at the right end of the birational sequence.
"""
