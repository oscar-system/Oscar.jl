...
Return `true` if `g` is a reflection, `false` otherwise. By a reflection, we mean
a matrix with fixed space of codimension 1.
"""
...
@doc raw"""
    subgroup_of_reflections(G::MatrixGroup)

Return the subgroup `H` of `G` generated by the reflections in `G` and an
embedding `H \to G`.

See also [`is_reflection`](@ref).
"""
...
Return `true` if `G` does not contain any reflections and `false` otherwise.
