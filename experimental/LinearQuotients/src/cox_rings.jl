...
function cox_ring(
  ...
  RH = invariant_ring(H)
  QH, QHtoR = affine_algebra(RH; algo_gens=algo_gens, algo_rels=algo_rels)
  S = base_ring(QH)
  StoR = hom(S, codomain(QHtoR), [QHtoR(f) for f in gens(QH)])
  ...
  if is_trivial(A)
    T0 = grade(forget_grading(S), [zero(A) for i in 1:ngens(S)])[1]

    relsT = elem_type(T0)[T0(forget_grading(r)) for r in gens(modulus(QH))]

    Q, TtoQ = quo(T0, ideal(T0, relsT))
  ...
  StoT = hom(S, Tu, [phi(gen(Tu, i)) for i in 1:ngens(Tu)])
  relsT = elem_type(Tu)[StoT(r) for r in gens(modulus(QH))]
  T = grade(Tu, degrees)[1]
...
function cox_ring_of_qq_factorial_terminalization(L::LinearQuotient)
  ...
  Sdeg, _ = grade(S, degree.(StoR.(gens(S))))
