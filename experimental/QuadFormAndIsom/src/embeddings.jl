...
@doc raw"""
    _sum_with_embeddings_orthogonal_groups(
      A::TorQuadModule,
      B::TorQuadModule
    ) -> TorQuadModule,
         TorQuadModuleMap,
...
direct sum in `D`, we can embed `O(A)` and `O(B)` in `O(D)` by setting the
identity on the complement.
...
for gluing lattices in a $p$-admissible triples are actually submodules of such
V's.
"""
function _get_V(
    f::TorQuadModuleMap,
...
!!!
Warning:
No sanity check are performed: in particular, if $D_A$ and $D_B$ are not in
orthogonal direct sum, if $\gamma$ is not a gluing, or if it is not
...
The default value, in case `compute_bar_Gf == false`, is set to be the identity
morphism of $O(D)$. This 
"""
...
@doc raw"""
    _change_to_bilinear_module(
      qM::TorQuadModule,
      GM::AutomorphismGroup{TorQuadModule},
      fqM::TorQuadModuleMap
    ) -> TorQuadModule, AutomorphismGroup{TorQuadModule}, TorQuadModuleMap

Given a torsion quadratic module $q_M$, return the associated torsion bilinear
module. Here $G_M$ and $f_{q_M}$ are respectively a group of isometries and
an isometry of $q_M$, for the quadratic form. The function also returns the
associated group of isometries and isometry for the torsion bilinear module
associated to $q_M$.
"""
...
# fit in a given gluing context, i.e. for which a given glue map is equivariant.
# These isometries all live in a same coset, and we identity two isometries in
# Let $N$ be the associated lattice.
# - `OqfN` here is the image of $O(N)\to O(qN)$ where `qN` is the discriminant group.
# - `HNinqN` is the embedding of the glue domain `HN` into `qN`.
# - `phig` is the glue map between a module `HM` and `HN`.
# - `fHM` is the isometry of `HM` which should coincide with the isometries of $N$ along the gluing `phig`.
# - `discrep` is the map $O(N)\to O(qN)$.
# - `stabN` is the stabilizer of `HN` in `GN`, where `GN` is a classifying group for $N$.
...
The content of $V$ depends on the value of `classification`. There are 6
possibilities:
