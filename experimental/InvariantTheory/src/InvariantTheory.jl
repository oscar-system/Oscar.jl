...
@attr MPolyIdeal function null_cone_ideal(R::RedGroupInvarRing)
    ...
    return ideal(gens(Z.group, I, Z.rep_mat))
end
...
function fundamental_invariants(z::RedGroupInvarRing) #unable to use abstract type
    if !isdefined(z, :fundamental)
        ...
        null_cone_ideal(z) = ideal(gens(R.group, I, R.rep_mat))
...
function gens(G::LinearlyReductiveGroup, X::MPolyIdeal, rep_mat::MatElem)
    n = ncols(rep_mat)
    gbasis = gens(X) 
