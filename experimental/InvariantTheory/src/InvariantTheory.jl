...
#we should in fact write a new inv_generators function in this case. 
...
function no_reynolds_basis(d::Int, mat_::MatElem, II::MPolyIdeal)
    ...
    H = collect(monomials_of_degree(mixed_ring, d, [i for i in 1:n]))#we only need monomials generated by xi!!
    ...
    #M2 = matrix(mixed_ring,1,1,[0])
    
    #this only gives a single solution. We need a basis of a solution space.
    #@show kernel(M1)
    #boo, sol = can_solve(M1,M2)
    #@assert boo == true
    
    #compute the size of the vector space required
    #check_list = Vector{Vector{Int}}()
    #space_vect = Vector{Vector{Int}}()
    
    #for i in 1:length(V)
    #    coeffs = collect(coefficients(V[i]))
    #    for j in 1:length(V[i])
    #        v = exponent_vector(V[i],j)
    #        if !(v in check_list)
    #            push!(check_list,v)
    #        end
    #    end
    ...
    #alternatively, 
    #check what the largest degree of a monomial in any polynomial of V. Let it be max_
    #compute the numer of monomials in x and z of degree <= max_
    #that is the dimension of the vector space 
