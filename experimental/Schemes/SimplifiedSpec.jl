...
function simplify(X::AbsSpec{<:Field})
  L, f, g = simplify(OO(X))
  Y = Spec(L)
  YtoX = affine_morphism(Y, X, f, check=false)
  XtoY = affine_morphism(X, Y, g, check=false)
...
function _flatten_open_subscheme(
    ...
    iso::AbsSpecMor=begin
      ...
      f = affine_morphism(U, UU, hom(OO(UU), OO(U), gens(OO(U)), check=false), check=false)
      f_inv = affine_morphism(UU, U, hom(OO(U), OO(UU), gens(OO(UU)), check=false), check=false)
  ...
  ident = affine_morphism(UV, WV, hom(OO(WV), OO(UV), gens(OO(UV)), check=false), check=false)
  inv_ident = affine_morphism(WV, UV, hom(OO(UV), OO(WV), gens(OO(WV)), check=false), check=false)
...
function _flatten_open_subscheme(
  ...
  WV = PrincipalOpenSubset(W, OO(W).([lifted_numerator(hU), lifted_numerator(hV)]))
  ident = affine_morphism(UV, WV, hom(OO(WV), OO(UV), gens(OO(UV)), check=false), check=false)
  ident_inv = affine_morphism(WV, UV, hom(OO(UV), OO(WV), gens(OO(WV)), check=false), check=false)
  new_iso =  compose(iso, ident)
...
function _flatten_open_subscheme(
  ...
  ident = affine_morphism(UV, WV, 
                  ...
                  check=false)
  new_iso =  compose(iso, ident)
  ident_inv = affine_morphism(WV, UV, 
                      hom(OO(UV), OO(WV), 
