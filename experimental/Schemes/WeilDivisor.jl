...
# but that equation is wrong (it always produces the unit ideal). 
# We correct it as follows. Let τ ∈ Σ be a ray and σ ∈ Σ a maximal 
# cone. The ideal of the intersection of the orbit with the chart 
#   I(O(τ) ∩ Uσ) ⊂ ℂ[σ ̌∩ M] 
# is either 
#   1) the unit ideal if τ ⊄ σ
#   2) the ideal generated by the elements in 
#         σ ̌∩ M ∖ τ⟂ 
#      i.e. the elements of σ ̌∩ M which are not in the annihilator 
#      of τ. 
