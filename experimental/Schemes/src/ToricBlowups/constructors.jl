...
Let us now blow up the smooth, 2-dimensional projective space
at a ray that is already a ray of the projective space.
Hence, the blowdown will be an isomorphism.
...
julia> bl = blow_up(p2, [1, 0])
Toric blowdown morphism

julia> domain(bl) == codomain(bl)
true
...
julia> exceptional_divisor(bl)
Torus-invariant, non-prime divisor on a normal toric variety
```

Finally a more interesting example. The new ray is a ray of the initial
toric variety, but the blowup will not be an isomorphism.

# Examples
```jldoctest
julia> R = matrix(QQ, [[1,0,0],[1,1,0],[1,0,1],[1,1,1]]);

julia> IM = IncidenceMatrix([[1,2,3,4]]);

julia> f = polyhedral_fan(IM, R)
Polyhedral fan in ambient dimension 3

julia> v = normal_toric_variety(f)
Normal toric variety
...
julia> bl = blow_up(v, [1,0,0])
Toric blowdown morphism

julia> new_v = domain(bl)
Normal toric variety

julia> n_maximal_cones(new_v)
2

julia> n_rays(new_v) == n_rays(new_v)
true

julia> center(bl)
Sheaf of ideals
  on normal, non-simplicial toric variety
with restriction
  1: Ideal (x_4_1, x_1_1)
...
function blow_up(v::NormalToricVariety, new_ray::AbstractVector{<:IntegerUnion}; coordinate_name::String = "e")
  ...
  if n_rays(v) == n_rays(new_variety) && n_maximal_cones(v) == n_maximal_cones(new_variety)
    # Compute the identity blowdown morphism
    return ToricBlowdownMorphism(v, new_ray)
  end
  if n_rays(v) == n_rays(new_variety) && n_maximal_cones(v) != n_maximal_cones(new_variety)
    return ToricBlowdownMorphism(v, new_ray, new_variety)
  end
...
Here is an example, where the blowup leads to an isomorphism since
the corresponding ray is already contained in the fan of the initial toric variety.
