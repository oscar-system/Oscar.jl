...
function twisting_sheaf(IP::AbsProjectiveScheme{<:Field}, d::Int)
  ...
  for G in values(glueings(C))
    ...
    MG[(U, V)] = (d>= 0 ? (x->OO(VV)(x, check=false))(h_V^d) : (inv((x->OO(VV)(x, check=false))(h_V))^(-d)))*identity_matrix(OO(VV), 1)
    MG[(V, U)] = (d>= 0 ? (x->OO(UU)(x, check=false))(h_U^d) : (inv((x->OO(UU)(x, check=false))(h_U))^(-d)))*identity_matrix(OO(UU), 1)
  end
