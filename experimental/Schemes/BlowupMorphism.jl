...
@attr AbsSpecMor function isomorphism_on_open_subset(f::BlowupMorphism)
  ...
  phi = affine_morphism(U_flat, V_flat, pullback(inverse(iso_U)).(pullback(pr_res).(pullback(iso_V).(gens(OO(V_flat))))), check=false)
  phi_inv = affine_morphism(V_flat, U_flat, pullback(inverse(iso_V)).(pullback(inverse(pr_res)).(pullback(iso_U).(gens(OO(U_flat))))), check=false)
