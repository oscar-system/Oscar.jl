...
function _compute_inherited_glueing(gd::InheritGlueingData)
  ...
  if success
    ...
    if iszero(h_X*h_Y)
      ...
      g = affine_morphism(YX, XY, hom(OO(XY), OO(YX), [zero(OO(YX)) for i in 1:ngens(OO(XY))], check=false), check=false)
      f = affine_morphism(XY, YX, hom(OO(YX), OO(XY), [zero(OO(XY)) for i in 1:ngens(OO(YX))], check=false), check=false)
    ...
    g = affine_morphism(YX, XY, hom(OO(XY), OO(YX), x_img, check=false), check=false)
    ...
    f = affine_morphism(XY, YX, hom(OO(YX), OO(XY), y_img, check=false), check=false)
    return SimpleGlueing(X, Y, f, g, check=false)
  end
  ...
  x_img = pullback(phi).(x_img)
  gg = affine_morphism(YX, XY, hom(OO(XY), OO(YX), x_img, check=false), check=false)
  ...
  y_img = pullback(psi).(y_img)
  ff = affine_morphism(XY, YX, hom(OO(YX), OO(XY), y_img, check=false), check=false)
