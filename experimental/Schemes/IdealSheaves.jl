...
export is_regular_sequence, as_smooth_lci
...
function as_smooth_lci(I::IdealSheaf)
  X = scheme(I)
  C = covering(I)
  for U in patches(C)
    as_smooth_lci(U, I[U]) 
...
function as_smooth_lci(U::Spec, g::Vector{T}; verbose::Bool=false) where {T<:MPolyElem}
  ...
  s = length(g)
  Dh = jacobi_matrix(h)
  _non_degeneration_cover(subscheme(U, g), Dh, codim(subscheme(U, g)), 
