...
@Markdown.doc """
    IdealSheaf{CoveredSchemeType<:AbsCoveredScheme, CoveringType<:Covering}

A sheaf of ideals on an `AbsCoveredScheme` ``X``.

Use `I[U]` to get the ideal on an affine chart ``U`` of ``X``.
"""
...
Returns the scheme on which `I` is defined.
"""
scheme(I::IdealSheaf) = I.X

@Markdown.doc """
    covering(I::IdealSheaf)

Returns the covering with respect to which `I` is defined.
"""
...
function ideal_sheaf_type(::Type{T}) where {T<:AbsCoveredScheme}
  return IdealSheaf{T, covering_type(T)}
end

ideal_sheaf_type(X::AbsCoveredScheme) = ideal_sheaf_type(typeof(X))

@Markdown.doc """
    IdealSheaf(X::ProjectiveScheme, g::Vector{RingElemType}) where {RingElemType<:MPolyElem_dec}
...
function is_canonically_isomorphic(I::IdealSheaf, J::IdealSheaf)
  X = scheme(I)
