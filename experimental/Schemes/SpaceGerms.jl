...
function germ_at_point(X::AbsSpec, I::Union{Ideal,Vector})
  Y = SpaceGerm(X, I)
  restr_map = affine_morphism(Y, X, hom(OO(X), OO(Y), gens(OO(Y)), check=false), check=false)
  return Y, restr_map
end
...
function germ_at_point(A::LocalRing)
  X = SpaceGerm(A)
  restr_map = affine_morphism(X, X, hom(OO(X), OO(X), gens(OO(X)), check=false), check=false)
  return X, restr_map
end
...
function singular_locus(X::AbsSpaceGerm)
  ...
  Sgerm = SpaceGerm(S)
  return Sgerm, ClosedEmbedding(affine_morphism(Sgerm, X, pullback(inc), check=false), image_ideal(inc), check=false)
end
