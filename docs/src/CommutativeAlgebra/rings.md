...
```@julia
polynomial_ring(C::Ring, V::Vector{String}; internal_ordering=:lex, cached::Bool = true)
```
...
The input `internal_ordering=:lex` refers to the lexicograpical monomial ordering which specifies the default way of storing and displaying polynomials in OSCAR  (terms are sorted in descending
...
!!! note
    Caching is used to ensure that a given ring constructed from given parameters is unique in the system. For example, there is only one ring of multivariate polynomials over  $\mathbb{Z}$ with variables printing as x, y, z, and  with `internal_ordering=:lex`.
...
```
```@docs
graded_polynomial_ring(C::Ring, V::Vector{String}, W; internal_ordering=:lex)
```
